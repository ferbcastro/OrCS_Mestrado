...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "vsyscall_area_tool.log", "log file for the tool");

std::ofstream outFile; // The tool's output file for printing the loaded images.
...
static UINT32 GetKernelRelease(KERNEL_RELEASE_KIND kind)
{
    ...
    ASSERTX(UtilsGetKernelRelease(buf, sizeof(buf)));
...
VOID Instruction(INS ins, VOID* v)
{
    ...
    if (INS_IsControlFlow(ins))
    {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(InsAnalysisBefore), IARG_INST_PTR, IARG_BRANCH_TARGET_ADDR,
                       IARG_BRANCH_TAKEN, IARG_END);

        if ((kernelReleaseMajor > 5) || ((kernelReleaseMajor == 5) && (kernelReleaseMinor >= 3)))
        {
            INS_InsertCall(ins, IPOINT_TAKEN_BRANCH, MAKE_AFUNPTR(InsAnalysisBranchTaken), IARG_INST_PTR, IARG_BRANCH_TARGET_ADDR,
                           IARG_END);

            if (INS_IsValidForIpointAfter(ins))
            {
                INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(InsAnalysisAfter), IARG_BRANCH_TARGET_ADDR, IARG_END);
            }
        }
    }
    ...
    {
        outFile << "Reached ins of vsyscall area, adderss = 0x" << std::hex << insAddress << std::endl;
...
VOID vSyscallBefore(ADDRINT ip, ADDRINT num, ADDRINT arg0, ADDRINT arg1, SYSCALL_STANDARD scStd)
{
    ...
    // logging prints
    std::cout << "\n----- Callback Entry #" << ++callback_counter << " -----" << std::endl;
    std::cout << "IP              : " << std::hex << "0x" << (unsigned long)ip << std::endl;
    std::cout << "vsyscall number : " << std::dec << (int)num << std::endl;
    std::cout << "Arg 0           : 0x" << std::hex << (unsigned long)arg0 << std::endl;
    std::cout << "Arg 1           : 0x" << std::hex << (unsigned long)arg1 << std::endl;
}
...
int main(INT32 argc, CHAR* argv[])
{
    ...
    {
        std::cerr << "TOOL ERROR: Unable to open the output file." << std::endl;
        PIN_ExitProcess(1);
    }
