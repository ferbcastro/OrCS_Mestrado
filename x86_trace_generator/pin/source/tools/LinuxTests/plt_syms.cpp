...
void Before_Write(VOID* s)
{
    ...
    std::cout << "calling write@plt" << std::endl;
}
...
VOID ImageLoad(IMG img, VOID* v)
{
    if (IMG_IsMainExecutable(img))
    {
        ...
        if (RTN_Valid(rtn))
        {
            if (RTN_InsertCallProbed(rtn, IPOINT_BEFORE, AFUNPTR(Before_Write), IARG_PROTOTYPE, proto_write,
            ...
            std::cout << "Inserted probe call before write@plt: " << IMG_Name(img) << std::endl;
        }
