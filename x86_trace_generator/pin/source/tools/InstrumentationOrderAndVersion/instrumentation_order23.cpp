...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "instrumentation_order23.out",
                                   "specify file name for instrumentation order output");

// std::ofstream object for handling the output.
std::ofstream outstream;

static VOID WatchRtnReplacement(int origArg) { outstream << "WatchRtnReplacement" << std::endl; }
