...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "instrumentation_order19.out",
...
static VOID Rtn(RTN rtn, VOID* v)
{
    ...
    RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "RTN instrumentation1", IARG_CALL_ORDER,
                   CALL_ORDER_FIRST + 3, IARG_END);
    RTN_Close(rtn);
}
