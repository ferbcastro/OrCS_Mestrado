...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "instrumentation_order8.out",
...
static VOID Ins(INS ins, VOID* v)
{
    ...
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "Ins instrumentation", IARG_CALL_ORDER, CALL_ORDER_FIRST,
                       IARG_END);
...
static VOID Rtn(RTN rtn, VOID* v)
{
    ...
    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "RTN instrumentation", IARG_CALL_ORDER, CALL_ORDER_FIRST + 3,
                   IARG_END);
    RTN_Close(rtn);
}
...
static VOID Image(IMG img, VOID* v)
{
    ...
    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "IMG instrumentation", IARG_CALL_ORDER, CALL_ORDER_FIRST + 2,
                   IARG_END);
    RTN_Close(rtn);
}
