...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "special_case_aoti1.out",
...
{
    outstream << "In watch_rtn, arg: " << arg << " version_reg_value " << version_reg_value << std::endl;
...
static VOID Trace(TRACE trace, VOID* v)
{
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        {
            case VERSION_BASE:
                INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "version base", IARG_END);
                break;
            case VERSION_1:
                INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "version 1", IARG_END);
                break;
            case VERSION_2:
                INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "version 2", IARG_END);
                break;
            default:
                assert(0);
                break;
...
static VOID Image(IMG img, VOID* v)
{
    ...
    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(select_version), IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_REG_VALUE,
                   version_reg, IARG_RETURN_REGS, version_reg, IARG_END);
