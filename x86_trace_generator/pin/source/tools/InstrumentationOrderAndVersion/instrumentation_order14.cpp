...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "instrumentation_order14.out",
...
static VOID Instruction(INS ins, VOID* v)
{
    ...
    if (INS_Address(ins) == RTN_Address(rtn))
    {
        ...
        RTN_Open(rtn);
        RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "Ins instrumentation1", IARG_END);
        RTN_Close(rtn);
    }
}
