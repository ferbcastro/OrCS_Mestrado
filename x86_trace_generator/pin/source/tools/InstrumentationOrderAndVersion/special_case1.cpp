...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "special_case1.out",
...
{
    outstream << "In watch_rtn, arg: " << arg << std::endl;
    return arg;
}
...
static VOID Trace(TRACE trace, VOID* v)
{
    ...
    if (TRACE_Address(trace) == RTN_Address(rtn))
    {
        ...
        if (version == VERSION_BASE)
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(select_version), IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_RETURN_REGS,
                           version_reg, IARG_END);
