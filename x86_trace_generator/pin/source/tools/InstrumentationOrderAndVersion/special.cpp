...
VOID Emit(const char* string) { outstream << string << std::endl; }
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    {
        TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "Enter red", IARG_END);
    ...
    {
        TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "Enter blue", IARG_END);
    ...
    if (TRACE_Address(trace) == commonEnter)
        TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, "Enter common", IARG_END);
    ...
    if (version != 0) TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(Emit), IARG_PTR, version, IARG_END);
}
