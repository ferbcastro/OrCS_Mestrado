...
#define YIELD WINDOWS::SwitchToThread
#else
#include <sched.h>
...
// Output file std::string for this tool's log file
std::ofstream Out;
...
static void Image(IMG img, VOID*)
{
    if (IMG_IsMainExecutable(img))
    {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(DoBreakpoint), IARG_THREAD_ID, IARG_CONTEXT, IARG_END);
        RTN_Close(rtn);
    }
}
