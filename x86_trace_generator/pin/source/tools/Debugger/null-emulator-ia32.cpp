 ...
 * emulation (e.g. the tool emulates REG_ECX by providing the value of REG_ECX).
...
//
DEBUGGER_REG_DESCRIPTION RegDescs[] = {{LEVEL_BASE::REG_EAX, 0, 32},
                                       {REG_NONE, EMULATED_REG_ECX, 32},
                                       {LEVEL_BASE::REG_EDX, 0, 32},
                                       {LEVEL_BASE::REG_EBX, 0, 32},
                                       {REG_NONE, EMULATED_REG_ESP, 32},
                                       {LEVEL_BASE::REG_EBP, 0, 32},
                                       {LEVEL_BASE::REG_ESI, 0, 32},
                                       {LEVEL_BASE::REG_EDI, 0, 32},
                                       {REG_INST_PTR, 0, 32},
                                       {REG_EFLAGS, 0, 32},
                                       {REG_SEG_CS, 0, 32},
                                       {REG_SEG_SS, 0, 32},
...
static VOID GetReg(unsigned toolRegId, THREADID tid, CONTEXT* ctxt, VOID* data, VOID*)
{
    ...
    switch (toolRegId)
    {
        case EMULATED_REG_ECX:
        {
            ...
            *val         = PIN_GetContextReg(ctxt, LEVEL_BASE::REG_ECX);
            break;
        }
        case EMULATED_REG_ESP:
        {
            ADDRINT* val = static_cast< ADDRINT* >(data);
            *val         = PIN_GetContextReg(ctxt, LEVEL_BASE::REG_ESP);
            break;
        }
...
static VOID SetReg(unsigned toolRegId, THREADID tid, CONTEXT* ctxt, const VOID* data, VOID*)
{
    ...
    switch (toolRegId)
    {
        case EMULATED_REG_ECX:
        {
            ...
            PIN_SetContextReg(ctxt, LEVEL_BASE::REG_ECX, *val);
            break;
        }
        case EMULATED_REG_ESP:
        {
            const ADDRINT* val = static_cast< const ADDRINT* >(data);
            PIN_SetContextReg(ctxt, LEVEL_BASE::REG_ESP, *val);
            break;
        }
        case EMULATED_REG_FPSW:
        {
