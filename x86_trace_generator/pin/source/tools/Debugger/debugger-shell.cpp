...
static const REG_INFO AllRegisters[] = {
    {LEVEL_BASE::REG_GAX, "rax"}, {LEVEL_BASE::REG_GBX, "rbx"}, {LEVEL_BASE::REG_GCX, "rcx"}, {LEVEL_BASE::REG_GDX, "rdx"},
    {LEVEL_BASE::REG_GSI, "rsi"}, {LEVEL_BASE::REG_GDI, "rdi"}, {LEVEL_BASE::REG_GBP, "rbp"}, {LEVEL_BASE::REG_RSP, "rsp"},
    {LEVEL_BASE::REG_R8, "r8"},   {LEVEL_BASE::REG_R9, "r9"},   {LEVEL_BASE::REG_R10, "r10"}, {LEVEL_BASE::REG_R11, "r11"},
    {LEVEL_BASE::REG_R12, "r12"}, {LEVEL_BASE::REG_R13, "r13"}, {LEVEL_BASE::REG_R14, "r14"}, {LEVEL_BASE::REG_R15, "r15"}};

#elif defined(TARGET_IA32)

static const REG_INFO AllRegisters[] = {{LEVEL_BASE::REG_GAX, "eax"}, {LEVEL_BASE::REG_GBX, "ebx"}, {LEVEL_BASE::REG_GCX, "ecx"},
                                        {LEVEL_BASE::REG_GDX, "edx"}, {LEVEL_BASE::REG_GSI, "esi"}, {LEVEL_BASE::REG_GDI, "edi"},
                                        {LEVEL_BASE::REG_GBP, "ebp"}, {LEVEL_BASE::REG_ESP, "esp"}};
#endif
...
class SHELL : public DEBUGGER_SHELL::ISHELL
{
  private:
    ...
    struct HELP
    {
        ...
        std::string _description; // Help std::string for the command.
    };
  ...
  private:
     ...
     */
    static BOOL DebugInterpreter(THREADID tid, CONTEXT* ctxt, const std::string& cmd, std::string* result, VOID* vme)
    {
    ...
    /*
     * Attempt to parse an unsigned integral number from a std::string.  The std::string's prefix
     * determines the radix: "0x" for std::hex, "0" for octal, otherwise decimal.
     *
     *  @param[in] val      The std::string to parse.
    ...
    REG ParseRegName(const std::string& name)
    {
        ...
        // Translate the std::string to lower case and remove the leading "$" or "%".
        //
    ...
    /*
     * @return  A single std::string showing all the active breakpoints.
     */
    ...
    /*
     * @return  A single std::string showing all the active tracepoints.
     */
     ...
     *
     * @return  A std::string to return to the debugger prompt.
     */
     ...
     *
     * @return  On success, the empty std::string.  On failure, an error message.
     */
