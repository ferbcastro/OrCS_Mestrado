...
tdata* get_tls(THREADID threadid)
{
    ...
    if (!data)
    {
        std::cerr << "specified key is invalid or the given thread is not yet registered in the pin thread database" << std::endl;
        PIN_ExitProcess(1);
    }
    return data;
}
...
VOID insCallback(INS ins, void* v) { INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(iCount), IARG_THREAD_ID, IARG_END); }
...
int main(int argc, char** argv)
{
    ...
    auto intTid = PIN_SpawnInternalThread(doPause, NULL, 0, NULL);
