...
{
    if (PIN_UndecorateSymbolName(RTN_Name(rtn), UNDECORATION_NAME_ONLY) == "Checkpoint")
    {
        RTN_Open(rtn);
        RTN_InsertCall(rtn, IPOINT_AFTER, MAKE_AFUNPTR(OnCheckpoint), IARG_CONTEXT, IARG_END);
...
static BOOL InterceptBreakpoint(THREADID tid, DEBUGGING_EVENT eventType, CONTEXT* ctxt, VOID*)
{
    ...
    std::string rtnName = RTN_Valid(rtn) ? PIN_UndecorateSymbolName(RTN_Name(rtn), UNDECORATION_NAME_ONLY) : "<N/A>";
