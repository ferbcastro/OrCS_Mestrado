...
//
DEBUGGER_REG_DESCRIPTION RegDescs[] = {{LEVEL_BASE::REG_RAX, 0, 64},
                                       {LEVEL_BASE::REG_RBX, 0, 64},
                                       {REG_NONE, EMULATED_REG_RCX, 64},
                                       {LEVEL_BASE::REG_RDX, 0, 64},
                                       {LEVEL_BASE::REG_RSI, 0, 64},
                                       {LEVEL_BASE::REG_RDI, 0, 64},
                                       {LEVEL_BASE::REG_RBP, 0, 64},
                                       {REG_NONE, EMULATED_REG_RSP, 64},
                                       {LEVEL_BASE::REG_R8, 0, 64},
                                       {LEVEL_BASE::REG_R9, 0, 64},
                                       {LEVEL_BASE::REG_R10, 0, 64},
                                       {LEVEL_BASE::REG_R11, 0, 64},
                                       {LEVEL_BASE::REG_R12, 0, 64},
                                       {LEVEL_BASE::REG_R13, 0, 64},
                                       {LEVEL_BASE::REG_R14, 0, 64},
                                       {LEVEL_BASE::REG_R15, 0, 64},
                                       {REG_INST_PTR, 0, 64},
                                       {REG_RFLAGS, 0, 32},
                                       {REG_SEG_CS, 0, 32},
                                       {REG_SEG_SS, 0, 32},
...
static VOID GetReg(unsigned toolRegId, THREADID tid, CONTEXT* ctxt, VOID* data, VOID*)
{
    ...
    switch (toolRegId)
    {
        case EMULATED_REG_RCX:
        {
            ...
            *val         = PIN_GetContextReg(ctxt, LEVEL_BASE::REG_RCX);
            break;
        }
        case EMULATED_REG_RSP:
        {
            ADDRINT* val = static_cast< ADDRINT* >(data);
            *val         = PIN_GetContextReg(ctxt, LEVEL_BASE::REG_RSP);
            break;
        }
...
static VOID SetReg(unsigned toolRegId, THREADID tid, CONTEXT* ctxt, const VOID* data, VOID*)
{
    ...
    switch (toolRegId)
    {
        case EMULATED_REG_RCX:
        {
            ...
            PIN_SetContextReg(ctxt, LEVEL_BASE::REG_RCX, *val);
            break;
        }
        case EMULATED_REG_RSP:
        {
            const ADDRINT* val = static_cast< const ADDRINT* >(data);
            PIN_SetContextReg(ctxt, LEVEL_BASE::REG_RSP, *val);
            break;
        }
        case EMULATED_REG_FPSW:
        {
