...
static BOOL BreakpointHandler(ADDRINT addr, UINT size, BOOL insert, VOID* data)
{
    ...
    //
    if (PIN_UndecorateSymbolName(RTN_FindNameByAddress(addr), UNDECORATION_NAME_ONLY) != "ToolControlled") return FALSE;
...
static VOID InstrumentImg(IMG img, VOID* data)
{
    ...
    for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
    {
        ...
        {
            if (PIN_UndecorateSymbolName(RTN_Name(rtn), UNDECORATION_NAME_ONLY) == "ToolControlled")
            {
            ...
            if (PIN_UndecorateSymbolName(RTN_Name(rtn), UNDECORATION_NAME_ONLY) == "ReActivateToolControlled")
            {
                RTN_Open(rtn);
                RTN_InsertCall(rtn, IPOINT_AFTER, MAKE_AFUNPTR(ReActivateControlledBreakpoint), IARG_END);
                RTN_Close(rtn);
            }
        }
    }
