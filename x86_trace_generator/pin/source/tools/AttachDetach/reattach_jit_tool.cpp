...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "reattach_tool.out", "specify file name");
...
VOID SESSION_CONTROL::DedicatedThread(VOID* arg)
{
    ...
    while (1)
    {
        ...
        {
            PIN_GetLock(&pinLock, PIN_GetTid());
            TraceFile << "Pin tool: failed to instrument application routine " << appExitRtnName << std::endl;
            TraceFile.close();
            PIN_ReleaseLock(&pinLock);
        ...
        {
            TraceFile << "Pin tool: PIN_Attach returned ATTACH_FAILED_DETACH" << std::endl;
            sleep(1);
        }
        reattachIteration++;
    }
