...
const std::string toFind = "The main thread of the application is a zombie thread. Pin can't attach to an application which its main "
...
bool isZombie(pid_t tid)
{
    ...
    std::string filename = "/proc/" + ss.str() + "/status";
 ...
 */
bool NotifyUserPinUnableToAttach(std::string fileName)
{
    std::string oneline;
    ...
    if (inFile.is_open())
    {
        ...
        {
            getline(inFile, oneline);
            if (oneline.find(toFind) != std::string::npos)
            {
                return true;
            }
        }
