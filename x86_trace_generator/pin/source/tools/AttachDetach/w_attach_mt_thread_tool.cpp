...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "callbacks_before_jit_or_probe_tool.out",
...
int PinAttached(unsigned int numOfThreads)
{
    ...
    TraceFile << "Replacement called." << std::endl;
    return 1;
}
...
{
    if ((IMG_Name(img).find("kernel.appcore.dll") != std::string::npos) || (IMG_Name(img).find("msvcrt.dll") != std::string::npos) ||
        (IMG_Name(img).find("RPCRT4.dll") != std::string::npos) || (IMG_Name(img).find("sechost.dll") != std::string::npos) ||
        (IMG_Name(img).find("SspiCli.dll") != std::string::npos))
