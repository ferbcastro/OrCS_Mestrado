...
{
    std::cout << std::endl << IMG_Name(img);
    ...
    for (SYM sym = IMG_RegsymHead(img); SYM_Valid(sym); sym = SYM_Next(sym))
    {
        ...
        if (undFuncName == sFuncName)
        {
            ...
            if (RTN_Valid(rtn))
            {
                // Even though this is not an error - print to std::cerr (in order to see it on the screen)
                std::cerr << "Replacing DoLoop() in " << IMG_Name(img) << std::endl;
...
int main(INT32 argc, CHAR** argv)
{
    ...
    std::cerr << "Application is starting" << std::endl << std::flush;
