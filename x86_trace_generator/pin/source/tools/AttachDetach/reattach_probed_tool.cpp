...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "reattach_probed_tool.out", "specify file name");
...
void BeforeFork(UINT32 childPid, void* data) { TraceFile << "TOOL: Before fork." << PIN_GetPid() << std::endl; }

void AfterForkInParent(UINT32 childPid, void* data) { TraceFile << "TOOL: After fork in parent." << PIN_GetPid() << std::endl; }

void AfterForkInChild(UINT32 childPid, void* data) { TraceFile << "TOOL: After fork in child." << PIN_GetPid() << std::endl; }
...
VOID DETACH_SESSION::DetachCompleted(VOID* arg)
{
    ...
    //cerr << "Iteration " << detachIteration << " completed." << std::endl;
...
VOID DETACH_SESSION::ImageLoad(IMG img, void* arg)
{
    ...
    if (detachIteration != SessionControl()->CurrentIteration())
    {
        std::cerr << "Detach iteration error: Expected " << SessionControl()->CurrentIteration() << " Received " << detachIteration
             << " In ImageLoad" << std::endl;
        exit(-1);
    }
    PIN_GetLock(&pinLock, PIN_GetTid());
    TraceFile << "Load image " << IMG_Name(img) << std::endl;
    PIN_ReleaseLock(&pinLock);
}
...
VOID SESSION_CONTROL::ApplicationStart(VOID* arg)
{
    ...
    if (iteration != SessionControl()->CurrentIteration())
    {
        std::cerr << "Iteration error: Expected " << SessionControl()->CurrentIteration() << " Received " << iteration
             << " In ApplicationStart" << std::endl;
        exit(-1);
    }

    PIN_GetLock(&pinLock, PIN_GetTid());
    TraceFile << "Application start notification at session " << iteration << std::endl;
    PIN_ReleaseLock(&pinLock);
...
VOID REATTACH_SESSION::ImageLoad(IMG img, void* arg)
{
    ...
    {
        std::cerr << "Iteration error: Expected " << SessionControl()->CurrentIteration() << " Received " << reattachIteration
             << " In ImageLoad" << std::endl;
        exit(-1);
    }
}
...
VOID AttachMain(VOID* arg)
{
    ...
    PIN_GetLock(&pinLock, PIN_GetTid());
    TraceFile << "Re-attach session start, inside AttachMain; iteration " << reattachIteration << std::endl;
    PIN_ReleaseLock(&pinLock);
...
int main(int argc, CHAR* argv[])
{
    ...
    std::string traceFileName = KnobOutputFile.Value() + pidStr;
