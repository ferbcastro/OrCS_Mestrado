...
void WINAPI SERVICE_MANAGER::Main(DWORD argc, LPTSTR* argv)
{
    ...
    outFile << "Success! someone changed DoLoop on time" << std::endl;
    outFile.flush();
    StopService();
    return;
}
...
DWORD WINAPI SERVICE_MANAGER::ServiceTimerProc(VOID* p)
{
    ...
    delete pMilliSec;
    //never returns
    outFile << "Failure! DoLoop was not changed on time" << std::endl;
...
void SERVICE_MANAGER::StartServiceTimer(DWORD milliSec)
{
    ...
    {
        outFile << "Failed to CreateThread, " << GetLastError() << std::endl;
    }
    return;
}
...
void SERVICE_ADMIN::Main(int argc, wchar_t* argv[])
{
    ...
    else if (wcscmp(argv[2], L"-start") == 0)
    {
        ...
        char digitBuffer[64];
        std::cout << _itoa(pid, digitBuffer, 10);
    }
...
DWORD SERVICE_ADMIN::Start(DWORD argc, LPCWSTR* argv)
{
    ...
    {
        std::cerr << "Failed, Service haven't started after 60 seconds" << std::endl;
        CloseServiceHandle(manager);
        CloseServiceHandle(service);
        free(pServiceStatusProcess);
        return 0;
...
BOOL SERVICE_ADMIN::Stop()
{
    ...
    {
        std::cerr << "Failed to ControlService, " << GetLastError() << std::endl;
        CloseServiceHandle(manager);
        CloseServiceHandle(service);
        return false;
    }
...
BOOL SERVICE_ADMIN::Delete()
{
    ...
    {
        std::cerr << "Failed to DeleteService, " << GetLastError() << std::endl;
        CloseServiceHandle(manager);
        CloseServiceHandle(service);
        return false;
    }
...
void SERVICE_ADMIN::Usage()
{
    ...
    std::cerr << "To start the service:  -admin -start <service arguments>" << std::endl;
