...
VOID before_ShortFunction1()
{
        ...
        followChildCounter >= KnobFollowChildEvents.Value())
    {
        ...
        std::cerr << "success - exiting from application before shortFunction1 !" << std::endl << std::flush;
        doLoopPred = 0;
    }
...
VOID before_ShortFunction2()
{
    ...
    {
        //eventhough this is not an error - print to std::cerr (in order to see it on the screen)
        std::cerr << "success - exiting from application before shortFunction2 !" << std::endl << std::flush;
        doLoopPred = 0;
    }
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    std::cout << std::endl << IMG_Name(img) << std::endl << std::flush;
    ...
    for (SYM sym = IMG_RegsymHead(img); SYM_Valid(sym); sym = SYM_Next(sym))
    {
        ...
        if (undFuncName == sFuncName1)
        {
            ...
            {
                //eventhough this is not an error - print to std::cerr (in order to see it on the screen)
                std::cerr << "Inserting analysis function before ShortFunction1() in " << IMG_Name(img) << std::endl << std::flush;

                RTN_Open(rtn);

                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(before_ShortFunction1), IARG_END);

                RTN_Close(rtn);
            }
        }
        else if (undFuncName == sFuncName2)
        {
            RTN rtn = RTN_FindByAddress(IMG_LowAddress(img) + SYM_Value(sym));
            if (RTN_Valid(rtn))
            {
                //eventhough this is not an error - print to std::cerr (in order to see it on the screen)
                std::cerr << "Inserting analysis function before ShortFunction2() in " << IMG_Name(img) << std::endl << std::flush;

                RTN_Open(rtn);

                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(before_ShortFunction2), IARG_END);

                RTN_Close(rtn);
            }
        }
        else if (undFuncName == sFuncName3)
        {
            RTN rtn = RTN_FindByAddress(IMG_LowAddress(img) + SYM_Value(sym));
            if (RTN_Valid(rtn))
            {
                //eventhough this is not an error - print to std::cerr (in order to see it on the screen)
                std::cerr << "Replacing DoLoop() in " << IMG_Name(img) << std::endl << std::flush;
...
{
    std::cerr << "Application started" << std::endl << std::flush;
