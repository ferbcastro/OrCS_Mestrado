...
void QueryAuxv(const char* name, ADDRINT value)
{
    ...
    if (found)
    {
        TraceFile << name << " value: " << vdso << std::endl;
    }
    else
    {
        TraceFile << "Could not find auxv value " << name << std::endl;
    }
}
#endif
...
{
    static std::set< OS_THREAD_ID > tids;
    static std::set< ADDRINT > args;

    PIN_GetLock(&pinLock, PIN_GetTid());

    TraceFile << "MyThreadMain called with " << std::dec << (ADDRINT)arg << std::endl;
    std::pair< std::set< OS_THREAD_ID >::iterator, bool > res = tids.insert(PIN_GetTid());
    ASSERT(res.second, "Thread " + decstr(PIN_GetTid()) + " started twice");
    std::pair< std::set< ADDRINT >::iterator, bool > res2 = args.insert((ADDRINT)arg);
...
VOID ImageLoad(IMG img, void* v)
{
    ...
    if (RTN_Valid(rtn))
    {
        ...
        RTN_Open(rtn);
        RTN_InsertCall(rtn, IPOINT_AFTER, MAKE_AFUNPTR(AllThreadsNotifed), IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_REG_REFERENCE,
                       REG_GAX, IARG_END);
        RTN_Close(rtn);
    }
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    TraceFile << "Fini was called" << std::endl;
...
int main(int argc, CHAR* argv[])
{
    ...
    {
        TraceFile << "Initial thread counter: " << PIN_GetInitialThreadCount() << std::endl;
        TraceFile.close();
        PIN_ExitProcess(0);
    }
