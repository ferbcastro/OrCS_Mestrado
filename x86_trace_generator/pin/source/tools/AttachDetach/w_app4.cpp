...
std::string UnitTestName("sw_interrupt");
std::string FunctionTestName;

static void StartFunctionTest(const char* functionTestName)
{
#if PRINT
    if (FunctionTestName != "")
    {
        std::cerr << UnitTestName << "[" << FunctionTestName << "] Success" << std::endl;
    }
#endif
...
static void ExitUnitTest()
{
    ...
    std::cerr << UnitTestName << " : Completed successfully" << std::endl;
#endif
    exit(0);
}
...
{
    std::cerr << UnitTestName << "[" << FunctionTestName << "] Failure: " << msg << std::endl;
    exit(1);
}
...
/*!
 * Exception filter for the ExecuteSafe function: copy the corresponding exception info to out arguments.
 *
 * @param[in]  exceptPtr  pointers to exception info provided by OS when exception occurs
 * @param[out] pExCode  pointer to exception code if the function raises an exception
 * @param[out] pExAddr  pointer to exception address if the function raises an exception
 * @param[out] pIp      pointer to IP value in exception context if the function raises an exception
 *
 * @return the exception disposition
 */
static int SafeExceptionFilter(LPEXCEPTION_POINTERS exceptPtr, DWORD* pExCode, ULONG_PTR* pExAddr, ULONG_PTR* pIp)
{
    *pExCode = exceptPtr->ExceptionRecord->ExceptionCode;
    *pExAddr = (ULONG_PTR)exceptPtr->ExceptionRecord->ExceptionAddress;
    *pIp     = GetIp(exceptPtr->ContextRecord);
 ...
 * an exception.
 * @param[in]  fp       function to execute
 * @param[out] pExCode  pointer to exception code if the function raises an exception
 * @param[out] pExAddr  pointer to exception address if the function raises an exception
 * @param[out] pIp      pointer to IP value in exception context if the function raises an exception
 * @return TRUE, if the function raised an exception
 */
template< typename FUNC > bool ExecuteSafe(FUNC fp, DWORD* pExCode, ULONG_PTR* pExAddr, ULONG_PTR* pIp)
{
    __try
    {
        fp();
    ...
    __except (SafeExceptionFilter(GetExceptionInformation(), pExCode, pExAddr, pIp))
    {
#if PRINT
        std::cerr << "Exception handler: "
             << "Exception code " << std::hex << *pExCode
             << ". Exception address " << std::hex << *pExAddr
             << ". Context IP " << std::hex << *pIp << "." << std::endl;
#endif
        return true;
    }
}
 ...
 */
static void CheckExceptionCode(DWORD exceptCode, DWORD refCode)
{
    if (refCode != exceptCode)
    {
 ...
 * instruction.
 */
static void CheckExceptionAddr(ULONG_PTR exceptAddr, void* insPtr, size_t insSize, ULONG_PTR ip, DWORD exceptCode)
{
    ULONG_PTR insStart      = (ULONG_PTR)insPtr;
    ULONG_PTR insEnd        = insStart + insSize;

    if ((exceptAddr < insStart) || (exceptAddr > insEnd))
    {
        std::cerr << "Exception handler: Exception code " << std::hex << exceptCode << ". Context IP " << ip << "." << std::endl;
		std::cerr << "Exception address: actual " << std::hex << exceptAddr << ", expected " << insStart << "." << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    while (DoLoop())
    {
        ...
        exceptionCaught = ExecuteSafe(procAddr[0], &exceptCode, &exceptAddr, &ip);
        if (!exceptionCaught)
        {
            Abort("Unhandled exception");
        }
        CheckExceptionAddr(exceptAddr, insPtr[0], insSize[0], ip, exceptCode);

        // int 1
        StartFunctionTest("int 1");
        exceptionCaught = ExecuteSafe(procAddr[1], &exceptCode, &exceptAddr, &ip);
        if (!exceptionCaught)
        {
            Abort("Unhandled exception");
        }
        CheckExceptionAddr(exceptAddr, insPtr[1], insSize[1], ip, exceptCode);

        // int 2
        StartFunctionTest("int 2");
        exceptionCaught = ExecuteSafe(procAddr[2], &exceptCode, &exceptAddr, &ip);
        if (!exceptionCaught)
        {
            Abort("Unhandled exception");
        }
        CheckExceptionAddr(exceptAddr, insPtr[2], insSize[2], ip, exceptCode);

        // int 3
        StartFunctionTest("int 3");
        exceptionCaught = ExecuteSafe(procAddr[3], &exceptCode, &exceptAddr, &ip);
        if (!exceptionCaught)
        {
            Abort("Unhandled exception");
        }
        CheckExceptionAddr(exceptAddr, insPtr[3], insSize[3], ip, exceptCode);
        CheckExceptionCode(exceptCode, EXCEPTION_BREAKPOINT);

        // int 4
        StartFunctionTest("int 4");
        exceptionCaught = ExecuteSafe(procAddr[4], &exceptCode, &exceptAddr, &ip);
        if (!exceptionCaught)
        {
            Abort("Unhandled exception");
        }
        CheckExceptionAddr(exceptAddr, insPtr[4], insSize[4], ip, exceptCode);
        CheckExceptionCode(exceptCode, EXCEPTION_INT_OVERFLOW);

        // int 5
        StartFunctionTest("int 5");
        exceptionCaught = ExecuteSafe(procAddr[5], &exceptCode, &exceptAddr, &ip);
        if (!exceptionCaught)
        {
            Abort("Unhandled exception");
        }
        CheckExceptionAddr(exceptAddr, insPtr[5], insSize[5], ip, exceptCode);

        // int1
        StartFunctionTest("int1");
        exceptionCaught = ExecuteSafe(procAddr[6], &exceptCode, &exceptAddr, &ip);
        if (!exceptionCaught)
        {
            Abort("Unhandled exception");
        }
        CheckExceptionAddr(exceptAddr, insPtr[6], insSize[6], ip, exceptCode);
        CheckExceptionCode(exceptCode, EXCEPTION_SINGLE_STEP);

        // int3
        StartFunctionTest("int3");
        exceptionCaught = ExecuteSafe(procAddr[7], &exceptCode, &exceptAddr, &ip);
        if (!exceptionCaught)
        {
            Abort("Unhandled exception");
        }
        CheckExceptionAddr(exceptAddr, insPtr[7], insSize[7], ip, exceptCode);
        CheckExceptionCode(exceptCode, EXCEPTION_BREAKPOINT);

#if defined(TARGET_IA32)
        // intO
        StartFunctionTest("intO");
        exceptionCaught = ExecuteSafe(procAddr[8], &exceptCode, &exceptAddr, &ip);
        if (!exceptionCaught)
        {
            Abort("Unhandled exception");
        }
        CheckExceptionAddr(exceptAddr, insPtr[8], insSize[8], ip, exceptCode);
        CheckExceptionCode(exceptCode, EXCEPTION_INT_OVERFLOW);
#endif
    }
