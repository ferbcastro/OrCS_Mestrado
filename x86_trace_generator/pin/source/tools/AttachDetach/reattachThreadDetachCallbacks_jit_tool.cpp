...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "reattachImageLoadCallbackAttached_jit.out",
                                   "specify file name");
...
VOID DETACH_SESSION::DetachCompleted(VOID* v)
{
    ...
    {
        std::cerr << "At " << __FUNCTION__ << "Iteration error: Expected " << SessionControl()->CurrentIteration() << " Received "
             << iteration << " In DetachThreadStart" << std::endl;
        PIN_ExitProcess(1);
    }

    PIN_GetLock(&pinLock, PIN_GetTid());
    TraceFile << "Detach session " << iteration << " Detach completed; tid = " << PIN_GetTid() << std::endl;
...
VOID DETACH_SESSION::ImageLoad(IMG img, VOID* v)
{
    ...
    PIN_GetLock(&pinLock, PIN_GetTid());
    TraceFile << "Load image " << IMG_Name(img) << " in iteration " << iteration << std::endl;
    PIN_ReleaseLock(&pinLock);
    size_t found;
