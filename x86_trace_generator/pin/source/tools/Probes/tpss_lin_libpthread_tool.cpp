...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "tpss_lin_libpthread.txt", "specify tool log file name");
...
INT32 Usage()
{
    std::cerr << "This tool mimics the behavior of TPSS on Linux by adding probes to various libpthread functions." << std::endl;
...
void ImgLoad(IMG img, void* v)
{
    ...
    if ((IMG_Name(img).find("libpthread.so") != std::string::npos) || (IMG_Name(img).find("LIBPTHREAD.SO") != std::string::npos) ||
        (IMG_Name(img).find("LIBPTHREAD.so") != std::string::npos))
    {
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_create at " << RTN_Address(rtnpthread_create) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_join at " << RTN_Address(rtnpthread_join) << std::endl;
            OutFile.flush();
        ...
        if (RTN_Valid(rtnpthread_barrier_destroy) && RTN_IsSafeForProbedReplacement(rtnpthread_barrier_destroy))
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_barrier_destroy at "
                    << RTN_Address(rtnpthread_barrier_destroy) << std::endl;
            OutFile.flush();
        ...
        if (RTN_Valid(rtnpthread_mutex_timedlock) && RTN_IsSafeForProbedReplacement(rtnpthread_mutex_timedlock))
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_mutex_timedlock at "
                    << RTN_Address(rtnpthread_mutex_timedlock) << std::endl;
            OutFile.flush();
        ...
        if (RTN_Valid(rtnpthread_rwlock_timedrdlock) && RTN_IsSafeForProbedReplacement(rtnpthread_rwlock_timedrdlock))
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_rwlock_timedrdlock at "
                    << RTN_Address(rtnpthread_rwlock_timedrdlock) << std::endl;
            OutFile.flush();
        ...
        if (RTN_Valid(rtnpthread_rwlock_timedwrlock) && RTN_IsSafeForProbedReplacement(rtnpthread_rwlock_timedwrlock))
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_rwlock_timedwrlock at "
                    << RTN_Address(rtnpthread_rwlock_timedwrlock) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_exit at " << RTN_Address(rtnpthread_exit) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_cancel at " << RTN_Address(rtnpthread_cancel) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_spin_init at " << RTN_Address(rtnpthread_spin_init) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_spin_lock at " << RTN_Address(rtnpthread_spin_lock) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_cond_init at " << RTN_Address(rtnpthread_cond_init) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_cond_wait at " << RTN_Address(rtnpthread_cond_wait) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for sem_init at " << RTN_Address(rtnsem_init) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for sem_destroy at " << RTN_Address(rtnsem_destroy) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for sem_wait at " << RTN_Address(rtnsem_wait) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for sem_post at " << RTN_Address(rtnsem_post) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_sigmask at " << RTN_Address(rtnpthread_sigmask) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for sigsuspend at " << RTN_Address(rtnsigsuspend) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_self at " << RTN_Address(rtnpthread_self) << std::endl;
            OutFile.flush();
        ...
        if (RTN_Valid(rtnpthread_attr_setstacksize) && RTN_IsSafeForProbedReplacement(rtnpthread_attr_setstacksize))
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_attr_setstacksize at "
                    << RTN_Address(rtnpthread_attr_setstacksize) << std::endl;
            OutFile.flush();
        ...
        if (RTN_Valid(rtnpthread_attr_getstacksize) && RTN_IsSafeForProbedReplacement(rtnpthread_attr_getstacksize))
        {
            OutFile << CurrentTime() << "Inserting probe for pthread_attr_getstacksize at "
                    << RTN_Address(rtnpthread_attr_getstacksize) << std::endl;
            OutFile.flush();
        ...
        {
            OutFile << CurrentTime() << "Inserting probe for fcntl at " << RTN_Address(rtnfcntl) << std::endl;
            OutFile.flush();
