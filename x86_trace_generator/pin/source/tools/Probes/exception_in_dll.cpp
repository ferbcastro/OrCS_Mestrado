...
{
    *Out << "foo1 rep called" << std::endl;
    orig_foo1();
}

static void ExcInDll_rep(foo_t orig_func)
{
    *Out << "ExcInDll rep called" << std::endl;
    orig_func();
}

static VOID foo2_before() { *Out << "foo2 before called" << std::endl; }
static VOID foo2_after() { *Out << "foo2 after called" << std::endl; }
