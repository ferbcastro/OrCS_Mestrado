...
{
    std::cout << "SpinLock: calling pthread_spin_lock()" << std::endl;
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if (RTN_Valid(rtn))
    {
        if (RTN_IsSafeForProbedReplacementEx(rtn, PROBE_MODE_ALLOW_RELOCATION))
        {
            ...
            std::cout << "ImageLoad: Replaced pthread_spin_lock() in:" << IMG_Name(img) << std::endl;
        }
        else
        {
            std::cout << "ImageLocad: Can't replace pthread_spin_lock() in:" << IMG_Name(img) << std::endl;
            exit(-1);
        }
    }
}
