...
INT32 Usage()
{
    std::cerr << "This pin tool replaces svcraw_create()\n"
            "\n";
...
{
    std::cout << "SvcrawCreate: calling original svcraw_create() from libc" << std::endl;
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if (RTN_Valid(rtn))
    {
        if (RTN_IsSafeForProbedReplacementEx(rtn, PROBE_MODE_ALLOW_RELOCATION))
        {
            ...
            std::cout << "ImageLoad: Replaced svcraw_create() in: " << IMG_Name(img) << std::endl;
        }
        else
        {
            std::cout << "ImageLoad: Pin can't replace svcraw_create() in: " << IMG_Name(img) << std::endl;
