...
void ValidateProbeSize(int nbytes)
{
    if (rtn_bytes)
    {
        ...
        {
            std::cerr << "Invalid probe size. Probe seems to be bigger than the expected " << nbytes << " bytes." << std::endl;
            PIN_ExitProcess(1);
        }
    }
}
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    {
        SaveRtnBytes(rtn);
        ASSERTX(RTN_InsertCallProbed(rtn, IPOINT_BEFORE, AFUNPTR(foo_before), IARG_END));
    }
}
