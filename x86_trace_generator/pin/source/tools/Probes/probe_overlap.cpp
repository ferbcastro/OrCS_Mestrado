...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if (RTN_Valid(rtn))
    {
        if (!KnobCreateOverlap)
        {
            ...
            {
                std::cerr << "Failed to probe " << RTN_Name(rtn) << std::endl;
                PIN_ExitProcess(1);
            }
        }
        else
        {
            ...
            if (!INS_Valid(ins))
            {
                std::cerr << "Failed to get first instruction of " << RTN_Name(rtn) << std::endl;
                PIN_ExitProcess(1);
            }
            ...
            if (!INS_Valid(ins))
            {
                std::cerr << "Failed to get next instruction after first of " << RTN_Name(rtn) << std::endl;
                PIN_ExitProcess(1);
            }
            ...
            {
                std::cerr << "RTN_CreateAt failed. " << std::endl;
                PIN_ExitProcess(1);
            }
            ...
            if (RTN_InsertCallProbed(rtn, IPOINT_BEFORE, AFUNPTR(before), IARG_END))
            {
                std::cerr << RTN_Name(rtn)
                     << "() is expected to be unsafe for probing after inserting overlapping probe with RTN_CreateAt " << std::endl;
                PIN_ExitProcess(1);
            }
        }
    }
