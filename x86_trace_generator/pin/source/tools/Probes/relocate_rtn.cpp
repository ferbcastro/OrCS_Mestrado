...
INT32 Usage()
{
    std::cerr << "This pin tool replaces relocatable_1() and relocatable_2()\n"
            "\n";
...
void R2()
{
    std::cout << "R2: calling original relocatable_2()" << std::endl;
...
{
    std::cout << "R3: calling original relocatable_3()" << std::endl;
...
AFUNPTR ReplaceRelocatable(IMG img, const char* fname, AFUNPTR replacementFunc)
{
    ...
    std::cout << "ImageLoad: Replaced " << fname << " in:" << IMG_Name(img) << std::endl;
...
VOID CheckNonRelocatable(const char* fname, IMG img)
{
    ...
    std::cout << "ImageLoad: Pin behavior is right: reject to replace " << fname << " routine" << std::endl;
}
