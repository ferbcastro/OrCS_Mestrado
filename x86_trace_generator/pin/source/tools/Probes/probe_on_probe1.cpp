...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "probe_on_probe1.outfile", "specify file name");
...
void Notify1()
{
    if (origFptrNotify1)
    {
        TraceFile << "Notify 1 before" << std::endl;
        (*origFptrNotify1)();
        TraceFile << "Notify 1 after." << std::endl;
    }
}
...
void Notify2()
{
    if (origFptrNotify2)
    {
        TraceFile << "Notify 2 before" << std::endl;
        (*origFptrNotify2)();
        TraceFile << "Notify 2 after." << std::endl;
    }
}

void Notify1Sig(VOIDFUNC origFunc)
{
    TraceFile << "NotifySig 1 before" << std::endl;
    (*origFunc)();
    TraceFile << "NotifySig 1 after." << std::endl;
}

void Notify2Sig(VOIDFUNC origFunc)
{
    TraceFile << "NotifySig 2 before" << std::endl;
    (*origFunc)();
    TraceFile << "NotifySig 2 after." << std::endl;
}
...
BOOL ReplaceProbed(IMG img, std::string rtnName)
{
    RTN rtn;
    ...
    TraceFile << "Inserted probe for " << rtnName << std::endl;
    return TRUE;
}

BOOL ReplaceSignatureProbed(IMG img, std::string rtnName)
{
    RTN rtn;
    if (!FindAndCheckRtn(img, rtnName, rtn)) return FALSE;
