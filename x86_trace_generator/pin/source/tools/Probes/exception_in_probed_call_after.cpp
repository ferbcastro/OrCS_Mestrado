...
{
    *Out << __FUNCTION__ << " called" << std::endl << std::flush;
    *Out << "In pintool, result=" << std::dec << (a0 + a1 + a2 + a3 + a4 + a5 + a6) << std::endl << std::flush;
}
        ...
        for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
        {
            for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
            {
                ...
                while (INS_Valid(ins))
                {
                    ...
                    {
                        ins = INS_Next(ins);
                        if (INS_Valid(ins)) *Out << "Return from call to " << std::hex << INS_Address(ins) << std::endl;
                    }
                    else
                    {
                        ins = INS_Next(ins);
