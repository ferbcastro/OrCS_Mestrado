...
{
    *Out << __FUNCTION__ << " called" << std::endl;
...
static VOID ImageLoad(IMG img, VOID* data)
{
    if (IMG_IsMainExecutable(img))
    {
        ...
        *Out << rtnName << " found in address " << std::hex << RTN_Address(rtn) << std::endl;
        ...
        for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins))
        {
            if (INS_IsDirectBranch(ins))
            {
                ...
                *Out << "Move probe point to address " << std::hex << firstJmpAddress << std::endl;
                break;
            }
        }
        RTN_Close(rtn);
        ...
        *Out << rtnName << " probed (relocation) in address " << std::hex << firstJmpAddress << std::endl;
    }
}
