...
static const std::string ExecVELib = "libc.so";
...
//Pin command line
KNOB< std::string > KnobPin(KNOB_MODE_WRITEONCE, "pintool", "pin", "", "pin full path");

//Parent configuration - Application name
KNOB< std::string > KnobApplication(KNOB_MODE_WRITEONCE, "pintool", "app", "", "application name");

KNOB< BOOL > KnobToolProbeMode(KNOB_MODE_WRITEONCE, "pintool", "probe", "0", "invoke tool in probe mode");

KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "unix_parent_tool.out", "specify output file name");
...
BOOL FollowChild(CHILD_PROCESS cProcess, VOID* userData)
{
    ...
    if (KnobApplication.Value() == childApp)
    {
        ...
        if (!KnobPin.Value().empty())
        {
            ...
            OutFile << "Process to execute: " << newPinCmd.String() << std::endl;
        }
        else
        {
            OutFile << "Pin command line remains unchanged" << std::endl;
            OutFile << "Application to execute: " << appCmd.String() << std::endl;
        }
        return TRUE;
    }
    OutFile << "knob val " << KnobApplication.Value() << "app " << childApp << std::endl;
    OutFile << "Do not run Pin under the child process" << std::endl;
    return FALSE;
}
...
{
    OutFile << "In unix_parent_tool PinTool" << std::endl;
    OutFile.close();
}
...
{
    OutFile << "myexecve called " << std::endl;
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if ((IMG_Name(img).find(ExecVELib) != std::string::npos))
    { // check that tool can also probe execve successfully
        ...
        if (RTN_Valid(rtnexecve))
        {
            ...
            {
                OutFile << "Inserting probe for execve at " << std::hex << RTN_Address(rtnexecve) << std::endl;
                AFUNPTR fptr = (RTN_ReplaceProbed(rtnexecve, AFUNPTR(myexecve)));
                fptrexecve   = (int (*)(__const char*, char* __const*, char* __const*))fptr;
            }
            else
            {
                OutFile << "Unsafe to probe execve" << std::endl;
            }
        }
    }
}
