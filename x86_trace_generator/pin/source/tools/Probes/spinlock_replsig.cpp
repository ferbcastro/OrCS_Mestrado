...
INT32 Usage()
{
    std::cerr << "This pin tool replaces pthread_spin_lock()\n"
            "\n";
...
{
    std::cout << "SpinLock: calling pthread_spin_lock() at address " << std::hex << (ADDRINT)orgFuncptr << std::dec << std::endl;

    int locked = orgFuncptr(lock);

    std::cout << "Spinlock: returned from pthread_spin_lock()." << std::endl;
...
#endif
            std::cout << "ImageLoad: Replaced pthread_spin_lock() at " << std::hex << RTN_Address(rtn) << " in:" << IMG_Name(img) << std::endl;
        }
        else
        {
            std::cout << "ImageLoad: Can't replace pthread_spin_lock() in:" << IMG_Name(img) << std::endl;
            exit(-1);
        }
    }
}
