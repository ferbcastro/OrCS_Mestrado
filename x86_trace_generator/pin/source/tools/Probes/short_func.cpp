...
INT32 Usage()
{
    std::cerr << "This pin tool instruments a very short function (2 bytes long).\n" << KNOB_BASE::StringKnobSummary() << std::endl;
    return -1;
}
...
STATIC VOID StoreRtnBytesBeforeInstrumentation(RTN rtn)
{
    ...
    *Out << "Before instrumentation " << std::hex << rtnAddress << " : " << rtnBytesBefore.toString() << std::endl;
}
...
STATIC VOID ValidateRtnBytesAfterInstrumentation()
{
    ...
    *Out << "After instrumentation  " << std::hex << rtnAddress << " : " << rtnBytesAfter.toString() << std::endl;

    // The first byte is expected to change to direct jmp opcode
    {
        if (rtnBytesBefore.GetBytes(0, 0) == rtnBytesAfter.GetBytes(0, 0))
        {
            *Out << "ERROR: Expected the routine first byte to change after instrumentation" << std::endl;
            PIN_ExitProcess(1);
        }
    }
    ...
    // The remaining 4 bytes are expected not to change, they are a random offset
    {
        ...
        {
            *Out << "ERROR: Unexpected change in the bytes 1-4 of the routine" << std::endl;
            PIN_ExitProcess(1);
        }
    }
}
...
{
    *Out << "Inserting before " << RTN_Name(rtn) << std::endl;

    BOOL isSafe = RTN_IsSafeForProbedInsertion(rtn);
    BOOL ok     = RTN_InsertCallProbed(rtn, IPOINT_BEFORE, AFUNPTR(BeforeFunc), IARG_END);
    if (!ok)
    {
        *Out << "Failed to instrument function " << RTN_Name(rtn) << std::endl;
        PIN_ExitProcess(0); // return with 0 - not a failure
    }
    *Out << "Successfully instrumented function " << RTN_Name(rtn) << std::endl;
    if (!isSafe)
    {
        *Out << "ERROR: RTN_IsSafeForProbedInsertion returned FALSE for " << RTN_Name(rtn) << std::endl;
        PIN_ExitProcess(1);
    }
}
...
{
    *Out << "Replacing" << (replaceSig ? " (signature) " : " ") << RTN_Name(rtn) << std::endl;
    ...
    *Out << "Successfully instrumented function " << RTN_Name(rtn) << std::endl;
    if (!isSafe)
    {
        *Out << "RTN_IsSafeForProbedReplacement returned FALSE for " << RTN_Name(rtn) << std::endl;
        PIN_ExitProcess(1);
    }
}
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if (!RTN_Valid(rtn))
    {
        *Out << "Failed to find function " << funcName << std::endl;
        PIN_ExitProcess(1);
    }
