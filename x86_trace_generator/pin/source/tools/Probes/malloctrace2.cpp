...
typedef decltype(malloc)* MallocType;
typedef decltype(free)* FreeType;
typedef decltype(dlopen)* DlopenType;
typedef decltype(dlsym)* DlsymType;
...
{
    // starting from GLIBC-2.34, libdl.so has been integrated into libc.so.
    // So we're looking dlopen in both libraries for full cover
    if (strstr(IMG_Name(img).c_str(), "libdl.so") || strstr(IMG_Name(img).c_str(), "libc.so"))
    {
...
#error symbol versions unknown for this target
    ...
    if (strstr(IMG_Name(img).c_str(), "libdyld.dylib"))
    {
        ...
        {
            std::cerr << "Error: dlsym and dlopen not found" << std::endl;
    ...
    if (strstr(IMG_Name(img).c_str(), MALLOC_LIB))
    {
        ...
        {
            std::cout << "Cannot replace free in " << IMG_Name(img) << std::endl;
            exit(1);
        }
    ...
    */
    if (IMG_IsMainExecutable(img))
    {
        ...
        ASSERTX(RTN_InsertCallProbed(mainRtn, IPOINT_BEFORE, AFUNPTR(MainRtnCallback), IARG_END));
    }
}
