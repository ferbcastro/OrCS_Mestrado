...
INT32 Usage()
{
    std::cerr << "This pin tool replaces exit() function in libc\n"
            "\n";
...
void MyExit(int arg)
{
    std::cout << "MyExit: calling original exit() from libc" << std::endl;
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if (RTN_Valid(rtn))
    {
        ...
        if (RTN_IsSafeForProbedReplacementEx(rtn, PROBE_MODE_ALLOW_RELOCATION))
        {
            ...
            std::cout << "ImageLoad: Replaced exit() in: " << IMG_Name(img) << std::endl;
        }
        else
        {
            std::cout << "ImageLoad: Pin can't replace exit() in: " << IMG_Name(img) << std::endl;
            exit(-1);
        }
    }
}
