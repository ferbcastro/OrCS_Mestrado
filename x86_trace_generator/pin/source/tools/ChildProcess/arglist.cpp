...
void ARGUMENTS_LIST::Add(const std::string& argsStr)
{
    size_t i = 0;
    while (i < argsStr.size())
    {
        ...
        while ((argsStr[j] != ' ') && (j < argsStr.size()))
            j++;
        std::string arg = argsStr.substr(i, j - i);
...
std::string ARGUMENTS_LIST::String() const
{
    std::string fullStr;
    std::list< std::string >::const_iterator it = m_argvStrList.begin();
