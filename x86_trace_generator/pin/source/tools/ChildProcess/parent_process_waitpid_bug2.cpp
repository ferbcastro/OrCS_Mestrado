...
void* WaitPidThread(void* arg)
{
    ...
    if (ret < 0)
    {
        std::cout << "waitpid() failed with errno " << errno << std::endl;
        abort();
    }
    else if (pid != ret)
    {
        std::cout << "waitpid() waited for the wrong process" << std::endl;
        abort();
    }
    else if (status != 0)
    {
...
void* WaitIdThread(void* arg)
{
    ...
    if (ret < 0)
    {
        std::cout << "waitid() failed with errno " << errno << std::endl;
        abort();
    }
    else if (siginfo.si_pid != pid)
    {
        std::cout << "waitid() waited for the wrong process" << std::endl;
        abort();
    }
    else if (siginfo.si_status != 0)
    {
        std::cout << "Parent report: Child process failed. Status of the child process is " << WEXITSTATUS(siginfo.si_status) << std::endl;
        abort();
    }
