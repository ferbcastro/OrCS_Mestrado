...
bool WaitAndVerify(HANDLE process)
{
    ...
    {
        std::cout << "WaitForSingleObject failed" << std::endl << std::flush;
        return FALSE;
    }
    DWORD processExitCode;
    if (GetExitCodeProcess(process, &processExitCode) == FALSE)
    {
        std::cout << "GetExitCodeProcess Failed" << std::endl << std::flush;
        return FALSE;
    }
    if (processExitCode != 0)
    {
        std::cout << "Got unexpected exit code" << std::endl << std::flush;
        return FALSE;
    }
    return TRUE;
}
...
int main(int argc, char* argv[])
{
...

    {
        ...
        std::cout << "First Child Process was created successfully!" << std::endl << std::flush;
    }
