...
union thread_data_t
{
    struct
    {
        ...
        INT32  _depth; // Each syscall may be nterrupted by an APC. The APC in turn can call other syscalls
                       // We will measure the time just for the  syscall with depth 0.
    } _data;
...
{
    thread_data_t* tdata           = static_cast< thread_data_t* >(PIN_GetThreadData(tls_key, threadIndex));
    ASSERTX(0 < tdata->_data._depth);
    if (1 == tdata->_data._depth)
    {
...
VOID ThreadFini(THREADID threadIndex, const CONTEXT* ctxt, INT32 code, VOID* v)
{
    ...
    *OutFile << "Thread " << decstr(threadIndex) << " spent " << tdata->_data._totalAccumulated
             << " processor cycles in system calls" << std::endl;
    delete tdata;
}
