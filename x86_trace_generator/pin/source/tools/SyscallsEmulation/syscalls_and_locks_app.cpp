...
void* thread_func1(void* arg)
{
    ...
    else if (syscall_type == 3 || syscall_type == 4)
    {
        ...
        {
#if PINT_6630_FIXED
            // Currently Pin doesn't properly support clone with fork sematics in Probe mode.
            // Calling clone with fork semantics will create a new child but Pind will not be
            // notified so a server will not be created for the new process. Depending on
            // the specific use case, this may or may not be a problem. But since we now
            // instrument _exit in Probe Mode to notify pind about exit_group, it is likely
            // that when the child exits, there will be an assert because there is not pind to
            // answer the exit group notification (This may not always happen because we don't
            // always await a response for exit_group notification, but will happen when we do).
            // PINT-6630 is a feature request to properly support clone with fork semantics in Probe mode.
