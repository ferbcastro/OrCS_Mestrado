...
// define the clone3 system call even if it not defined by the system header file
#ifndef __NR_clone3
...
#define CLONE3_SUPPORTED 0
#else
#define CLONE3_SUPPORTED 1
#endif
...
static int call_clone3(uint64_t flags)
{
    ...
    {
        ret = -errno;
        printf("*** failed to create new process - error %s ***\n", strerror(errno));
        return ret;
    }
...
static void test_clone3_success(uint64_t flags, int expected)
{
    ...
    printf("\nTest Case #%d : calling clone3() from parent with flags = 0x%08x... ", ++testCounter, flags);
...
int main(int argc, char* argv[])
{
    ...
    if (!CLONE3_SUPPORTED)
    {
        int retval = call_clone3(0);
        if ((retval < 0) && (retval == -ENOSYS))
        {
            printf("clone3() syscall returned with error ENOSYS (0x26) - syscall is not supported.\n");
        }
