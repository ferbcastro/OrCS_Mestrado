...
KNOB< std::string > KnobThreadsStartsFile(KNOB_MODE_WRITEONCE, "pintool", "startsfile", "threadStarts.out",
                                          "specify file name for thread start callbacks output");
KNOB< std::string > KnobThreadsFinisFile(KNOB_MODE_WRITEONCE, "pintool", "finisfile", "threadFinis.out",
                                         "specify file name for thread fini callbacks output");
KNOB< std::string > KnobOutFile(KNOB_MODE_WRITEONCE, "pintool", "o", "tool.out", "specify file name for general tool output");

// Output file streams
std::ofstream startsOut;
std::ofstream finisOut;
FILE* outfile;
...
static VOID AppThreadStart(THREADID threadIndex)
{
    ...
    startsOut << *tidData << std::endl << std::flush;
...
static VOID ThreadFini(THREADID threadIndex, CONTEXT const* c, INT32 code, VOID* v)
{
    ...
    if (appThreads.find(threadIndex) != appThreads.end())
    {
        ...
        finisOut << *tidData << std::endl << std::flush;
