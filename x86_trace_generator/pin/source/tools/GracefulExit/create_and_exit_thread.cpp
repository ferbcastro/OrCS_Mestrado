...
{
    *outFile << "TOOL: Thread " << tid << " executing DoTest." << std::endl;
    ...
    if (maxNumTests > testCounter)
    {
        ...
        if (!ret)
        {
            *outFile << "TOOL ERROR: Failed to spawn new application thread." << std::endl;
            PIN_ExitProcess(RETVAL_FAILURE_TOOL_FAILED_TO_SPAWN);
        }
    }
    PIN_ExitThread(RETVAL_SUCCESS);

    *outFile << "TOOL ERROR: Should not reach this point in DoTest." << std::endl;
...
{
    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(DoTest), IARG_THREAD_ID, IARG_CONTEXT, IARG_END);
}
...
VOID ThreadStart(const THREADID tid, CONTEXT* ctxt, INT32 flags, VOID* v)
{
    ...
    if (finiCalled)
    {
        std::cerr << "TOOL ERROR: ThreadStart called after application Fini." << std::endl;
...
VOID ThreadFini(const THREADID tid, const CONTEXT* ctxt, INT32 code, VOID* v)
{
    ASSERTX(tid == currTid);
    ASSERTX(!PIN_SemaphoreIsSet(&noActiveThreads));
    *outFile << "TOOL: ThreadFini for thread " << tid << " with code " << code << "." << std::endl;
    --numThreads;
    currTid = nextTid;
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    *outFile << "TOOL: Fini called after " << testCounter << " trials." << std::endl;
    finiCalled = true;
    if (&std::cerr != outFile)
    {
        delete outFile;
...
int main(int argc, char* argv[])
{
    ...
    outFile = KnobOutputFile.Value().empty() ? &std::cerr : new std::ofstream(KnobOutputFile.Value().c_str());
