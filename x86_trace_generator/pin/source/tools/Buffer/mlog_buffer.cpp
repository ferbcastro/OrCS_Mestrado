 ...
 */
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "mlog_buffer.out", "output file");
...
VOID MLOG::DumpBufferToFile(struct MEMREF* reference, UINT64 numElements, THREADID tid)
{
    ...
    for (UINT64 i = 0; i < numElements; i++, reference++)
    {
        if (reference->ea != 0)
        {
            _ofile << reference->pc << (reference->read ? "  R " : "  W ") << reference->ea << " : " << reference->tsc << std::endl;
        }
    }
}
...
VOID ThreadStart(THREADID tid, CONTEXT* ctxt, INT32 flags, VOID* v)
{
    ...
    std::cerr << "Thread " << tid << " started" << std::endl << std::flush;
}

VOID ThreadFini(THREADID tid, const CONTEXT* ctxt, INT32 code, VOID* v)
{
    std::cerr << "Thread " << tid << " finished" << std::endl << std::flush;
