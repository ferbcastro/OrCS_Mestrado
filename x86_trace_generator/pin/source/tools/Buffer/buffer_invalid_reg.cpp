 ...
 * Register to test
 */
KNOB< std::string > KnobRegister(KNOB_MODE_WRITEONCE, "pintool", "r", "xmm0", "register to test its retrieve");
...
INT32 Usage()
{
    std::cerr << "This tool contains errors.  Do not use!" << std::endl;
...
REG FindRegToTest()
{
    std::string reg_string = KnobRegister;
    ...
    std::cerr << "Invalid register specified at command line: " << reg_string << std::endl;
    exit(2);
}
...
int main(int argc, char* argv[])
{
    ...
    {
        std::cerr << "Error allocating initial buffer" << std::endl;
        return 1;
    }
