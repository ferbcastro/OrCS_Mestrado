...
KNOB< std::string > KnobOutput2File(KNOB_MODE_WRITEONCE, "pintool", "o2", "bblcnt.out", "specify file name for bblcnt profile");
...
static VOID DumpStats(std::ofstream& out, STATS& stats, BOOL predicated_true, BOOL print_zeros, const std::string& title)
{
    ...
    for (UINT32 i = 0; i < MAX_INDEX; i++)
    {
        ...
        out << std::setw(4) << i << " " << ljstr(IndexToOpcodeString(i), 20) << " " << std::setw(16) << stats.unpredicated[i] << " "
...
VOID PrintBblCount()
{
    ...
    for (std::vector< const BBLSTATS* >::iterator bi = statsList.begin(); bi != statsList.end(); bi++)
    {
        ...
        out << "0x" << std::hex << b->_addr << " " << std::dec << b->_counter << " " << b->_numins << " " << b->_size << std::endl;
    }
