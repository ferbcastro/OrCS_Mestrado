...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "inscount_per_image.out",
...
typedef std::map< ADDRINT, LoadedModuleInfo, std::less< ADDRINT > > LoadedModuleInfoMap;
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    const std::pair< const ADDRINT, LoadedModuleInfo >& newpair = std::make_pair(IMG_LowAddress(img), loadedModuleInfo);
