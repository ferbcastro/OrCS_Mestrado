...
{
    icount++;
    if ((icount % 1000) == 1) out << "ip:" << ip << " count:" << icount << " SP:" << reg << std::endl;
}
...
{
    std::cerr << std::hex << "ip " << ip << " al " << al << " ah " << ah << " ax " << ax << " eax " << eax << " rax " << rax << std::endl;
    exit(1);
}
...
// Make it inlineable
VOID CaptureRef(ADDRINT* ref, ADDRINT const* constRef)
{
    capturedVal      = *ref;
...
VOID CheckRef(char const* name, ADDRINT val, ADDRINT* ref, ADDRINT const* constRef)
{
    ...
    {
        std::cerr << std::hex << name << " val: " << val << " ref: " << *ref << " constRef: " << *constRef << std::endl;
        exit(1);
    }
}
...
{
    // This is inlineable
    TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(CaptureRef), IARG_REG_REFERENCE, reg, IARG_REG_CONST_REFERENCE, reg,
                     IARG_END);
    // This is NOT inlineable
    TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(CheckRef), IARG_PTR, name, IARG_REG_VALUE, reg, IARG_REG_REFERENCE, reg,
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(CheckRegs), IARG_INST_PTR, IARG_REG_VALUE, REG_AL, IARG_REG_VALUE, REG_AH,
                     IARG_REG_VALUE, REG_AX, IARG_REG_VALUE, LEVEL_BASE::REG_EAX, IARG_REG_VALUE,
#if defined(TARGET_IA32E)
                     LEVEL_BASE::REG_RAX,
#else
                     LEVEL_BASE::REG_EAX,
    ...
    if (immcount > 0)
    {
        for (UINT32 op = 0; op < INS_OperandCount(ins); op++)
        {
            ...
            {
                std::cerr << "Immediate: " << std::hex << INS_OperandImmediate(ins, op) << " ins " << INS_Disassemble(ins) << std::endl;
                immcount--;
            }
        }
    }
