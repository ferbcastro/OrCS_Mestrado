...
{
    TraceFile << "mmap(" << arg1 << "," << arg2 << "," << arg3 << "," << arg4 << "," << arg5 << "," << arg6 << ")" << std::endl;
    TraceFile << "  called from " << ra << std::endl;
}

VOID CallArgs(ADDRINT arg1) { TraceFile << "Call(" << arg1 << ")" << std::endl; }

VOID FoobarArgs(ADDRINT arg1, ADDRINT arg2)
{
    TraceFile << "Foobar(" << arg1 << "," << arg2 << ")" << std::endl;
...
{
    TraceFile << "Baz(" << *arg1 << "," << *arg2 << "," << *arg3 << ")" << std::endl;
    *arg1 = 4;
    *arg2 = 5;
    *arg3 = 6;
}
...
VOID Ins(INS ins, VOID* v)
{
    ...
    {
        TraceFile << "Instrument call to foobar" << std::endl;
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(FoobarArgs), IARG_FUNCARG_CALLSITE_VALUE, 0, IARG_FUNCARG_CALLSITE_VALUE,
                       1, IARG_END);
    }
    ...
    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(CallArgs), IARG_FUNCARG_CALLSITE_VALUE, 0, IARG_END);
}
...
VOID Image(IMG img, VOID* v)
{
    ...
    {
        RTN_Open(mmapRtn);
        RTN_InsertCall(mmapRtn, IPOINT_BEFORE, MAKE_AFUNPTR(MmapArgs), IARG_RETURN_IP, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
    ...
    if (RTN_Valid(foobarRtn))
    {
        ...
        RTN_Open(foobarRtn);
        RTN_InsertCall(foobarRtn, IPOINT_BEFORE, MAKE_AFUNPTR(FoobarArgs), IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
    ...
    {
        RTN_Open(bazRtn);
        RTN_InsertCall(bazRtn, IPOINT_BEFORE, MAKE_AFUNPTR(BazArg), IARG_FUNCARG_ENTRYPOINT_REFERENCE, 0,
