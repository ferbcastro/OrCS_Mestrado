    ...
    for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
    {
        for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
        {
            ...
            for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins))
            {
                ...
                {
                    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(Mem), IARG_MEMORYREAD2_EA, IARG_MEMORYREAD_SIZE, IARG_END);
                }
                ...
                if (INS_IsMemoryRead(ins))
                {
                    ...
                    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(Mem), IARG_MEMORYREAD_EA, IARG_MEMORYREAD_SIZE, IARG_END);
                }
                ...
                if (INS_IsMemoryWrite(ins))
                {
                    ...
                    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(Mem), IARG_MEMORYWRITE_EA, IARG_MEMORYWRITE_SIZE, IARG_END);
                }
            }
