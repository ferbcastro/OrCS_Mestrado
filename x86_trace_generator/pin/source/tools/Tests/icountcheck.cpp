...
VOID docount_bbl_ins(void* pc, INT32 icount)
{
    ...
    {
        std::cerr << "mismatch: count_ins " << count_ins << " != count_bbl_ins " << count_bbl_ins << " at pc " << pc << std::endl;
        exit(1);
    }
}
...
VOID Fini(INT32 code, VOID* v) { std::cerr << "Count (ins) " << count_ins << ",  (bbl_ins) " << count_bbl_ins << std::endl; }
