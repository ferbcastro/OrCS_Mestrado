...
VOID CountError()
{
    ...
    if (error > 100)
    {
        std::cerr << "Too many errors, giving up" << std::endl;
        exit(error);
    }
}
...
VOID CheckPc(VOID* iaddr)
{
    ...
    {
        std::cerr << "error: at PC " << pc << " computed target " << next_pc << " but executing at " << iaddr << std::endl;
        CountError();
    }
    ...
    if ((icount % 1000) == 0)
    {
        std::cerr << icount << " BBLs checked" << std::endl;
    }
}
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        if (INS_IsControlFlow(tail))
        {
            ...
            INS_InsertCall(tail, IPOINT_BEFORE, MAKE_AFUNPTR(RecordPc), IARG_INST_PTR, IARG_BRANCH_TARGET_ADDR, IARG_BRANCH_TAKEN,
                           IARG_END);
        }
...
{
    std::cerr << error << " errors (" << icount << " BBLs checked)" << std::endl;
