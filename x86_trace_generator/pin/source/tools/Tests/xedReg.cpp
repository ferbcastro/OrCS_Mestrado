...
VOID InstrumentOperand(INS ins, xed_decoded_inst_t const* const xedd, xed_inst_t const* const xedi, unsigned int operand_index)
{
    ...
    if (xed_operand_is_register(operand_name))
    {
        ...
        if (reg_class == XED_REG_CLASS_GPR)
        {
            ...
            if (pinreg == LEVEL_BASE::REG_EAX)
                INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)CountIfZero, IARG_REG_VALUE, LEVEL_BASE::REG_EAX, IARG_END);
        }
    }
}
...
VOID Fini(INT32 code, VOID* v) { std::cout << "eaxIsZeroCount = " << eaxIsZeroCount << std::endl; }
