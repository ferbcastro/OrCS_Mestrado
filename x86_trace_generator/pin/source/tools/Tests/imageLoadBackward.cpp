...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "imageloadbackward.out", "specify output file name");

VOID ImageLoad(IMG img, VOID* v)
{
    *out << "Tool loading " << IMG_Name(img) << " at " << IMG_LoadOffset(img) << std::endl;
    for (SEC sec = IMG_SecTail(img); SEC_Valid(sec); sec = SEC_Prev(sec))
    {
        *out << "  sec " << SEC_Name(sec) << " " << SEC_Address(sec) << ":" << SEC_Size(sec) << std::endl;
        for (RTN rtn = SEC_RtnTail(sec); RTN_Valid(rtn); rtn = RTN_Prev(rtn))
        {
            *out << "    rtn " << RTN_Name(rtn) << " " << RTN_Address(rtn) << ":" << RTN_Size(rtn) << std::endl;
...
VOID ImageUnload(IMG img, VOID* v) { *out << "Tool unloading " << IMG_Name(img) << " at " << IMG_LoadOffset(img) << std::endl; }
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    if (file != "")
    {
        *out << file << ":" << std::dec << line << ":" << column << " " << std::hex;
    }
