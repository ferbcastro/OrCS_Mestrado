...
VOID Image(IMG img, VOID *v)
{
    ...
    for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
    {
        for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
        {
            ...
            {
                RTN_Open(rtn);
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(HookAnalyze), IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
...
static void analyzeInsturction(INS ins, VOID* val)
{
    if (INS_Address(ins) == data.instructionAddress)
    {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(MoveImmediateToRegister), IARG_RETURN_REGS, reg, IARG_END);
