...
VOID Strcmp(const char* str1, const char* str2) { std::cout << "strcmp called" << std::endl; }

VOID IfuncStrcmp() { std::cout << "ifunc strcmp called" << std::endl; }
...
VOID Routine(RTN rtn)
{
    ...
    if (isStrcmp || isSbcmp)
    {
        ...
        {
            std::cout << "Found IFUNC strcmp" << std::endl;
            if (RunInProbeMode)
            {
            ...
            {
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(IfuncStrcmp), IARG_END);
            }
        }
        else
        {
            std::cout << "Found non IFUNC strcmp" << std::endl;
            if (RunInProbeMode)
            {
            ...
            {
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(Strcmp), IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 1, IARG_END);
            }
        }
    }
