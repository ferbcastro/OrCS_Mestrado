                     ...
                     VOID (*OpMV)(ADDRINT*, ADDRINT))
{
        ...
        && INS_IsMemoryWrite(ins))
    {
        if (INS_OperandIsReg(ins, 1) && REG_is_gr(INS_OperandReg(ins, 1)))
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(OpMV), IARG_MEMORYWRITE_EA, IARG_REG_VALUE, INS_OperandReg(ins, 1),
                           IARG_END);
            INS_Delete(ins);
        }
        else
    ...
    else if (INS_OperandIsReg(ins, 0))
    {
        ...
        if (INS_OperandIsReg(ins, 1))
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(OpRV), IARG_REG_VALUE, INS_OperandReg(ins, 0), IARG_REG_VALUE,
        ...
        {
            // Source register, dst register
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(OpRM), IARG_REG_VALUE, INS_OperandReg(ins, 0), IARG_MEMORYREAD_EA,
