...
static void print_hex_line(char* buf, int buf_size, const UINT8* array, const int length)
{
    int n = length;
    int i = 0;
    ...
    for (i = 0; i < n; i++)
    {
        // This assert checks the index that accessed in 'buf' and prevents 'stringop-overflow' warning
        // which warn on std::string manipulation that might overflow the destination buffer
        ASSERT(2 * i + 1 < buf_size, "buffer overflow\n");
...
static std::string disassemble(UINT64 start, UINT64 stop)
{
    UINT64 pc = start;
    xed_state_t dstate;
    ...
    /*while( pc < stop )*/ {
        ...
        std::iostream::fmtflags fmt = os.flags();
        ...
        if (okay)
        {
            ...
            print_hex_line(buffer, BUFFER_SIZE, reinterpret_cast< UINT8* >(pc), dec_len);
            os << " " << buffer;
            ...
            if (dis_okay)
                ...
                os << "Error disasassembling pc 0x" << std::hex << pc << std::dec << std::endl;
            pc += dec_len;
        }
        else
...
#ifdef TARGET_IA32E
    PIN_GetContextReg(ctxt, LEVEL_BASE::REG_R8);

    PIN_GetContextReg(ctxt, LEVEL_BASE::REG_R9);

    PIN_GetContextReg(ctxt, LEVEL_BASE::REG_R10);

    PIN_GetContextReg(ctxt, LEVEL_BASE::REG_R11);

    PIN_GetContextReg(ctxt, LEVEL_BASE::REG_R12);

    PIN_GetContextReg(ctxt, LEVEL_BASE::REG_R13);

    PIN_GetContextReg(ctxt, LEVEL_BASE::REG_R14);

    PIN_GetContextReg(ctxt, LEVEL_BASE::REG_R15);
...
BOOL CompareIntContext(CONTEXT* context1, CONTEXT* context2)
{
...
#ifdef TARGET_IA32E
    if (PIN_GetContextReg(context1, LEVEL_BASE::REG_R8) != PIN_GetContextReg(context2, LEVEL_BASE::REG_R8))
    {
        printf("REG_R8 ERROR\n");
        compareOk = FALSE;
    }
    if (PIN_GetContextReg(context1, LEVEL_BASE::REG_R9) != PIN_GetContextReg(context2, LEVEL_BASE::REG_R9))
    {
        printf("REG_R9 ERROR\n");
        compareOk = FALSE;
    }
    if (PIN_GetContextReg(context1, LEVEL_BASE::REG_R10) != PIN_GetContextReg(context2, LEVEL_BASE::REG_R10))
    {
        printf("REG_R10 ERROR\n");
        compareOk = FALSE;
    }
    if (PIN_GetContextReg(context1, LEVEL_BASE::REG_R11) != PIN_GetContextReg(context2, LEVEL_BASE::REG_R11))
    {
        printf("REG_R11 ERROR\n");
        compareOk = FALSE;
    }
    if (PIN_GetContextReg(context1, LEVEL_BASE::REG_R12) != PIN_GetContextReg(context2, LEVEL_BASE::REG_R12))
    {
        printf("REG_R12 ERROR\n");
        compareOk = FALSE;
    }
    if (PIN_GetContextReg(context1, LEVEL_BASE::REG_R13) != PIN_GetContextReg(context2, LEVEL_BASE::REG_R13))
    {
        printf("REG_R13 ERROR\n");
        compareOk = FALSE;
    }
    if (PIN_GetContextReg(context1, LEVEL_BASE::REG_R14) != PIN_GetContextReg(context2, LEVEL_BASE::REG_R14))
    {
        printf("REG_R14 ERROR\n");
        compareOk = FALSE;
    }
    if (PIN_GetContextReg(context1, LEVEL_BASE::REG_R15) != PIN_GetContextReg(context2, LEVEL_BASE::REG_R15))
    {
    ...
    if (!compareOk)
    {
        std::string s = disassemble((regInstPtr1), (regInstPtr1) + 15);
