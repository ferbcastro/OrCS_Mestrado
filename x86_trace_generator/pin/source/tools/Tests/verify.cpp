...
std::atomic< ADDRINT > lastInstPtr;
std::atomic< BOOL > isPredictedTaken;
std::atomic< ADDRINT > predictedInstPtr;
UINT64 icount = 0;
UINT64 errors = 0;
std::atomic< BOOL > isSkipped {TRUE}; // always skip checking the first inst
...
VOID ThreadStart(THREADID tid, CONTEXT* ctxt, INT32 flags, VOID* v)
{
    ...
    THREADID expectedValue = INVALID_THREADID;
    myThread.compare_exchange_strong(expectedValue, tid);
}
...
VOID CheckFlow(THREADID tid, ADDRINT instPtr, INT32 isTaken, ADDRINT fallthroughAddr, ADDRINT takenAddr, UINT32 stutters)
{
    ...
    isPredictedTaken.store((BOOL)isTaken);

    icount++;

    if (instPtr != predictedInstPtr.load() && !isSkipped.load() &&
        !(stutters && instPtr == lastInstPtr.load())) // An instruction which stutters can stay at the same IP
    {
        fprintf(stderr, "From: %p predicted InstPtr %p, actual InstPtr %p\n", (VOID*)(ADDRINT)lastInstPtr,
                (VOID*)(ADDRINT)predictedInstPtr, (VOID*)instPtr);
        IncrementError();
    }

    isSkipped.store(FALSE);

    if (isTaken)
    {
        predictedInstPtr.store(takenAddr);
    }
    else
    {
        predictedInstPtr.store(fallthroughAddr);
    }

    lastInstPtr.store(instPtr);
}

VOID Taken()
{
    if (!isPredictedTaken.load())
    {
...
VOID Skip() { isSkipped.store(TRUE); }
...
VOID CheckXlat(INS ins)
{
    ...
    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(SaveEa), IARG_THREAD_ID, IARG_MEMORYREAD_EA, IARG_END);
    INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(CheckXlatAfter), IARG_THREAD_ID, IARG_REG_VALUE, LEVEL_BASE::REG_EAX,
                   IARG_END);
}
#else
VOID CheckXlat(INS ins) {}
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    std::cerr << errors << " errors (" << icount << " instructions checked)" << std::endl;
    exit(errors);
}
