...
void* DLopen(const char* filename)
{
    ...
    if (handle == NULL)
    {
        std::cerr << "APP ERROR: failed to open " << filename << " - " << dlerror() << std::endl;
        exit(-1);
    }
    return handle;
}
...
void* DLsym(void* handle, const char* symname)
{
    ...
    if (error != NULL)
    {
        std::cerr << "APP ERROR: failed to get address for symbol " << symname << " - " << error << std::endl;
        exit(-1);
    }
    return sym;
}
...
void DLcloseBoth(void* first, void* second)
{
    if (dlclose(first) != 0)
    {
        std::cerr << "APP ERROR: failed to close image at " << first << " - " << dlerror() << std::endl;
        exit(-1);
    }
    if (dlclose(second) != 0)
    {
        std::cerr << "APP ERROR: failed to close image at " << second << " - " << dlerror() << std::endl;
        exit(-1);
    }
}
