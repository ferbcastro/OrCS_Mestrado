...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "", "specify trace file name");
...
{
    TraceFile << name << "(" << dst << "," << src << "," << size << ")" << std::endl;
}
...
VOID* IfuncMemcpyWrapper(CONTEXT* context, AFUNPTR orgFuncptr)
{
    ...
    PIN_CallApplicationFunction(context, PIN_ThreadId(), CALLINGSTD_DEFAULT, orgFuncptr, NULL, PIN_PARG(void*), &ret,
                                PIN_PARG_END());
    TraceFile << "memcpy (ifunc)" << std::endl;
    std::cout << "ifunc_memcpy("
         << ") returns " << std::hex << ret << std::endl;
    return ret;
}
...
VOID Routine(IMG img, RTN rtn, void* v)
{
    ...
    if (isMemmove || isMemcpy)
    {
        ...
        {
            TraceFile << "Ifunc memcpy" << std::endl;
        ...
        else
        {
            ...
            TraceFile << "Normal memcpy" << std::endl;
...
INT32 Usage()
{
    std::cerr << "This tool produces a trace of calls to normal and ifunc memcpy." << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    {
        std::cerr << "No output file name has been provided to the tool" << std::endl;
    }
