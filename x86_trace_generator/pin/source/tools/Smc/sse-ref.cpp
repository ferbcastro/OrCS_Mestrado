...
VOID mmx_arg(PINTOOL_REGISTER* r, UINT32 opnd_indx, UINT32 regno)
{
    char buffer[512 + 16];
    ...
    std::cout << "MMX" << regno << " operand_index: " << opnd_indx << " ";
    std::cout << std::setw(10) << r->dword[0] << " ";
    std::cout << std::setw(10) << r->dword[1] << " ";
    std::cout << std::endl;
#endif
...
VOID xmm_arg(PINTOOL_REGISTER* r, UINT32 opnd_indx, UINT32 regno)
{
#if defined(DEBUG_SSE_REF)
    std::cout << "XMM" << regno << " operand_index: " << opnd_indx << " ";
    for (unsigned int i = 0; i < MAX_DWORDS_PER_PINTOOL_REG; i++)
    {
        std::cout << std::setw(10) << r->dword[i] << " ";
    }
    std::cout << std::endl;
#endif
    // increment the destination...
