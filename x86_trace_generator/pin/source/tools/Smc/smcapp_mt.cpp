...
static unsigned long ExecuteInAllThreads(THREAD_POOL* tPool, const FUNC_OBJ& funcObj)
{
    ...
    for (unsigned long tid = 0; tid < numThreads; ++tid, ++startedThreads)
    {
        ...
        {
            std::cerr << "Thread " << tid << ": " << runObj->Name() << ": start failed" << std::endl;
            break;
        }
    }
    ...
    for (unsigned long tid = 0; tid < startedThreads; ++tid)
    {
        ...
        if (runObj == 0)
        {
            std::cerr << "Thread " << tid << ": " << funcObj.Name() << ": wait failed" << std::endl;
            continue;
        }

        std::cerr << "Thread " << tid << ": " << runObj->Name() << ": " << runObj->ErrorMessage() << std::endl;
        if (runObj->Status())
        {
            ++succeededThreads;
        }
...
int main(int argc, char* argv[])
{
    ...
    std::cerr << "Create " << numThreads << " threads" << std::endl;
    if (threadPool.Create(numThreads) != numThreads)
    {
        std::cerr << "Thread creation failed" << std::endl;
        return 1;
    }
    std::cerr << "All threads are created" << std::endl;
    ...
    for (int i = 0; i < 3; ++i)
    {
        unsigned long succeededThreads;
        {
            ...
            std::cerr << "Execute " << fooFunc.Name() << " in all threads" << std::endl;
            succeededThreads = ExecuteInAllThreads(&threadPool, fooFunc);
            if (succeededThreads != numThreads)
            {
                std::cerr << fooFunc.Name() << ": failed in " << (numThreads - succeededThreads) << " threads" << std::endl;
                return 1;
            }
        }
        {
            ...
            std::cerr << "Execute " << barFunc.Name() << " in all threads" << std::endl;
            succeededThreads = ExecuteInAllThreads(&threadPool, barFunc);
            if (succeededThreads != numThreads)
            {
                std::cerr << barFunc.Name() << ": failed in " << (numThreads - succeededThreads) << " threads" << std::endl;
                return 1;
            }
        }
    }
