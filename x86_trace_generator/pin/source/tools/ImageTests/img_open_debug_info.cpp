...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "img_open_debug_info_win.out",
...
static void output(ADDRINT address, INS ins)
{
    ...
    if (!filename.empty())
    {
        OutFile << "0x" << address << " " << asmOrFuncName << " # " << filename << ":" << line << std::endl;
        if (!debugInfoFound) debugInfoFound = TRUE;
    }
    else
    {
        OutFile << "0x" << address << " " << asmOrFuncName << " # "
                << "No debug info" << std::endl;
    }
}
INT32 Usage()
{
...
VOID OpenImage()
{
    ...
    if (!IMG_Valid(img))
    {
        OutFile << "Could not open " << KnobInputFile.Value() << std::endl;
        exit(1);
    }
    ...
    for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
    {
        ...
        for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
        {
            ...
            OutFile << "RTN address: 0x" << RTN_Address(rtn) << ", RTN name: " << RTN_Name(rtn) << std::endl;

            if (RTN_InsHead(rtn) == INS_Invalid())
            {
                OutFile << "No instructions in routine" << std::endl;
            }
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    OutFile << "Loading " << IMG_Name(img) << ", Image id = " << IMG_Id(img) << std::endl;
    if (KnobCheckImageRemoved)
    {
        ASSERT(IMG_Name(img).find("little_malloc") == std::string::npos,
