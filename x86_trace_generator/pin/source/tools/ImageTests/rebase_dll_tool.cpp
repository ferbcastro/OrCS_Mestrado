...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "rebase_dll.out", "specify file name");
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if (std::string::npos == IMG_Name(img).find("dummy_dll")) return;

    TraceFile << "library:" << IMG_Name(img).c_str() << std::endl;

    for (sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
        for (rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
            TraceFile << "rtn:" << RTN_Name(rtn).c_str() << std::endl;
}
