...
KNOB< std::string > KnobImagesLog(KNOB_MODE_WRITEONCE, "pintool", "o", "images_on_attach.log", "log file for the tool");
...
static VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if (IMG_IsMainExecutable(img))
    {
        ...
        {
            std::cerr << "TOOL ERROR: Unable to find the DoRelease function in the application." << std::endl;
            PIN_ExitProcess(1);
        }
        RTN_Open(doReleaseRtn);
        RTN_InsertCall(doReleaseRtn, IPOINT_BEFORE, MAKE_AFUNPTR(OnDoRelease), IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
...
int main(INT32 argc, CHAR* argv[])
{
    ...
    {
        std::cerr << "TOOL ERROR: Unable to open the images file." << std::endl;
        PIN_ExitProcess(1);
    }
