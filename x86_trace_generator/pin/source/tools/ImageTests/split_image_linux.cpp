...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "split_image.out", "specify output file name");

KNOB< bool > KnobVerbose(KNOB_MODE_WRITEONCE, "pintool", "verbose", "0", "verbose output");

std::vector< std::pair< std::string, ADDRESS_RANGE > > allImages;
std::vector< std::pair< std::string, ADDRESS_RANGE > > allRegions;
...
void mmap_replacement(void** start, ADDRINT flags, ADDRINT fd)
{
    ...
    if (NULL == *start && fd >= 0 && !(flags & MAP_ANON))
    {
        ...
        char path[4096]            = {0};
        char buf[4096]             = {0};
        (void)sprintf(path, "/proc/self/fd/%d", (int)fd);
 ...
 */
void checkRegion(std::string type, std::vector< std::pair< std::string, ADDRESS_RANGE > >& all, std::string newName, ADDRESS_RANGE newRange)
{
    for (std::vector< std::pair< std::string, ADDRESS_RANGE > >::iterator it = all.begin(); it != all.end(); it++)
    {
    ...
    all.push_back(std::pair< std::string, ADDRESS_RANGE >(newName, newRange));
}

VOID ImageLoad(IMG img, VOID* v)
{
    if (IMG_Name(img).find("ld-linux") != std::string::npos)
    {
