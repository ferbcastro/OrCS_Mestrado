...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "l_vdso_image.log", "log file for the tool");
...
{
    outFile << "Before __vdso_gettimeofday" << std::endl;
    beforeTimeOfDayCalled = true;
}

VOID clockGetTime64Before()
{
    outFile << "Before __vdso_clock_gettime64" << std::endl;
    beforeTimeOfDayCalled = true;
}
...
VOID kernelVSyscallRtnBefore(ADDRINT l_eax)
{
    ...
    if (l_eax == __NR_gettimeofday_time32)
#else
    if (l_eax == __NR_gettimeofday)
#endif
    {
        outFile << "Before __kernel_vsyscall with EAX equal to __NR_gettimeofday" << std::endl;
        beforeTimeOfDayCalled = true;
    }
}
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    {
        outFile << "Reached ins of vsyscall area, adderss = 0x" << std::hex << traceAddress << std::endl;
...
static VOID ImageLoad(IMG img, VOID* v)
{
    ...
    {
        std::cerr << "TOOL ERROR: Unable to find the __vdso_gettimeofday or __kernel_vsyscall functions in the application." << std::endl;
        PIN_ExitProcess(1);
    }
    ...
    if (RTN_Valid(kernelVSyscallRtn))
    {
        ...
        RTN_InsertCall(kernelVSyscallRtn, IPOINT_BEFORE, (AFUNPTR)kernelVSyscallRtnBefore, IARG_REG_VALUE, LEVEL_BASE::REG_EAX,
                       IARG_END);
