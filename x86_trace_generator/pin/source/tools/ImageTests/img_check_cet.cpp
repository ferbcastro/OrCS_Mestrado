...
KNOB< std::string > KnobImageName(KNOB_MODE_WRITEONCE, "pintool", "m", "", "image name to process");
...
int main(int argc, char* argv[])
{
    ...
    {
        std::cerr << "image name must be specified" << std::endl;
        PIN_ExitProcess(1);
    }
    ...
    if (!IMG_Valid(img))
    {
        std::cerr << "could not open image " + KnobImageName.Value();
        PIN_ExitProcess(1);
    }

    if (KnobIbt && !IMG_HasProperty(img, IMG_PROPERTY_IBT_ENABLED))
    {
        std::cerr << "image is not IBT enabled" << std::endl;
        PIN_ExitProcess(1);
    }
    if (KnobShstk && !IMG_HasProperty(img, IMG_PROPERTY_SHSTK_ENABLED))
    {
        std::cerr << "image is not SHSTK enabled" << std::endl;
        PIN_ExitProcess(1);
    }
    if (!KnobIbt && IMG_HasProperty(img, IMG_PROPERTY_IBT_ENABLED))
    {
        std::cerr << "image is IBT enabled" << std::endl;
        PIN_ExitProcess(1);
    }
    if (!KnobShstk && IMG_HasProperty(img, IMG_PROPERTY_SHSTK_ENABLED))
    {
        std::cerr << "image is SHSTK enabled" << std::endl;
        PIN_ExitProcess(1);
    }
