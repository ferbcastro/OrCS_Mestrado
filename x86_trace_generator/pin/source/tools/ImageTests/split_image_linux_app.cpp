...
int dl_iterate(struct dl_phdr_info* info, size_t size, void* data)
{
    ...
    std::vector< std::pair< void*, void* > > vecSegments;
    ...
    for (int j = 0; j < info->dlpi_phnum; j++)
    {
        if (info->dlpi_phdr[j].p_type == PT_LOAD)
        {
            ...
            vecSegments.push_back(std::pair< void*, void* >(start, end));
    ...
    if (*realname != 0)
    {
        for (std::vector< std::pair< void*, void* > >::iterator it = vecSegments.begin(); it != vecSegments.end(); it++)
        {
