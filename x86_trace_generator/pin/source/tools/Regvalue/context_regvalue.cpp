...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "context_regvalue.out", "specify output file name");
...
{
    OutFile << "Context values before ChangeRegs functions" << std::endl << std::flush;
    StoreContext(ctxt);
...
{
    OutFile << "Context values after ChangeRegs functions" << std::endl << std::flush;
    StoreContext(ctxt);
    PrintStoredRegisters(OutFile);
