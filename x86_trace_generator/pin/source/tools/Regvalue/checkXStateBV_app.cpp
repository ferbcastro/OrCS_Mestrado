...
int main(int argc, const char* argv[])
{
    ...
    if (argc != 2)
    {
        std::cerr << "ERROR: Expected 1 command line argument, got " << argc - 1 << ". Aborting test." << std::endl;
        return 1;
    }
    ...
    {
        std::cerr << "ERROR: Unknown state component '" << argv[1] << "'. Aborting test." << std::endl;
        return 2;
    }
    ...
    if (IsOn(regClass))
    {
        std::cerr << "WARNING: The " << componentStrings[regClass] << " state bit was expected to be clear at this point "
             << "in the test but it was set." << std::endl;
    }
    ...
    if (!IsOn(regClass))
    {
        std::cerr << "ERROR: The " << componentStrings[regClass] << " state bit was expected to be set at this point "
             << "in the test but it was clear." << std::endl;
        success = false;
    }
    GetRegValue(after, regClass);
    if (0 == memcmp(before, after, testRegSize[regClass]))
    {
        std::cerr << "ERROR: Error while testing the " << componentStrings[regClass] << " state. The values before and after "
             << "the test were the same." << std::endl;
        success = false;
    }
    if (0 != memcmp(after, toolRegisterValues[regClass], testRegSize[regClass]))
    {
        std::cerr << "ERROR: Error while testing the " << componentStrings[regClass] << " state. The value after the tool's change "
             << "did not match the expected value." << std::endl;
        success = false;
    }
