...
static void GetAndSetRflagsBefore(CONTEXT* ctxt, ADDRINT addr)
{
    ...
    if (firstBefore && PIN_ThreadId() == myThread) // to ensure that next run with changed context run "as is"
    {
        ...
        if (changedFlag)
        {
            ...
            if (fVal != rflagsAfter)
            {
                *OutFile << "FLAGS values " << hexstr(fVal) << " did not change to " << hexstr(rflagsAfter) << std::endl << std::flush;
...
static void GetAndRestoreRflagsAfter(CONTEXT* ctxt, ADDRINT addr)
{
    if (PIN_ThreadId() == myThread)
    {
        ...
        if (val != rflagsAfter)
        {
            *OutFile << "Expecting " << hexstr(rflagsAfter) << " saw " << hexstr(val) << std::endl << std::flush;
...
static VOID InstructionForCanonicalContext(INS ins, VOID* v)
{
    ...
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(GetAndSetRflagsBefore), IARG_CONTEXT, IARG_INST_PTR, IARG_END);
        INS_InsertCall(nextIns, IPOINT_BEFORE, MAKE_AFUNPTR(GetAndRestoreRflagsAfter), IARG_CONTEXT, IARG_INST_PTR, IARG_END);
    }
}
...
static VOID InstructionForPartialContext(INS ins, VOID* v)
{
    ...
    if (IsInstructionsInstrumentable(ins, nextIns))
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(GetAndSetRflagsBefore), IARG_PARTIAL_CONTEXT, &regsIn, &regsOut,
                       IARG_INST_PTR, IARG_END);
        INS_InsertCall(nextIns, IPOINT_BEFORE, MAKE_AFUNPTR(GetAndRestoreRflagsAfter), IARG_PARTIAL_CONTEXT, &regsIn, &regsOut,
                       IARG_INST_PTR, IARG_END);
    }
}
...
int main(int argc, char* argv[])
{
    ...
    *OutFile << " Knob value " << decstr(KnobChangeFlags.Value()) << std::endl << std::flush;
