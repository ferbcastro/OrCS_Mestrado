...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "standard-memop.out", "specify output file name");
...
INT32 Usage()
{
    std::cerr << "This tool preforms various test cases which are related to PIN's behavior" << std::endl
         << "when it tries to instrument XSAVE family instructions which has non-standard memory operand" << std::endl;
 ...
 * ===================================================================== */
VOID MemOpAnalysisXSAVE_EA(ADDRINT addr) { *out << std::hex << "XSAVE on 0x" << addr << std::endl; }

VOID MemOpAnalysisXSAVE_Size(UINT32 addr) { *out << std::hex << "XSAVE of size 0x" << addr << std::endl; }

/* =====================================================================
 * The analysis routine that is instrumented before/after XSAVEOPT instruction
 * ===================================================================== */
VOID MemOpAnalysisXSAVEOPT_EA(ADDRINT addr) { *out << std::hex << "XSAVEOPT on 0x" << addr << std::endl; }

VOID MemOpAnalysisXSAVEOPT_Size(UINT32 addr) { *out << std::hex << "XSAVEOPT of size 0x" << addr << std::endl; }

/* =====================================================================
 * The analysis routine that is instrumented before/after XRSTOR instruction
 * ===================================================================== */
VOID MemOpAnalysisXRSTOR_EA(ADDRINT addr) { *out << std::hex << "XRSTOR on 0x" << addr << std::endl; }

VOID MemOpAnalysisXRSTOR_Size(UINT32 addr) { *out << std::hex << "XRSTOR of size 0x" << addr << std::endl; }
...
int main(int argc, CHAR* argv[])
{
    ...
    {
        *out << "Bad test case number specified on command line: " << KnobTestCase << std::endl;
        return 4;
    }
    if (KnobIpoint < 0 || KnobIpoint > (int)(sizeof(TESTCASE_IPOINT) / sizeof(TESTCASE_IPOINT[0]) - 1))
    {
        *out << "Bad test case number specified on command line: " << KnobIpoint << std::endl;
        return 4;
    }
