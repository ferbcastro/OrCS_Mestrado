...
KNOB< std::string > KnobModificationMethod(KNOB_MODE_WRITEONCE, "pintool", "method", "context",
...
KNOB< std::string > KnobTestStateClass(KNOB_MODE_WRITEONCE, "pintool", "stateClass", "x87",
                                       "specify which context to test. One of x87|SSE|AVX (case sensitive).");

// A knob for defining the output file name
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "checkXStateBV_tool.out", "specify output file name");
...
static void ChangeRegisterReference(PINTOOL_REGISTER* reg)
{
    const UINT64* newval = reinterpret_cast< const UINT64* >(toolRegisterValues[regClass]);
...
static VOID ImageLoad(IMG img, VOID* v)
{
    if (IMG_IsMainExecutable(img))
    {
        ...
        {
            RTN_InsertCall(changeRegisterValueRtn, IPOINT_BEFORE, MAKE_AFUNPTR(ChangeRegisterAndExecute), IARG_CONTEXT,
                           IARG_ADDRINT, RTN_Address(executeAtRtn), IARG_END);
        }
        else if (KnobModificationMethod.Value() == "partial")
        {
            ...
            RTN_InsertCall(changeRegisterValueRtn, IPOINT_BEFORE, MAKE_AFUNPTR(ChangeRegister), IARG_PARTIAL_CONTEXT, &regsin,
                           &regsout, IARG_END);
        }
        else if (KnobModificationMethod.Value() == "reference")
        {
            RTN_InsertCall(changeRegisterValueRtn, IPOINT_BEFORE, MAKE_AFUNPTR(ChangeRegisterReference), IARG_REG_REFERENCE,
                           GetRegister(), IARG_END);
        }
        else
        {
            OutFile << "ERROR: Unknown modification method specified: " << KnobModificationMethod.Value() << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    {
        OutFile << "ERROR: Unknown state class: " << KnobTestStateClass.Value() << std::endl;
