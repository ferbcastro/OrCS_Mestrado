...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "emu_context_regvalue.out", "specify output file name");
...
bool CheckAllModifiedValues(std::ostream& ost)
{
    bool success       = true;
...
static VOID OnSyscall(THREADID tid, CONTEXT* ctxt, SYSCALL_STANDARD std, VOID* v)
{
    ...
    OutFile << "Begin tests before entering system call" << std::endl;
    ...
    ModifyContext(ctxt);
    OutFile << std::endl << "After modifications" << std::endl;
    StoreContext(ctxt);
    PrintStoredRegisters(OutFile);
    if (!CheckAllModifiedValues(OutFile))
    {
        OutFile << "ERROR: values mismatch" << std::endl << std::flush;
        PIN_ExitApplication(1); // never returns
    }

    // Restore the original context
    PIN_SaveContext(&copy, ctxt);
    OutFile << std::endl << "After restoring" << std::endl;
    StoreContext(ctxt);
    PrintStoredRegisters(OutFile);

    // Test complete
    OutFile << std::endl << "Done" << std::endl;
}
