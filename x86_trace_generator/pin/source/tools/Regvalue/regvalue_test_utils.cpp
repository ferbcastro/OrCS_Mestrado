...
class RegisterDB
{
  public:
    ...
    inline const std::vector< REG >& GetTestRegs() const;
    inline const REGSET& GetTestRegset() const;
    void PrintStoredRegisters(std::ostream& ost) const;
  ...
  private:
    ...
    std::vector< REG > _testRegs; // A vector of all the ARS REGs.
    ...
    std::map< REG, TestReg > _regToTestReg;
  ...
  private:
    ...
    inline void PrintRegval(REG reg, std::ostream& ost) const;
...
const std::vector< REG >& RegisterDB::GetTestRegs() const { return _testRegs; }

const REGSET& RegisterDB::GetTestRegset() const { return _testRegset; }

void RegisterDB::PrintStoredRegisters(std::ostream& ost) const
{
...
void RegisterDB::PrintRegval(REG reg, std::ostream& ost) const
{
    ost << REG_StringShort(reg) << " = " << Val2Str(_regvalMap.find(reg)->second, REG_Size(reg)) << std::endl << std::flush;
}
...
const std::vector< REG >& GetTestRegs() { return RegisterDB::Instance()->GetTestRegs(); }

const REGSET& GetTestRegset() { return RegisterDB::Instance()->GetTestRegset(); }

void PrintStoredRegisters(std::ostream& ost) { RegisterDB::Instance()->PrintStoredRegisters(ost); }
...
bool CheckAllExpectedValues(std::ostream& ost)
{
    bool success              = true;
