...
VOID CheckFptagConversion(char* fnstenv_area, const CONTEXT* ctxt)
{
    ...
    {
        std::cerr << "Error in conversion from 16b to 8b" << std::endl;
        exit(-1);
    }
    //Checking full conversion from 8b to 16b
    if (fullTag != REG_ConvertX87AbridgedTagToFull(&fpState->fxsave_legacy))
    {
        std::cerr << "Error in full conversion from 8b to 16b" << std::endl;
        exit(-2);
    }
}
/* ===================================================================== */
...
INT32 Usage()
{
    std::cerr << "This tool checks FPTAG conversion" << std::endl;
    return -1;
}
