...
// The test fills stack in units of 1K ADDRINT entries (referred to as stack fill units).
// Number of stack fill units, corresponding to the specified stack size.
static INT32 fillDepth;
...
// SPARE_SPACE is defined in stack fill units to account for current stack size once recursion starts.
...
ADDRINT stackAlloc(INT32 depth)
{
    ADDRINT arr[1024];
...
{
    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(fillThreadStack), IARG_THREAD_ID, IARG_END);
}
...
int main(INT32 argc, CHAR** argv)
{
    ...
    // Also account for extra space in each recursive frame.
    fillDepth = KnobStackSize.Value() / sizeof(ADDRINT) /* max # of stack fill units that may fit in the stack */
        - KnobStackSize.Value() / 32 /* 1/32 size of stack frame in stackAlloc() considered as size of other vars in its frame */
        - SPARE_SPACE;
    if (fillDepth <= 0)
    {
        std::cerr << "Specified stack size is too small" << std::endl;
        std::cerr.flush();
        exit(-1);
    }
