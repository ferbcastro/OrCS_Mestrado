...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "address_mapping_probe.out", "specify output file name");
...
VOID TestMemory()
{
    ...
    free(bigMalloc);
    for (std::vector< void* >::iterator it = smallMallocs.begin(); it != smallMallocs.end(); it++)
    {
        free(*it);
    }
