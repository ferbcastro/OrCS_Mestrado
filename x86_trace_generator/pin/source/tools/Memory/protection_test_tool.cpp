...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "protection_test_tool.log", "specify tool log file name");
                        ...
                        VOID* v)
{
    ...
    {
        OutFile << "Tool: Start handling exception." << std::endl;
        OutFile.flush();
...
{
    OutFile << "PinFiniFunction" << std::endl;
    OutFile.flush();
    OutFile.close();
}
