...
static void AbortProcess(const std::string& msg)
{
    ...
    *TraceFile << "malloc_stress test aborted: " << msg << "." << std::endl << std::flush;
...
VOID SpawnThreads()
{
    ...
    for (ADDRINT threads_started = 0; threads_started < KnobNumThreads.Value(); ++threads_started)
    {
        ...
        *TraceFile << "Creating thread " << std::endl;
...
static VOID PrepareForFini(VOID* v)
{
    for (UINT32 i = 0; i < KnobNumThreads.Value(); ++i)
    {
        ...
        {
            *TraceFile << "Tool's thread " << i << " did not started" << std::endl;
            continue;
        }
    ...
    *TraceFile << "All threads joined" << std::endl;
}
...
INT32 Usage()
{
    std::cerr << "This tool test for malloc_stress" << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    *TraceFile << "All Threads Created" << std::endl;
