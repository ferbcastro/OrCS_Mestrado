...
#if defined(TARGET_WINDOWS)
    WINDOWS::MEMORYSTATUSEX memStatus;
    memStatus.dwLength = sizeof(memStatus);
    if (WINDOWS::GlobalMemoryStatusEx(&memStatus) == 0)
    {
        std::cerr << "BIG MALLOC ERROR: Encountered an error while reading the memory status from the system." << std::endl;
        PIN_ExitProcess(1);
    }
...
int main(int argc, char* argv[])
{
    ...
    {
        std::cerr << "BIG MALLOC WARNING: The system does not have enough available memory for the big allocation." << std::endl;
        PIN_StartProgram(); // never returns
        std::cerr << "BIG MALLOC ERROR: This line should not have been reached." << std::endl;
        PIN_ExitProcess(2);
    }
    ...
    if (bigPtr == NULL)
    {
        ...
        std::cerr << "BIG MALLOC ERROR: Unable to allocate the requested " << mbSize << " MB of memory." << std::endl;
        PIN_ExitProcess(3);
    }
