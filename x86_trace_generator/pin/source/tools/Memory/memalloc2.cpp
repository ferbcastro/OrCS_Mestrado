...
/**
 * @file memalloc2.cpp
 * @brief The pintool in this file is used to test OOM condition and to verify that
 *        the OOM callbacks are called. On Linux we limit the process address space
 *        before testing.
 * 
 */
...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "memalloc2.out", "specify memalloc2 file name");
...
void LimitAvailableSpace()
{
    ...
    // Make the limit at most 80% of total swap area or 800Mb
    if (totalswap == 0)
    {
        myLimit = (TOP_LIMIT / 10) * 8;
    }
    else
    {
...
// clang compiler optimization removes part of
// apparently unnecessary code, disabled for this function
// Also ICX compiler optimizes out this function, clang includes it.
#if __clang__
