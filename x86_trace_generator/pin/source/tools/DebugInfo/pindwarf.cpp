...
KNOB< std::string > KnobBinary(KNOB_MODE_WRITEONCE, "pintool", "bin", "", "specify binary file name for dwarf parsing");
// The output file - where to dump the subroutines list
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "", "specify stdout file name");
...
void parse_binary_dwarf(const char* binary)
{
    ...
    {
        // The format of the printed lines should match the format in the reference files (llvm-diff.intel64.ref / llvm-diff.ia32.ref)
