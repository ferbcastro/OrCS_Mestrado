...
const std::string& Target2RtnName(ADDRINT target);
const std::string& Target2LibName(ADDRINT target);
...
std::set< void* > addrsToDump;
std::set< ADDRINT > pushIps;
...
const std::string& Target2RtnName(ADDRINT target)
{
    const std::string& name = RTN_FindNameByAddress(target);

    if (name == "")
        return *new std::string("[Unknown routine]");
    else
...
const std::string& Target2LibName(ADDRINT target)
{
    ...
    static const std::string _invalid_rtn("[Unknown image]");
...
void A_UnregisterAddr(void* addr)
{
    ...
    {
        std::cerr << "MAID ERROR: unregistered address " << std::hex << addr << std::dec << std::endl;
    }
    else
    {
...
void A_ProcessSyscall(ADDRINT ip, UINT32 num, ADDRINT sp, ADDRINT arg0)
{
    if (main_entry_seen)
    {
        //cout << callStack.Depth() << ":" << SYS_SyscallName(num) << std::endl;
    }
...
{
    //cout << "Direct call: " << Target2String(target) << std::endl;
    callStack.ProcessCall(sp, target);
}

void A_ProcessIndirectCall(ADDRINT ip, ADDRINT target, ADDRINT sp)
{
    //cout << "Indirect call: " << Target2String(target) << std::endl;
    callStack.ProcessCall(sp, target);
}

static void A_ProcessStub(ADDRINT ip, ADDRINT target, ADDRINT sp)
{
    //cout << "Instrumenting stub: " << Target2String(target) << std::endl;
    //cout << "STUB: ";
    //cout << Target2RtnName(target) << std::endl;
    callStack.ProcessCall(sp, target);
}
...
static void A_DoMem(bool isStore, void* ea, ADDRINT pc)
{
    ...
    if (addrsToDump.find(ea) != addrsToDump.end())
    {
        ...
        *Output << (isStore ? "store" : "load") << " pc=" << (void*)pc << " ea=" << ea << std::endl;
        if (filename != "")
        {
...
int main(int argc, char** argv)
{
    ...
    {
        if (!(Output = new std::ofstream(strTmp)))
        {
            perror(strTmp);
            exit(1);
        }
