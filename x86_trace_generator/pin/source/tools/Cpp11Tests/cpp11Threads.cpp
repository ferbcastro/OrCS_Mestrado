...
// starting from glibc-2.34, applications starts using clone3() syscall instead of the regular clone() syscall
// so we need to check and support those type of syscall as well
#define SC_clone3 435

/* ===================================================================== */
// Command line switches
/* ===================================================================== */
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "", "specify file name for cpp11Threads output");
...
SYSCALL_SEMANTIC GetSyscallSemantic(CONTEXT* ctx, SYSCALL_STANDARD std)
{
    ...
    switch (PIN_GetSyscallNumber(ctx, std))
    {
        ...
        {
            ADDRINT arg0         = PIN_GetSyscallArgument(ctx, std, 0);
            UINT64 syscall_flags = *((UINT64*)(arg0));
            if (IsVForkOrFork(syscall_flags))
                res = SCS_UNKNOWN;
            else
                res = SCS_THREADCREATE;
            break;
...
{
    *out << "===============================================" << std::endl;
    *out << "cpp11Threads analysis results: " << std::endl;
    *out << "Number of threads started: " << threadStartCount << std::endl;
    *out << "Number of threads created: " << threadCreateCount << std::endl;
    *out << "Number of threads finish: " << threadFiniCount << std::endl;
    *out << "===============================================" << std::endl;
}
