                                              ...
                                              CONTEXT* ctxt)
{
    ...
    Out << "IpointBeforeIndirectThruStackCall at rip " << std::hex << rip << " branchTargetAddr " << std::hex << branchTargetAddr
        ...
        << displacementSigned << "]==(" << std::hex << (rsp + displacementSigned) << ") contains " << std::hex << *ptr1 << std::endl;
    if (*ptr1 != branchTargetAddr)
    {
        Out << "***Error expected *ptr1 == branchTargetAddr" << std::endl;
        exit(-1);
    }
                                               ...
                                               CONTEXT* ictxt)
{
    ...
    Out << "IpointBeforeIndirectThruStackCall1 at rip " << std::hex << rip << " branchTargetAddr " << std::hex << branchTargetAddr
        ...
        << (rsp + displacementSigned) << ") contains " << std::hex << *ptr1 << std::endl;
    if (*ptr1 != branchTargetAddr)
    {
        Out << "***Error expected *ptr1 == branchTargetAddr" << std::endl;
        exit(-1);
    ...
    {
        Out << "***Error expected savedBranchTargetAddr == branchTargetAddr" << std::endl;
        exit(-1);
    }
}
...
static void PreviousInsExecutedWasAnIndirectCallThruStack(ADDRINT rip)
{
    ...
    Out << "IndirectThruStackCall target reached at rip " << std::hex << rip << std::endl;
    if (rip != savedBranchTargetAddr)
    {
        Out << "***Error expected rip == savedBranchTargetAddr" << std::endl;
        exit(-1);
    }
...
static void InstrumentIndirects(INS ins, VOID*)
{
        ...
        INS_OperandMemoryBaseReg(ins, 0) == REG_STACK_PTR)
    {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(IpointBeforeIndirectThruStackCall), IARG_BRANCH_TARGET_ADDR,
                       IARG_REG_VALUE, REG_STACK_PTR, IARG_ADDRINT, INS_MemoryDisplacement(ins), IARG_INST_PTR, IARG_CONTEXT,
                       IARG_END);
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(IpointBeforeIndirectThruStackCall1), IARG_BRANCH_TARGET_ADDR,
                       IARG_REG_VALUE, REG_STACK_PTR, IARG_ADDRINT, INS_MemoryDisplacement(ins), IARG_INST_PTR,
                       IARG_CONST_CONTEXT, IARG_END);
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(IpointBeforeIndirectThruStackCall2), IARG_BRANCH_TARGET_ADDR,
                       IARG_REG_VALUE, REG_STACK_PTR, IARG_ADDRINT, INS_MemoryDisplacement(ins), IARG_INST_PTR, IARG_END);
        INS_InsertCall(ins, IPOINT_TAKEN_BRANCH, MAKE_AFUNPTR(IpointTakenBranchIndirectThruStackCall), IARG_BRANCH_TARGET_ADDR,
                       IARG_INST_PTR, IARG_CONTEXT, IARG_END);
        INS_InsertCall(ins, IPOINT_TAKEN_BRANCH, MAKE_AFUNPTR(IpointTakenBranchIndirectThruStackCall1), IARG_BRANCH_TARGET_ADDR,
                       IARG_INST_PTR, IARG_CONST_CONTEXT, IARG_END);
        INS_InsertCall(ins, IPOINT_TAKEN_BRANCH, MAKE_AFUNPTR(IpointTakenBranchIndirectThruStackCall2), IARG_BRANCH_TARGET_ADDR,
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    {
        Out << "***Error not all expected tests executed" << std::endl;
        exit(-1);
    }
}
