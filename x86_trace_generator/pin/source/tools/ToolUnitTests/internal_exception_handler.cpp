...
volatile WINDOWS::LPVOID ptr1   = NULL;
volatile BOOL isGlobalExpected2 = FALSE;
volatile WINDOWS::LPVOID ptr2   = NULL;
...
{
    std::cout << "AnalysisHandler2: Caught exception. " << PIN_ExceptionToString(pExceptInfo) << std::endl << std::flush;
    ...
    {
        std::cout << "AnalysisHandler2: Unmatching exception address. Abort" << std::endl << std::flush;
        return EHR_UNHANDLED;
    }
    else
    {
        std::cout << "AnalysisHandler2: Matching exception address. Continue search" << std::endl << std::flush;
...
{
    std::cout << "AnalysisHandler1: Caught exception. " << PIN_ExceptionToString(pExceptInfo) << std::endl << std::flush;
    if (isExpected[tid])
    {
        std::cout << "AnalysisHandler1: Raising exception on behalf of the application" << std::endl << std::flush;
...
{
    std::cout << "GlobalHandler2: Caught exception. " << PIN_ExceptionToString(pExceptInfo) << std::endl << std::flush;
    return EHR_CONTINUE_SEARCH;
}

EXCEPT_HANDLING_RESULT GlobalHandler1(THREADID threadIndex, EXCEPTION_INFO* pExceptInfo, PHYSICAL_CONTEXT* pPhysCtxt, VOID* v)
{
    std::cout << "GlobalHandler1: Caught exception. " << PIN_ExceptionToString(pExceptInfo) << std::endl << std::flush;
    if (isGlobalExpected1 && (PIN_GetExceptionCode(pExceptInfo) == EXCEPTCODE_RECEIVED_ACCESS_FAULT))
    {
        isGlobalExpected1 = FALSE;
        std::cout << "GlobalHandler1: Fixing exception and continuing execution" << std::endl << std::flush;
        //Fix and continue execution
        WINDOWS::VirtualAlloc(ptr1, 0x1000, MEM_COMMIT, PAGE_READWRITE);
        return EHR_HANDLED;
    }
    else if (isGlobalExpected2 && (PIN_GetExceptionCode(pExceptInfo) == EXCEPTCODE_RECEIVED_ACCESS_FAULT))
    {
        ...
        std::cout << "GlobalHandler1: Fixing exception and continuing execution" << std::endl << std::flush;
        //Fix and continue execution
        WINDOWS::VirtualAlloc(ptr2, 0x1000, MEM_COMMIT, PAGE_READWRITE);
        return EHR_HANDLED;
    }
    else
    {
        // Message logging
        std::cout << "GlobalHandler1: Unexpected exception. Abort" << std::endl << std::flush;
 ...
 * Go around clang optimization to force clang to issue
 * a real div instruction and exception
 */
volatile int g_zero = 1;
volatile int g_result = 1;

__declspec(noinline) static VOID IntDivideByZeroException() 
{
    g_result = g_result / g_zero;
}
...
VOID AppStart(VOID* v)
{
    ...
    *(int*)ptr1 = 12345;
    ...
    WINDOWS::VirtualFree(ptr1, 0x1000, MEM_DECOMMIT);
}
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    if (j != 0)
    {
        std::cout << "internal_exception_handler : Unexpected value: " << j << std::endl << std::flush;
    }
    else
    {
        std::cout << "internal_exception_handler : Completed successfully" << std::endl << std::flush;
    }
}
...
int main(int argc, char* argv[])
{
    ...
    //Reserve memory
    ptr1 = WINDOWS::VirtualAlloc(NULL, 0x1000, MEM_RESERVE, PAGE_READWRITE);
    ptr2 = WINDOWS::VirtualAlloc(NULL, 0x1000, MEM_RESERVE, PAGE_READWRITE);
