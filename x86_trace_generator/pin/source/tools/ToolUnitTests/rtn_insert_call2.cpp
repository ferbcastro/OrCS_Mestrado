    ...
    for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
    {
        for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
        {
            ...
            if (INS_IsValidForIpointAfter(ins))
            {
                ...
                numRtnsInstrumented++;
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(AtRtn), IARG_ADDRINT, RTN_Address(rtn), IARG_THREAD_ID, IARG_END);

                INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(BeforeInsHead), IARG_ADDRINT, INS_Address(ins), IARG_THREAD_ID,
                               IARG_END);
                INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(AfterInsHead), IARG_ADDRINT, INS_Address(ins), IARG_THREAD_ID,
                               IARG_END);
                ins = RTN_InsHeadOnly(rtn);
                ASSERTX(INS_Invalid() != ins);
                ASSERTX(INS_Address(ins) == insAddress);
                INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(BeforeInsHeadOnly), IARG_ADDRINT, insAddress, IARG_THREAD_ID,
                               IARG_END);
                INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(AfterInsHeadOnly), IARG_ADDRINT, insAddress, IARG_THREAD_ID,
                               IARG_END);
            }
            RTN_Close(rtn);
        }
