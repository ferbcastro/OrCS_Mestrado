...
VOID Instruction(INS ins, VOID* v)
{
    if (INS_IsMemoryRead(ins) && !instrumentedReadFromIpWithNoOffset)
    {
        ...
        if (INS_IsValidForIpointAfter(ins)) // Instrument only if both before and after can be instrumented
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)IpReadBefore, IARG_INST_PTR, IARG_MEMORYREAD_EA, IARG_REG_VALUE,
