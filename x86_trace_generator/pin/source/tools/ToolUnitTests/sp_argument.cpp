    ...
    if (RTN_Valid(rtn))
    {
        ...
        if (!PIN_IsProbeMode())
        {
            RTN_ReplaceSignature(rtn, AFUNPTR(check_sp_value), IARG_PROTOTYPE, proto, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                                 IARG_REG_VALUE, LEVEL_BASE::REG_STACK_PTR, IARG_REG_VALUE, LEVEL_BASE::REG_ESP, IARG_REG_VALUE,
                                 LEVEL_BASE::REG_SP, IARG_END);
        }
        else if (RTN_IsSafeForProbedReplacement(rtn))
        {
            RTN_ReplaceSignatureProbed(rtn, AFUNPTR(check_sp_value), IARG_PROTOTYPE, proto, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                                       IARG_REG_VALUE, LEVEL_BASE::REG_STACK_PTR, IARG_REG_VALUE, LEVEL_BASE::REG_ESP,
                                       IARG_REG_VALUE, LEVEL_BASE::REG_SP, IARG_END);
        }
