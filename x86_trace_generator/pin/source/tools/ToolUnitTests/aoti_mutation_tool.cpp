...
static VOID insertIndirectJump(RTN rtn)
{
    for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins))
    {
        if (INS_IsMov(ins) && INS_HasImmediateOperand(ins))
        {
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(returnValue), IARG_ADDRINT, INS_Address(ins) + INS_Size(ins),
...
static VOID modifyAddressing(RTN rtn)
{
    for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins))
    {
        if (INS_Opcode(ins) == XED_ICLASS_OR)
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(AddrValueA), IARG_MEMORYOP_EA, 0, IARG_END);

            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(returnValueMinus4), IARG_MEMORYOP_EA, 0, IARG_RETURN_REGS, scratchReg,
                           IARG_END);
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(AddrValueB), IARG_MEMORYOP_EA, 0, IARG_END);
