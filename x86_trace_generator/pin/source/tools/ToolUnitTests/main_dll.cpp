...
KNOB< std::string > KnobOutputFile1(KNOB_MODE_WRITEONCE, "pintool", "o1", "static_secondary_dll.out", "Output file 1");

KNOB< std::string > KnobOutputFile2(KNOB_MODE_WRITEONCE, "pintool", "o2", "dynamic_secondary_dll.out", "Output file 2");
...
{
    std::cerr << info->dlpi_name << " " << std::hex << info->dlpi_addr << " " << info->dlpi_phdr->p_memsz << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    if (module == NULL)
    {
        std::cerr << "Failed to load dynamic_secondary_dll.dll" << std::endl;
        exit(1);
    }
    ...
    {
        std::cerr << "Failed to find proc addresses in dynamic_secondary_dll.dll" << std::endl;
        exit(1);
    }
