...
int main(int argc, char** argv)
{
    ...
    for (UINT32 i = 0; i < NUM_XMM_REGS; i++)
    {
        ...
        if (valsToSet[i] != x)
        {
            std::cout << "ERROR during first init_sse\n";
            exit(-1);
        }
    }
    ...
    for (UINT32 i = 0; i < NUM_XMM_REGS; i++)
    {
        ...
        {
            std::cout << "ERROR during second init_sse\n";
            exit(-1);
        }
    }
    std::cout << std::endl;
    ...
    for (UINT32 i = 0; i < NUM_XMM_REGS; i++)
    {
        ...
        {
            std::cout << "ERROR during sse restore\n";
            exit(-1);
        }
    }
    std::cout << "xmm registers saved and restored successfully\n";
    return 0;
}
