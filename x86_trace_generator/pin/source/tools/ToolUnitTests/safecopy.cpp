...
// Global variables
/* ================================================================== */
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "safecopy.out", "specify output file name");
...
VOID* MemAlloc(size_t size)
{
    ...
    pageFrameStart = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, -1, 0);
...
{
    int prot = enableAccess ? (PROT_READ | PROT_WRITE) : PROT_NONE;
...
VOID SafeCopyTest()
{
    ...
    out << "SafeCopy (A): Entire buffer has been copied successfully." << std::endl << std::flush;
    ...
    out << "SafeCopy (B): Accessible part of the buffer has been copied successfully." << std::endl << std::flush;

    //C.
    MemProtect(dst, pageSize, FALSE); // dst is inaccessible
    copySize = PIN_SafeCopy(dstBuf, srcBuf, bufSize);
    ASSERT((copySize == 0), "SafeCopy (C) failed.\n");
    out << "SafeCopy (C): Inaccessible buffer has not been copied." << std::endl << std::flush;
...
VOID SafeCopyTestInTrace()
{
    ...
    {
        first = FALSE;
        out << "Test SafeCopy in the trace analysis routine." << std::endl << std::flush;
        SafeCopyTest();
    }
}
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    {
        first = FALSE;
        out << "Test SafeCopy in the trace instrumentation callback." << std::endl << std::flush;
        SafeCopyTest();
    ...
    if (probeAddr != 0)
    {
        out << "Test SafeCopy in a region overwritten by probe." << std::endl << std::flush;
        CHAR buffer[8];
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    {
        first = FALSE;
        out << "Test SafeCopy in the image instrumentation callback." << std::endl << std::flush;
        SafeCopyTest();
    }
}

VOID Fini(INT32 code, VOID* v) { out << "SafeCopy test completed." << std::endl << std::flush; }
