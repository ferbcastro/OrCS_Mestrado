...
class sse_aligner_t
{
  public:
    ...
    std::ofstream* out;
    KNOB< std::string > knob_output_file;
    KNOB< BOOL > knob_pid;
    ...
    void activate()
    {
        ...
        if (knob_verbose)
        {
            std::string filename = knob_output_file.Value();
            if (knob_pid)
            {
        ...
        if (verbose) *out << "sse aligner activated" << std::endl;
    }
    ...
    {
        if (pthis->verbose) *(pthis->out) << "thead begin " << static_cast< uint32_t >(tid) << std::endl;
    ...
    static void rewrite_instruction(INS ins, bool is_read, sse_aligner_t* pthis)
    {
        ...
        if (pthis->verbose)
            *(pthis->out) << "REWRITE " << std::string(is_read ? "LOAD :" : "STORE:") << std::setw(16) << std::hex << INS_Address(ins)
        ...
        if (is_read)
        {
            ...
            // an aligned bufffer.
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(copy_to_aligned_load_buffer_and_return_pointer), IARG_MEMORYREAD_EA,
        ...
        else
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(return_pointer_to_aligned_store_buffer), IARG_MEMORYWRITE_EA,
                           IARG_INST_PTR, IARG_THREAD_ID, IARG_PTR, pthis, IARG_REG_REFERENCE, REG_INST_G1, IARG_RETURN_REGS,
                           REG_INST_G0, IARG_END);
            INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(copy_from_aligned_store_buffer), IARG_REG_VALUE, REG_INST_G1,
