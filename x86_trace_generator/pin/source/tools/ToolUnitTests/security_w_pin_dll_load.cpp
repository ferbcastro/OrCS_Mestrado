...
{
    std::cerr << "Load dynamic_secondary_dll.dll" << std::endl;
    void* dllHandle = dlopen("dynamic_secondary_dll.dll", RTLD_NOW);
    if (NULL == dllHandle)
    {
        std::cerr << "Failure loading dynamic_secondary_dll.dll : " << dlerror() << std::endl << std::flush;
        return 1;
    }
    return 0;
...
static VOID imageLoad(IMG img, VOID* v)
{
    ...
    {
        std::cerr << "Instrument AfterSetCurrentDirectory function" << std::endl;
        ...
        std::cerr << "AfterSetCurrentDirectory function is not found or unsafe to instrument" << std::endl << std::flush;
        PIN_ExitProcess(2);
    }
}
