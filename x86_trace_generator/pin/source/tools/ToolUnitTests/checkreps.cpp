...
static KNOB< std::string > KnobOutput(KNOB_MODE_WRITEONCE, "pintool", "o", "checkreps.out", "output file");
...
{
    out << std::endl << "Dynamic Instruction Count" << std::endl;
    out << "Total dynamic instructions: " << totalCount << std::endl << std::endl;

    out << "Op              Count  Repeated Count       %      Zero Count   Mean reps(NZ)" << std::endl;
...
static VOID printMemoryAccessStats()
{
    ...
    out << "Dynamic Memory Operation Count" << std::endl;
    out << "Memory Ops:     Reads       %          Writes       %" << std::endl;
    ...
    allReps.output();
    out << std::endl;
    out << "Reads+Writes" << std::endl;
    out << " Whole Program ";
    out << std::setw(fieldWidth) << totalMemoryOps.allOps() << std::endl;
    out << " REP prefixed  " << std::setw(fieldWidth) << allReps.allOps();
    out << " (" << std::fixed << std::setprecision(1) << (100.0 * allReps.allOps()) / totalMemoryOps.allOps() << "%)" << std::endl;
}
...
static VOID ClearPrevRep() { prevRep = 0; }
...
static VOID addCount(ADDRINT ip, UINT32 opIdx, UINT32 firstRep, UINT32 repCount)
{
    ...
    if (firstRep)
    {
        if (prevRep == ip)
        {
            out << "***FirstRep error (set)  at " << std::hex << ip << " prevRep " << prevRep << std::dec << " count " << repCount << std::endl;
        }
        prevRep = ip;
    }
    else
    {
        if (prevRep == 0)
        {
            out << "***FirstRep error (clear) at " << std::hex << ip << " prevRep " << prevRep << std::dec << " count " << repCount << std::endl;
        }
    }
...
static VOID logMemoryAddress(UINT32 op, BOOL first, ADDRINT baseEA, ADDRINT count, UINT32 size, UINT32 eflags, ADDRINT tag)
{
    ...
    if ((memoryOpsLogged < memoryOpsToLog) && count != 0)
    {
        ...
        out << std::dec << std::setw(20) << size * count << std::endl;
...
static VOID insertRepExecutionCountInstrumentation(INS ins, UINT32 opIdx)
{
    if (takesConditionalRep(opIdx))
    {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)addCount, IARG_INST_PTR, IARG_UINT32, opIdx, IARG_FIRST_REP_ITERATION,
    ...
    else
    {
        ...
        INS_InsertThenCall(ins, IPOINT_BEFORE, (AFUNPTR)addCount, IARG_INST_PTR, IARG_UINT32, opIdx, IARG_UINT32, 1,
                           IARG_REG_VALUE, INS_RepCountRegister(ins), IARG_END);
    }
}
