...
KNOB< std::string > KnobOutput(KNOB_MODE_WRITEONCE, "pintool", "o", "ea_verifier_bt.out", "Name for log file");
...
static VOID ProcessAddress(ADDRINT name, UINT32 size, ADDRINT ea, INT32 bitIndex)
{
    ...
    if (baseAddr + byteOffset(size, bitIndex) != ea)
    {
        ...
        out << " Bit offset " << bitIndex << " Access size " << size << std::endl;
    }
}
...
static VOID Instrument(INS ins)
{
    ...
    INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(ProcessAddress), IARG_ADDRINT, ADDRINT(formatInstruction(ins)),
...
{
    out << "Target exited with code : " << code << std::endl;
    out << errors << " BTx addressing errors in " << tests << " tests" << std::endl;
    out.close();
    if (tests == 0)
        exit(-1);
    else
