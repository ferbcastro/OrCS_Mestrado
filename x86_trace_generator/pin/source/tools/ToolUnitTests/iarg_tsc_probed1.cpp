...
VOID AtRtn(VOID* name, ADDRINT arg1, UINT64 tsc)
{
    *OutFile << reinterpret_cast< CHAR* >(name) << "(" << arg1 << ")"
             << "\tTime Stamp Counter is: " << std::hex << tsc << std::dec << std::endl;
}
...
VOID Image(IMG img, VOID* v)
{
    ...
    if (RTN_Valid(rtn))
    {
        if (RTN_InsertCallProbed(rtn, IPOINT_BEFORE, AFUNPTR(AtRtn), IARG_PTR, RTN_Name(rtn).c_str(),
                                 IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_TSC, IARG_END))
        {
            ASSERTX(RTN_IsSafeForProbedInsertion(rtn));
        }
        else
        {
            *OutFile << "It is not safe to insert a call before " << RTN_Name(rtn) << " in " << IMG_Name(img) << std::endl;
            return;
        }
    }
    else
    {
        *OutFile << FACTORIAL_NAME << " wasn't found." << std::endl;
    }
}
