...
int Boo(CONTEXT* ctxt, AFUNPTR pf_Blue, int one, int two, int three, int four, int five, int six, int seven, int eight, int nine,
        int zero)
{
    std::cout << "Inside Boo(" << one << ", " << two << ", " << three << ", " << four << ", " << five << ", " << six << ", " << seven
         << ", " << eight << ", " << nine << ", " << zero << ")" << std::endl;

    int ret = 0;

    std::cout << "Jitting Blue10() with ten arguments and one return value." << std::endl;

    PIN_CallApplicationFunction(ctxt, PIN_ThreadId(), CALLINGSTD_REGPARMS, pf_Blue, NULL, PIN_PARG(int), &ret, PIN_PARG(int), one,
    ...
    std::cout << "Returned from Blue(); ret = " << ret << std::endl;
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    {
        PROTO proto = PROTO_Allocate(PIN_PARG(int), CALLINGSTD_REGPARMS, "FastBar10", PIN_PARG(int), PIN_PARG(int), PIN_PARG(int),
                                     PIN_PARG(int), PIN_PARG(int), PIN_PARG(int), PIN_PARG(int), PIN_PARG(int), PIN_PARG(int),
                                     PIN_PARG(int), PIN_PARG_END());

        VOID* pf_Blue;
        RTN rtn1 = RTN_FindByName(img, "FastBlue10");
        if (RTN_Valid(rtn1))
            pf_Blue = reinterpret_cast< VOID* >(RTN_Address(rtn1));
        else
        {
            std::cout << "FastBlue10 cannot be found." << std::endl;
            exit(1);
        }

        RTN rtn = RTN_FindByName(img, "FastBar10");
        if (RTN_Valid(rtn))
        {
