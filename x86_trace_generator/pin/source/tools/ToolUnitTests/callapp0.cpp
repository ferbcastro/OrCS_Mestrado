...
{
    std::cout << "Jitting Blue() with no arguments; application address: " << std::hex << (unsigned long)pf_Blue << std::dec << std::endl;

    PIN_CallApplicationFunction(ctxt, PIN_ThreadId(), CALLINGSTD_DEFAULT, pf_Blue, NULL, PIN_PARG_END());

    std::cout << "Returned from Blue(); Native execution. " << std::endl;
}
