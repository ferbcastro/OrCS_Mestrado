...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "line_tool.out",
        ...
        for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
        {
            for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
            {
                if (PIN_UndecorateSymbolName(RTN_Name(rtn), UNDECORATION_NAME_ONLY) == MAINNAME)
                {
                    ...
                    if (filePath != "")
                    {
                        std::string::size_type index = filePath.find("hello");
                        if (index != std::string::npos)
                        {
                            std::string file = filePath.substr(index);
                            trace << "File " << file << " line " << line << std::endl;
                        }
                    }
