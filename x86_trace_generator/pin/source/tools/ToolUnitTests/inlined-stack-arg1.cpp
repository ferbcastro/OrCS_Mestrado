...
FILE* log_inl;
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "inlined-stack-arg1.out", "output file");
...
static void print_hex_line(char* buf, unsigned int buf_size, const UINT8* array, const int length)
{
    ...
    for (i = 0; i < n; i++)
    {
        // This assert checks the index that accessed in 'buf' and prevents 'stringop-overflow'
        // warning which warn on std::string manipulation that might overflow the destination buffer
...
static std::string disassemble(UINT64 start, UINT64 stop)
{
    ...
    /*while( pc < stop )*/ {
        ...
        if (okay)
        {
            ...
            int dis_okay = xed_format_context(syntax, &xedd, buffer, BUFFER_SIZE, pc, 0, 0);
            if (dis_okay)
                os << buffer << std::endl;
            else
                           ...
                           INT8 readValue4, INT16 readValue5, INT32 readValue6)
{
    ...
    if (hadError)
    {
        ...
        std::string s = disassemble((pcval), (pcval) + 15);
        fprintf(log_inl, "    %s\n", s.c_str());
        exit(-1);
    }
    hadError = FALSE;
...
INT32 Usage()
{
    std::cerr << "This tests if the stack arguments are passed correctly by an inlined analysis function"
                 "\n";
...
int main(int argc, char* argv[])
{
    ...
    std::string logfile = KnobOutputFile.Value();
