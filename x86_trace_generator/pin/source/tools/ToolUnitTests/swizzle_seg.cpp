...
{
    RTN rtn = INS_Rtn(ins);
    if (RTN_Valid(rtn) && ((PIN_UndecorateSymbolName(RTN_Name(rtn), UNDECORATION_NAME_ONLY) == "SegAccessRtn") ||
                           PIN_UndecorateSymbolName(RTN_Name(rtn), UNDECORATION_NAME_ONLY) == "SegAccessStrRtn"))
    {
        ...
        for (UINT32 memopIdx = 0; memopIdx < INS_MemoryOperandCount(ins); memopIdx++)
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(ProcessAddress), IARG_REG_VALUE, baseReg, IARG_MEMORYOP_EA, memopIdx,
