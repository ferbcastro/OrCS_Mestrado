...
static KNOB< std::string > KnobOutput(KNOB_MODE_WRITEONCE, "pintool", "o", "callapp14.out", "output file");
...
int MyReplacementFunction(CONTEXT* ctxt, AFUNPTR origPtr, int one, int two)
{
    ...
    out << " MyReplacementFunction: Returned from Replaced Function res = " << res << std::endl;
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if (RTN_Valid(rtn))
    {
        out << " RTN_ReplaceSignature " << RTN_Name(rtn) << " in " << IMG_Name(img) << " at address " << hexstr(RTN_Address(rtn))
            << " with MyReplacementFunction" << std::endl;
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    if (!replacementDone)
    {
        out << "***Error !replacementDone" << std::endl;
        hadError = TRUE;
    }
    if (!functionToBeReplacedInstrumented)
    {
        out << "***Error !functionToBeReplacedInstrumented" << std::endl;
        hadError = TRUE;
    }
    if (!functionCalledByFunctionToBeReplacedInstrumented)
    {
        out << "***Error !functionCalledByFunctionToBeReplacedInstrumented" << std::endl;
        hadError = TRUE;
    }
    if (!functionToBeReplacedInstrumentationCalled)
    {
        out << "***Error !functionToBeReplacedInstrumentationCalled" << std::endl;
        hadError = TRUE;
    }
    if (!functionCalledByFunctionToBeReplacedInstrumentationCalled)
    {
        out << "***Error !functionCalledByFunctionToBeReplacedInstrumentationCalled" << std::endl;
        hadError = TRUE;
    }
    if (hadError)
    {
        out << "***Error hadError" << std::endl;
        exit(-1);
    }
}
...
INT32 Usage()
{
    std::cerr << "Tool: callapp14" << std::endl;
