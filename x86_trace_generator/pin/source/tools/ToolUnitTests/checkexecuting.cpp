...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "checkexecuting.out", "Output file");
...
INT32 Usage()
{
    std::cerr << "This pin tool counts predicated instructions selected by the\n"
...
{
    out << "Total : " << total << " Executed : " << executed << std::endl;
    for (UINT32 i = 0; i < NumOps; i++)
    {
        ...
        out << opInfo[i].name << " :  " << counts->total << " " << counts->executed << std::endl;
        ...
        if (KnobVerbose)
        {
            out << "   via predicated       " << counts->viaPredicated << std::endl;
            out << "   via ifPredicated     " << counts->viaIfPredicated << std::endl;
            out << "   via thenPredicated   " << counts->viaThenPredicated << std::endl;
            out << "   via ifThenPredicated " << counts->viaIfThenPredicated << std::endl;
        }

        if (counts->executed != counts->viaPredicated)
        {
            out << "***Error : executed gave " << counts->executed << " predicated gave " << counts->viaPredicated << std::endl;
            exit(1);
        }

        if (counts->executed != counts->viaIfPredicated)
        {
            out << "***Error : executed gave " << counts->executed << " if predicated gave " << counts->viaIfPredicated << std::endl;
            exit(1);
        }
