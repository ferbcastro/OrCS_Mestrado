...
{
        std::cout << "internal_exception_handler_app"
         << "[" << functionTestName << "] Success" << std::endl
             << std::flush;
}

static void Abort(const std::string& msg, const std::string& functionTestName)
{
    std::cout << "internal_exception_handler_app"
         << "[" << functionTestName << "] Failure: " << msg << std::endl
         << std::flush;
    exit(1);
}
...
static bool CheckExceptionCode(DWORD exceptCode, DWORD expectedExceptCode)
{
    ...
    {
        std::cout << "Unexpected exception code " << std::hex << exceptCode << ". Should be " << std::hex << expectedExceptCode << std::endl << std::flush;
        return false;
    }
    return true;
}
