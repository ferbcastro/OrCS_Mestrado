...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "strace.out", "Output file");
...
VOID SysAfter(ADDRINT value, INT32 err, UINT32 gax)
{
    ...
    if (error == 0)
        trace << "Success: value=0x" << std::hex << value << ", errno=" << std::dec << err << std::endl;
    else
    {
        trace << "Failure " << error << ": value=0x" << std::hex << value << ", errno=" << std::dec << err;
        trace << ", gax=0x" << std::hex << gax << std::endl;
    }
...
VOID Instruction(INS ins, VOID* v)
{
    ...
    if (INS_IsSyscall(ins) && INS_IsValidForIpointAfter(ins))
    {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(SysBefore), IARG_INST_PTR, IARG_SYSCALL_NUMBER, IARG_SYSARG_VALUE, 0,
        ...
        INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(SysAfter), IARG_SYSRET_VALUE, IARG_SYSRET_ERRNO, IARG_REG_VALUE, REG_GAX,
                       IARG_END);
    }
}
