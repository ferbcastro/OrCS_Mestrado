...
KNOB< std::string > KnobOutput(KNOB_MODE_WRITEONCE, "pintool", "o", "operandtool.out", "Name for log file");
...
static std::string formatMask(UINT32 m)
{
    std::string r;
    BOOL first = TRUE;
    ...
    for (UINT32 j = 0; j < NumTestDescriptions; j++)
    {
        ...
        if (result != t->result)
        {
            ...
            out << std::setw(50) << std::left << INS_Disassemble(ins) << std::right << "Operand " << operand << " Expected '"
                << formatMask(t->result) << "' Got '" << formatMask(result) << "'" << std::endl;
        }
    }
}
