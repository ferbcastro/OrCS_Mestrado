...
void MyReplacement() { std::cout << "The newly created routine was replaced successfully" << std::endl; }

void MyInsertedCall() { std::cout << "The call was inserted successfully" << std::endl; }
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
    {
        if (SEC_Name(sec) == ".fTable")
        {
            ...
            if (RTN_Address(rtn) != proc1Addr)
            {
                ...
                if (!RTN_Valid(rtn))
                {
                    std::cout << "Can't create a routine " << std::endl;
                    exit(-1);
                }
            ...
            {
                std::cout << "Proc1: Existing routine has been found at the given address, 0x" << std::hex << proc1Addr
                     << ". The new routine will not be created" << std::endl;
                exit(-1);
            }
            ...
            if (RTN_Address(rtn) != proc2Addr)
            {
                ...
                if (PIN_IsProbeMode())
                {
                    if (RTN_InsertCallProbed(rtn, IPOINT_BEFORE, AFUNPTR(MyInsertedCall), IARG_PROTOTYPE, proto, IARG_END))
                    {
                ...
                {
                    RTN_Open(rtn);
                    RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(MyInsertedCall), IARG_PROTOTYPE, proto, IARG_END);
                    RTN_Close(rtn);
                }
            }
            else
            {
                std::cout << "Proc2: Existing routine has been found at the given address, 0x" << std::hex << proc2Addr
                     << ". The new routine will not be created" << std::endl;
                exit(-1);
            }
    ...
    if (found)
    {
        ...
        {
            std::cout << "The newly created routine is not correctly inserted into the image." << std::endl;
            exit(-1);
        }
    }
}
...
int main(INT32 argc, CHAR** argv)
{
    ...
    if (PIN_IsProbeMode())
    {
        std::cout << "Testing the Probe mode." << std::endl;
        PIN_StartProgramProbed();
    }
    else
    {
        std::cout << "Testing the JIT mode." << std::endl;
        PIN_StartProgram();
    }
