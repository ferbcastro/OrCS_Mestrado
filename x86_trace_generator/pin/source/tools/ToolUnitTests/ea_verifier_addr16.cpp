...
inline ADDRINT InitializeThreadData() { return reinterpret_cast< ADDRINT >(WINDOWS::NtCurrentTeb()); }
...
{
    ADDRINT threadTeb                                             = 0;
    int numEffectiveAddressesAnalyzed[MAX_ADDRESSING_TYPES]       = {0};
    int numNonSegEffectiveAddressesAnalyzed[MAX_ADDRESSING_TYPES] = {0};
} THREAD_DATA;
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    {
        printf("Had ERRORS - search above for std::string ERROR\n");
    }
    fflush(trace);
}
...
VOID ThreadStart(THREADID threadid, CONTEXT* ctxt, INT32 flags, VOID* v)
{
    ...
    {
        // I assume this is called only for the main thread
...
VOID InstrumentRtn(RTN rtn, VOID*)
{
    ...
    {
        // This is only called application secondary threads that starts running one of these functions.
        // Will not be called for Windows automatic generated threads.
        RTN_Open(rtn);
        RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(AppThreadStart), IARG_THREAD_ID, IARG_CONTEXT, IARG_END);
        RTN_Close(rtn);
    }
}
