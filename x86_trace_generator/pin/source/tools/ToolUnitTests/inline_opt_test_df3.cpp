...
VOID CheckCopyAndReset(unsigned char* src, unsigned char* dst)
{
    for (int i = 0; i < 128; i++)
    {
        if (src[i] != i)
        {
            std::cout << "***Error in copy of src to dst, src changed" << std::endl;
            exit(-1);
        }
        if (dst[i] != i)
        {
            std::cout << "***Error in tool copy of src to dst, unexpected value in dst" << std::endl;
            exit(-1);
        }
    }
    for (int i = 0; i < 128; i++)
    ...
    for (int i = 0; i < 128; i++)
    {
        if (src[i] != i)
        {
            std::cout << "***Error in tool re-initialization of src" << std::endl;
            exit(-1);
        }
        if (dst[i] != ((i + 1) & 0xff))
        {
            std::cout << "***Error in tool re-initialization of dst" << std::endl;
            exit(-1);
        }
    }
}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < 128; i++)
    {
        if (src[i] != i)
        {
            std::cout << "***Error in tool initialization of src" << std::endl;
            exit(-1);
        }
        if (dst[i] != ((i + 1) & 0xff))
        {
            std::cout << "***Error in tool initialization of dst" << std::endl;
            exit(-1);
        }
    }
