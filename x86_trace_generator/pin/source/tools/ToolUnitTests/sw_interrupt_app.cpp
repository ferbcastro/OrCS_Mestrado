...
template< typename FUNC > bool ExecuteSafe(FUNC fp, EXCEPTION_RECORD* pExceptRecord, CONTEXT* pExceptContext)
{
    ...
    __except (SafeExceptionFilter(GetExceptionInformation(), pExceptRecord, pExceptContext))
    {
        std::cerr << "Exception handler: "
             << "Exception code " << std::hex << pExceptRecord->ExceptionCode << "."
             << "Exception address " << std::hex << (ULONG_PTR)(pExceptRecord->ExceptionAddress) << "."
             << "Context IP " << std::hex << GetIp(pExceptContext) << "." << std::endl;
        return true;
    }
}
