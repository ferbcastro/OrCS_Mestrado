...
VOID BeforeSyscall(ADDRINT ip, ADDRINT num, ADDRINT arg0, ADDRINT arg1, ADDRINT arg2, ADDRINT arg3, ADDRINT arg4, ADDRINT arg5)
{
    if (inMy_AllocateVirtualMemory)
    {
        ...
        std::cout << "NtAllocateVirtualMemory: ";
        std::cout << " size = " << std::dec << *((size_t*)arg3);
        std::cout << " allocation type = " << std::hex << (unsigned long)arg4;
        std::cout << " protect = " << std::hex << (unsigned long)arg5 << std::endl;

        if (pfnMy_SetNtAllocateNumber)
        {
            pfnMy_SetNtAllocateNumber((UINT32)num);
            std::cout << "Set NtAllocateVirtualMemory system call number" << std::endl;
...
VOID Instruction(INS ins, VOID* v)
{
    if (INS_IsSyscall(ins))
    {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(BeforeSyscall), IARG_INST_PTR, IARG_SYSCALL_NUMBER, IARG_SYSARG_VALUE, 0,
    ...
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(BeforeMyAlloc), IARG_END);
    }
}
