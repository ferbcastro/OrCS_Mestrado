...
// Printing utilities
//==========================================================================
std::string UnitTestName("internal_exception_handler_app_lin");

static void StartFunctionTest(const std::string& functionTestName)
{
    std::cerr << UnitTestName << " [ " << functionTestName << " ] Start" << std::endl << std::flush;
}

static void EndFunctionTest(const std::string& functionTestName)
{
...
static bool CheckExceptionCode(unsigned int exceptCode, unsigned int expectedExceptCode)
{
    ...
    {
        std::cerr << "Unexpected exception code " << std::hex << exceptCode << ". Should be " << std::hex << expectedExceptCode << std::endl;
        return false;
    }
    return true;
}
