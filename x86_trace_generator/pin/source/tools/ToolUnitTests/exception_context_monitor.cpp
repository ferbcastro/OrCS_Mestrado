...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "exception_context_monitor_tool.out",
                                   "specify file name");
...
static void OnException(THREADID threadIndex, CONTEXT_CHANGE_REASON reason, const CONTEXT* ctxtFrom, CONTEXT* ctxtTo, INT32 info,
                        VOID* v)
{
    /* Some newer versions of Windows: Windows 10 2019 server, Windows 11... 
       generate an early callback with
       a NULL ctxtFrom and it does not have anything to do with our
       application NULL pointer access */
    // check if this is an Exception from our app code
    if (reason == CONTEXT_CHANGE_REASON_EXCEPTION)
    {
        ASSERTX(ctxtFrom);
        ADDRINT exceptionAddr = PIN_GetContextReg(ctxtFrom, REG_INST_PTR);
        *logFile << "got exception at address " << hexstr(exceptionAddr) << std::endl;

        if (exceptionAddr < appMainStart || exceptionAddr > appMainEnd) return;

        *logFile << "handling exception" << std::endl;
        hadException      = TRUE;
        static bool first = true;

        if (first)
        {
            *logFile << "first time" << std::endl;
            first = false;
        }
        else
        {
            if (KnobCheckFp)
            {
                *logFile << "tool: checking FP" << std::endl;
    ...
    {
        *logFile << "Received a context change callback with reason: " << hexstr(reason) << std::endl;
    }
}
...
{
    *logFile << "img load " << IMG_Name(img) << std::endl;
    if (IMG_IsMainExecutable(img))
    {
        ...
        *logFile << "main exec range " << hexstr(appMainStart) << "-" << hexstr(appMainEnd) << std::endl;
    }
}
