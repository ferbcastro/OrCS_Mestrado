...
static KNOB< std::string > KnobOutput(KNOB_MODE_WRITEONCE, "pintool", "o", "callapp_nested_tool.out", "output file");
...
VOID ImageLoad(IMG img, VOID* v)
{
    if (IMG_IsMainExecutable(img))
    {
        ...
        if (RTN_Valid(rtn))
        {
            ...
            out << "Replacing " << RTN_Name(rtn) << " in " << IMG_Name(img) << std::endl;
        ...
        {
            out << "Original cannot be found." << std::endl;
            exit(1);
        }
        ...
        {
            out << "Original2 cannot be found." << std::endl;
            exit(1);
        }
...
INT32 Usage()
{
    std::cerr << "Nested application call from tool." << std::endl;
