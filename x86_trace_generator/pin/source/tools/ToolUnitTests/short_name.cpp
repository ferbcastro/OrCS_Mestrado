...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "short_name.outfile", "specify profile file name");

/* ===================================================================== */

INT32 Usage()
{
    std::cerr << "This pin tool prints the shortest name for each address\n"
            "\n";
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
    {
        for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
        {
            ...
            const std::string& rtnName = RTN_FindNameByAddress(rtnAddr);
