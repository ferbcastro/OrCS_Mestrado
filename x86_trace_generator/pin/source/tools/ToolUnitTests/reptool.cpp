...
KNOB< std::string > KnobOutput(KNOB_MODE_WRITEONCE, "pintool", "o", "reptool.out", "output file");

FILE* out    = 0;
UINT8 oldVal = 0;
UINT8* ea;

// Change the first byte of std::string
...
// Restore first byte of std::string
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    if (PIN_UndecorateSymbolName(RTN_Name(rtn), UNDECORATION_NAME_ONLY) != "main") return;
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
        {
            ...
            if (INS_IsMemoryRead(ins))
            {
                ...
                INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)RecordMemory, IARG_INST_PTR, IARG_REG_VALUE,
                                         LEVEL_BASE::REG_EDX, IARG_PTR, "read1", IARG_MEMORYREAD_EA, IARG_MEMORYREAD_SIZE,
                                         IARG_END);
            ...
            {
                INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)RecordMemory, IARG_INST_PTR, IARG_REG_VALUE,
                                         LEVEL_BASE::REG_EDX, IARG_PTR, "read2", IARG_MEMORYREAD2_EA, IARG_MEMORYREAD_SIZE,
            ...
            if (INS_IsMemoryWrite(ins))
            {
                ...
                INS_InsertPredicatedCall(ins, IPOINT_AFTER, (AFUNPTR)RecordMemory, IARG_INST_PTR, IARG_REG_VALUE,
                                         LEVEL_BASE::REG_EDX, IARG_PTR, "write", IARG_ADDRINT, ADDRINT(0), IARG_MEMORYWRITE_SIZE,
                                         IARG_END);
            }
        }
    }
