...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "check_fpstate.out", "specify output file name");
...
VOID CompreContext(CONTEXT* c1, CONTEXT* c2)
{
    ...
    for (unsigned int i = 0; i < sizeof(fields) / sizeof(fields[0]); i++)
    {
        ...
        if (memcmp(v1, v2, size) == 0)
            ...
            out << "Mismatch ";
        out << fields[i].name << " " << Val2Str(v1, size) << " " << Val2Str(v2, size) << std::endl;
