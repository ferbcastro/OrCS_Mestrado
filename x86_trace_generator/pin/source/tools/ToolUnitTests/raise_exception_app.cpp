...
// Printing utilities
//==========================================================================
std::string UnitTestName("raise_exception");
...
static bool CheckExceptionCode(EXCEPTION_RECORD* pExceptRecord, unsigned exceptCode)
{
    ...
    {
        std::cerr << "Unexpected exception code " << std::hex << pExceptRecord->ExceptionCode << ". Should be " << std::hex << exceptCode
             << std::endl;
        return false;
    }
    return true;
                               ...
                               void* accessAddr)
{
    ...
    {
        std::cerr << "Unexpected exception address " << std::hex << (ULONG_PTR)(pExceptRecord->ExceptionAddress) << ". Should be " << std::hex
             << (ULONG_PTR)exceptAddr << std::endl;
        return false;
    }
    ...
    if ((pExceptRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION) && (pExceptRecord->NumberParameters >= 2))
    {
        ...
        {
            std::cerr << "Unexpected access type " << std::hex << pExceptRecord->ExceptionInformation[0] << ". Should be " << std::hex
                 << accessType << std::endl;
            return false;
        }
        if (pExceptRecord->ExceptionInformation[1] != (ULONG_PTR)accessAddr)
        {
            std::cerr << "Unexpected address of the faulty access " << std::hex << pExceptRecord->ExceptionInformation[1] << ". Should be "
                 << std::hex << (ULONG_PTR)accessAddr << std::endl;
            return false;
        }
    }
    return true;
