...
// Printing utilities
//==========================================================================
std::string UnitTestName("suspend_context_win");
std::string FunctionTestName;

static void StartFunctionTest(const std::string& functionTestName)
{
    if (FunctionTestName != "")
    {
        std::cerr << UnitTestName << "[" << FunctionTestName << "] Success" << std::endl << std::flush;
    }
    FunctionTestName = functionTestName;
}

static void SkipFunctionTest(const std::string& msg)
{
    std::cerr << UnitTestName << "[" << FunctionTestName << "] Skip: " << msg << std::endl << std::flush;
    FunctionTestName = "";
}
...
static void ExitUnitTest()
{
    ...
    std::cerr << UnitTestName << " : Completed successfully" << std::endl << std::flush;
    exit(0);
}

static void Abort(const std::string& msg)
{
    std::cerr << UnitTestName << "[" << FunctionTestName << "] Failure: " << msg << std::endl << std::flush;
    exit(1);
}
...
/*!
 * Suspend specified threads and update their contexts.
 * Exchange full context is bad idea that will eventually cause malfunction (proven).
 * Instead we perform context change operation without changing actual content of contexts.
 */
void ExchangeContexts(HANDLE hThread0, HANDLE hThread1)
{
    ...
    bStatus = SetThreadContext(hThread0, &ctxt0);
    if (bStatus == 0)
    {
        Abort("SetThreadContext(0) failed");
    }
    bStatus = SetThreadContext(hThread1, &ctxt1);
    if (bStatus == 0)
    {
...
int main(int argc, char* argv[])
{
...

    {
        ...
        // Both threads passed their IDs
        tid[0] = targ[0];
        tid[1] = targ[1];

        // Suspend threads and update their contexts
        ...
        // Suspend threads and update their contexts back. This is needed
        ...
        // Check to see that <targ> and <tid> arrays match
        if ((targ[0] != tid[0]) || (targ[1] != tid[1]))
        {
