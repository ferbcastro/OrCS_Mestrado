 ...
 */
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "memtrace.out", "output file");
    ...
    for (int cmd = 0; cmd < traceHeader->NumCmds(); cmd++)
    {
        switch (traceHeader->CmdType(cmd))
        {
            case TCMD_IMMEDIATE:
                ...
                {
                    ofile << traceHeader->Ref(ref)->IP() << " " << Immediate(logCursor) << std::endl;
                }
                ref++;
                break;
...
{
    for (std::vector< CALL >::iterator c = _calls.begin(); c != _calls.end(); c++)
    {
...
#define INSERT_IF
    ...
    TRACE_InsertThenCall(trace, IPOINT_BEFORE, AFUNPTR(MLOG::TraceAllocThen), IARG_FAST_ANALYSIS_CALL, IARG_REG_VALUE,
                         ...
                         IARG_END);
    TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(MLOG::RecordTraceBegin), IARG_FAST_ANALYSIS_CALL, IARG_REG_VALUE,
                     scratch_reg0, IARG_PTR, theader, IARG_UINT32, theader->LogBytes(), IARG_RETURN_REGS, scratch_reg0, IARG_END);
#else
    TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(MLOG::BeginTrace), IARG_FAST_ANALYSIS_CALL, IARG_REG_VALUE, scratch_reg0,
