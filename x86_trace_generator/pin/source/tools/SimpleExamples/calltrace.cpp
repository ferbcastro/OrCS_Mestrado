...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "calltrace.out", "specify trace file name");
...
INT32 Usage()
{
    std::cerr << "This tool produces a call trace." << std::endl << std::endl;
...
VOID do_call_args(const std::string* s, ADDRINT arg0) { TraceFile << *s << "(" << arg0 << ",...)" << std::endl; }
...
VOID do_call_args_indirect(ADDRINT target, BOOL taken, ADDRINT arg0)
{
    ...
    const std::string* s = Target2String(target);
    do_call_args(s, arg0);
...
VOID do_call(const std::string* s) { TraceFile << *s << std::endl; }
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        if (INS_IsCall(tail))
        {
            ...
            else
            {
                if (print_args)
                {
                    INS_InsertCall(tail, IPOINT_BEFORE, MAKE_AFUNPTR(do_call_args_indirect), IARG_BRANCH_TARGET_ADDR,
                                   IARG_BRANCH_TAKEN, IARG_FUNCARG_CALLSITE_VALUE, 0, IARG_END);
                }
                else
                {
                    INS_InsertCall(tail, IPOINT_BEFORE, MAKE_AFUNPTR(do_call_indirect), IARG_BRANCH_TARGET_ADDR,
