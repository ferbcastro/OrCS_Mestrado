...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "ldstmix.out", "specify profile file name");
...
INT32 Usage()
{
    std::cerr << "This pin tool computes a dynamic register/memory pattern mix profile\n"
            "\n";
...
INT32 RecordRegisters(BBL bbl, UINT16* stats, UINT32 max_stats)
{
    ...
    for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
    {
        if (count >= max_stats)
        {
            std::cerr << "Too many stats in this block" << std::endl;
            exit(1);
        }
...
#define MAX_STATS_PER_BLOCK (128 * 1024)
        ...
        INS_InsertCall(BBL_InsHead(bbl), IPOINT_BEFORE, MAKE_AFUNPTR(docount), IARG_PTR, bblstats->_counter, IARG_THREAD_ID,
                       IARG_END);
...
/* ===================================================================== */
VOID EmitPerThreadStats(std::ostream* out)
{
    *out << std::setprecision(4) << std::showpoint;
