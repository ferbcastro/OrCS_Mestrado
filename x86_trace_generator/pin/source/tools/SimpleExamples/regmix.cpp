...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "regmix.out", "specify profile file name");
...
INT32 Usage()
{
    std::cerr << "This pin tool computes a dynamic register usage mix profile\n"
            "\n";
    ...
    *out << "#\n"
    ...
    for (UINT32 i = 0; i < MAX_REG; i++)
    {
        ...
        *out << decstr(i, 3) << " " << ljstr(REG_StringShort(REG(i)), 15) << decstr(GlobalStats.reg_r[i], 12)
             << decstr(GlobalStats.reg_w[i], 12) << std::endl;
    }
