...
std::string instrumented_routines[] = {BSR_RTN_NAME, BSF_RTN_NAME};
...
INT32 Usage()
{
    std::cerr << "This tool prints information on the destination's validity of the BSR/BSF instructions." << std::endl;
...
VOID examine_z_flag(char* mnemonics, ADDRINT eflags)
{
    *outFile << mnemonics << " destination operand is "
             << ((eflags & EFLAGS_ZF) ? "undefined (source equals zero)." : "valid (source is not zero).") << std::endl
             << std::flush;
}
