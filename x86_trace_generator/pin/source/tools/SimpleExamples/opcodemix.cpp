...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "opcodemix.out", "specify profile file name");
...
INT32 Usage()
{
    std::cerr << "This pin tool computes a static and dynamic opcode mix profile\n"
            "\n";
...
std::atomic_uint32_t enabled {0};

static VOID ThreadStart(THREADID threadId, CONTEXT* ctx, INT32 flags, VOID* v) { enabled = 1; }

static VOID ThreadFini(THREADID threadId, const CONTEXT* ctx, INT32 code, VOID* v) { enabled = 0; }
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        *curr++ = 0;
        ...
        INS_InsertCall(head, IPOINT_BEFORE, MAKE_AFUNPTR(docount), IARG_FAST_ANALYSIS_CALL, IARG_PTR, &(bblstats->_counter),
                       IARG_END);
...
/* ===================================================================== */
VOID DumpStats(std::ofstream& out, STATS& stats, BOOL predicated_true, const std::string& title)
{
    out << "#\n"
           "# "
        << title
    ...
    for (UINT32 i = 0; i < MAX_INDEX; i++)
    {
        ...
        out << std::setw(4) << i << " " << ljstr(IndexToOpcodeString(i), 15) << " " << std::setw(16) << stats.unpredicated[i] << " "
            << std::setw(16) << stats.predicated[i];
        if (predicated_true) out << " " << std::setw(16) << stats.predicated_true[i];
        out << std::endl;
    }
}
