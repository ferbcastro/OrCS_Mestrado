...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "coco.out", "specify profile file name");
...
INT32 Usage()
{
    std::cerr << "This pin tool computes code coverage\n"
            "\n";
...
std::list< const BBLSTATS* > statsList;
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        INS_InsertCall(BBL_InsHead(bbl), IPOINT_BEFORE, MAKE_AFUNPTR(docount), IARG_PTR, &(bblstats->_executed), IARG_END);
...
VOID PrintUntouchedRanges(SEC sec)
{
    ...
    std::vector< bool > touched(SEC_Size(sec));

    // Put the rtn's that are touched in a set
    std::set< RTN > rtnSet;

    // Mark the ranges for bbls that have been executed
    for (std::list< const BBLSTATS* >::const_iterator bi = statsList.begin(); bi != statsList.end(); bi++)
    {
    ...
    out << "    Routines that are not executed" << std::endl;
    for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
    {
        if (rtnSet.find(rtn) == rtnSet.end())
        {
            out << "      " << RTN_Name(rtn) << std::endl;
        }
    }

    // Print the ranges of untouched addresses
    out << "    Code ranges that are not executed" << std::endl;
    std::string rtnName = "";
    for (UINT32 i = 0; i < SEC_Size(sec);)
    {
        ...
        std::string imgName = (IMG_Valid(img) ? IMG_Name(img) : "InvalidImg");
        RTN rtn        = RTN_FindByAddress(startAddress);
        std::string newName = (RTN_Valid(rtn) ? RTN_Name(rtn) : "InvalidRtn");
        if (rtnName != newName)
        {
            out << " Image: " << imgName << "  Rtn: " << newName << std::endl;
            rtnName = newName;
        }

        out << "        " << SEC_Address(sec) + start << ":" << SEC_Address(sec) + i - 1 << std::endl;
    }
}
...
VOID ImageUnload(IMG img, VOID* v)
{
    ...
    for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
    {
        ...
        out << "  Section: " << SEC_Name(sec) << std::endl;
