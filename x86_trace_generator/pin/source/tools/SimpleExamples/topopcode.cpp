...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "opcodetop.out", "specify output file name");
...
INT32 Usage()
{
    std::cerr << "This pin tool provides a real time  opcode mix profile\n"
            "\n";
...
static std::vector< BBLSTATS* > statsList;
std::mutex statsListMtx; // Protect access to statsList
...
VOID DumpHistogram(std::ostream& out)
{
    ...
    out << "\033[2J";
    out << "\033[44m";
    out << ljstr("OPCODE", 15) << " " << std::setw(16) << "COUNT";
    out << "\033[0m";
    out << std::endl;

    statsListMtx.lock(); // Protect access to statsList
    for (std::vector< BBLSTATS* >::iterator bi = statsList.begin(); bi != statsList.end(); bi++)
    {
        BBLSTATS* b = (*bi);
    ...
    {
        UINT32 i = bi->first;
        //        out << std::setw(4) << i << " " <<  ljstr(IndexToOpcodeString(i),15) << " " <<
        out << ljstr(IndexToOpcodeString(i), 15) << " " << std::setw(16) << bi->second << std::endl;
        lines++;
...
VOID Trace(TRACE trace, VOID* v)
{
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        *curr++ = 0;
        ...
        ASSERTX(nullptr != bblstats);
        INS_InsertCall(head, IPOINT_BEFORE, MAKE_AFUNPTR(docount), IARG_PTR, &(bblstats->_counter), IARG_END);

        std::lock_guard< std::mutex > lock(statsListMtx); // // Protect access to statsList (Scope lock)
