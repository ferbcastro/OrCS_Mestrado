...
thread_data_t* get_tls(THREADID threadid)
{
    ...
    if (tdata == NULL)
    {
        std::cerr << "PIN_GetThreadData(" << tls_key << "," << threadid << ") Failed" << std::endl;
        PIN_ExitProcess(1);
    }
    ...
    {
        std::cerr << "PIN_GetThreadData(" << tls_key << "," << threadid << ") returned the wrong thread data" << std::endl;
        PIN_ExitProcess(1);
    }
...
VOID TestIllegalTLSOPerations(THREADID threadid)
{
    ...
    {
        std::cerr << "PIN_GetThreadData(" << tls_key << "," << threadid << ") should have failed" << std::endl;
        PIN_ExitProcess(1);
    }
    // Create a key
...
{
    thread_data_t* tdata = get_tls(threadid);
    *OutFile << "Count[" << decstr(threadid) << "] = " << tdata->_count << std::endl;

    PIN_GetLock(&pinLock, threadid);
    threads_data.erase(threadid);
    PIN_ReleaseLock(&pinLock);
    delete tdata;
}
