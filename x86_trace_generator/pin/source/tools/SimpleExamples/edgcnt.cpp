...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "edgcnt.out", "specify trace file name");
...
static INT32 Usage()
{
    std::cerr << "This pin tool collects an edge profile for an application\n";
    std::cerr << "The edge profile is partial as it only considers control flow changes (taken\n";
    std::cerr << "branch edges, etc.). It is the left to the profile consumer to compute the missing counts.\n";
    std::cerr << "\n";

    std::cerr << "The pin tool *does* keep track of edges from indirect jumps within, out of, and into\n";
    std::cerr << "the application. Traps to the OS a recorded with a target of -1.\n";
...
std::string StringFromEtype(ETYPE etype)
{
    switch (etype)
    {
        case ETYPE_CALL:
...
typedef std::map< EDGE, COUNTER* > EDG_HASH_SET;
...
VOID Fini(int n, void* v)
{
    ...
    *out << "EDGs " << count << std::endl;
    *out << "# src          dst        type    count     next-ins\n";
    *out << "DATA:START" << std::endl;
    ...
    for (EDG_HASH_SET::const_iterator it = EdgeSet.begin(); it != EdgeSet.end(); it++)
    {
        ...
        *out << StringFromAddrint(tuple.first._src) << " " << StringFromAddrint(tuple.first._dst) << " "
             ...
             << StringFromAddrint(tuple.first._next_ins) << std::endl;
    }
