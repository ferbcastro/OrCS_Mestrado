...
INT32 Usage()
{
    std::cerr << "This tool prints information on immediate operands." << std::endl;
    std::cerr << KNOB_BASE::StringKnobSummary() << std::endl;
    return 1;
}

/*
 * A routine to report value (signed and std::hex-formatted) and width of an immediate operand.
 */
static void OnInstruction(ADDRINT unsigned_value, long signed_value, bool is_signed, INT32 length_bits)
{
    *outFile << "Contains an immediate value: "
             << "0x" << std::hex << std::setfill('0') << std::setw(length_bits / 4) << unsigned_value << std::setfill(' ') << std::setw(1) << std::dec;
    if (is_signed)
    {
        *outFile << " (signed: " << signed_value << "),";
    }
    *outFile << " operand is " << length_bits << " bits long." << std::endl;
}
...
VOID Instruction(INS ins, VOID* v)
{
    ...
    *outFile << "Querying instruction w/opcode: " << INS_Mnemonic(ins) << std::endl;
    // Go over operands
    ...
    {
        *outFile << "No immediate operands per this command" << std::endl;
    }
}
...
static VOID ImageLoad(IMG img, VOID* v)
{
    ...
    {
        *outFile << "In main application image" << std::endl;
        ...
        if (RTN_Valid(AsmRtn))
        {
            *outFile << "Function operImmCmds found" << std::endl;
            RTN_Open(AsmRtn);
            ...
            RTN_Close(AsmRtn);
            *outFile << "Done with function operImmCmds" << std::endl;
        }
        else
        {
            *outFile << "Function operImmCmds not found!" << std::endl;
        }
    }
}
