...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "funtop.out", "specify profile file name");
...
INT32 Usage()
{
    std::cerr << "This pin tool \"real time\" tool for showing the currently hostest routines\n"
            "\n";
...
std::string invalid = "invalid_rtn";

/* ===================================================================== */
const std::string* Target2String(ADDRINT target)
{
...
typedef std::map< ADDRINT, UINT64 > ADDR_CNT_MAP;
...
VOID DumpHistogram(std::ostream& out)
{
...
#if 0
        out << std::setw(18) << (void *)(bi->first) << " " <<
            std::setw(10) << bi->second <<
            "   " << Target2String(bi->first) << std::endl;
#endif
    }
    ...
    {
        out << std::setw(18) << (void*)(bi->first) << " " << std::setw(10) << bi->second << "   " << Target2String(bi->first) << std::endl;
        lines++;
    ...
    //out << "Total Functions: " << CountMap.size() << std::endl;
}
...
VOID Trace(TRACE trace, VOID* v)
{
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        if (INS_IsCall(tail))
        {
            INS_InsertCall(tail, IPOINT_BEFORE, MAKE_AFUNPTR(do_call_indirect), IARG_BRANCH_TARGET_ADDR, IARG_BRANCH_TAKEN,
                           IARG_END);
        }
        else
        {
