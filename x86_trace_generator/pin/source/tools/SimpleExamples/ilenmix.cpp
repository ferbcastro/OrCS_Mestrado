...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "ilenmix.out", "specify profile file name");
...
INT32 Usage()
{
    std::cerr << "This pin tool computes the static and dynamic instruction length mix profile\n"
            "\n";
...
VOID ComputeGlobalStats()
{
    ...
    for (std::list< const BBLSTATS* >::iterator bi = statsList.begin(); bi != statsList.end(); bi++)
    {
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        INS_InsertCall(BBL_InsHead(bbl), IPOINT_BEFORE, MAKE_AFUNPTR(docount), IARG_PTR, &(bblstats->_counter), IARG_END);
...
/* ===================================================================== */
VOID DumpStats(std::ofstream& out, STATS& stats, const std::string& title)
{
    out << "#\n"
           "# "
        << title
    ...
    for (UINT32 i = 0; i < MAX_INDEX; i++)
    {
        ...
        out << decstr(i, 3) << " " << ljstr(decstr(i, 3), 15) << decstr(stats.unpredicated[i], 12)
            << decstr(stats.predicated[i], 12) << decstr(stats.predicated_true[i], 12) << std::endl;
    ...
    out << "avg unpredicated: " << avg << std::endl;
}
