...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "regval.out", "specify output file name");
...
static void PrintRegisters(const CONTEXT* ctxt)
{
    ...
    for (int reg = (int)REG_GR_BASE; reg <= (int)REG_GR_LAST; ++reg)
    {
        ...
        OutFile << REG_StringShort((REG)reg) << ": 0x" << std::hex << val << std::endl;
    }
    for (int reg = (int)REG_ST_BASE; reg <= (int)REG_ST_LAST; ++reg)
    {
        ...
        PIN_GetContextRegval(ctxt, (REG)reg, reinterpret_cast< UINT8* >(&val));
        OutFile << REG_StringShort((REG)reg) << ": " << Val2Str(&val, stRegSize) << std::endl;
    }
}
