...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "inscount2_vregs.out", "specify output file name");
...
VOID Trace(TRACE trace, VOID* v)
{
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        BBL_InsertCall(bbl, IPOINT_ANYWHERE, MAKE_AFUNPTR(DoCount), IARG_FAST_ANALYSIS_CALL, IARG_ADDRINT, BBL_NumIns(bbl),
...
{
    OutFile << "Count= " << TotalCount << std::endl;
    OutFile.close();
}
...
INT32 Usage()
{
    std::cerr << "This Pintool counts the number of dynamic instructions executed" << std::endl;
