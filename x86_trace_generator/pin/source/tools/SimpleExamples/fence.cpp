...
class SANDBOX
{
  private:
    // types
    typedef std::map< const char*, const char* > AddrMap;
    ...
    {
        static std::string errorPrefix = "ERROR:\n";
    ...
    {
        static std::string warningPrefix = "WARNING:\n";
...
VOID SANDBOX::PrintMessage(std::string msg)
{
    ...
    std::string::size_type pos = msg.length() > 1 ? msg.length() - 2 : 0;

    // prefix every line
    for (pos = msg.find_last_of('\n', pos); pos != std::string::npos;
         pos = (pos == 0 ? std::string::npos : msg.find_last_of('\n', pos - 1)))
    {
        if (pos == msg.length() - 1)
        {
            msg += "Fence: \n";
