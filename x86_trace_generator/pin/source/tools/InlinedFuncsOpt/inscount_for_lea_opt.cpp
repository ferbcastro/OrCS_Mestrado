...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "inscount_for_lea_opt.out", "specify output file name");
...
VOID Fini(INT32 code, VOID* v)
{
    if (icount1 != icount2 || icount1 != icount3 || icount1 != icount4 || icount1 != icount5 || thenCount != 0)
    {
        ...
        if (thenCount != 0)
        {
            OutFile << "****ERROR thenCount was expected to be 0  not: " << thenCount << std::endl;
        }
        else
        {
            OutFile << "Count1 " << icount1 << std::endl;
            OutFile << "Count2 " << icount2 << std::endl;
            OutFile << "Count3 " << icount3 << std::endl;
            OutFile << "Count4 " << icount3 << std::endl;
            OutFile << "Count5 " << icount3 << std::endl;
            OutFile << "***ERROR - mismatch in icounts " << std::endl;
        }
        OutFile.close();
        exit(1);
    }
