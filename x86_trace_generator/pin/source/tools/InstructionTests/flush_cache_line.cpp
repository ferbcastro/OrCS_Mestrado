 ...
 */
VOID AbortProcess(const std::string& msg, unsigned long code)
{
...
VOID FlushInstruction(ADDRINT add, UINT32 size)
{
    ...
    if (FlushParm != add)
    {
        std::cout << std::hex << FlushParm << "    " << add << std::endl;
...
VOID ImageLoad(IMG img, VOID* v)
{
    if (IMG_IsMainExecutable(img))
    {
        ...
        {
            RTN_Open(rtn);
            RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(GetFlushParm), IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
            RTN_Close(rtn);
        }
    }
}
