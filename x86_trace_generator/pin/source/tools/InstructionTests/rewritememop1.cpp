...
KNOB< std::string > KnobOutput(KNOB_MODE_WRITEONCE, "pintool", "o", "rewritememop1.out", "Name for log file");
...
static char const* formatInstruction(INS ins)
{
    ...
    std::string formatted = hexstr(ip) + " " + INS_Disassemble(ins);
...
static VOID printInstruction(THREADID thread, ADDRINT disas)
{
    ...
    out << std::dec << seqNo << ":" << ((const char*)Addrint2VoidStar(disas)) << std::endl;
}
...
static VOID printRegisterDiffs(THREADID tid, CONTEXT* ctx)
{
    ...
    else
    {
        for (UINT32 i = 0; i < sizeof(checkedRegisters) / sizeof(checkedRegisters[0]); i++)
        {
            ...
            {
                out << std::dec << seqNo << ": " << checkedRegisters[i].name << " = " << std::hex << newValue << std::endl;
...
static VOID RewriteIns(INS ins)
{
    ...
    if (KnobTrace)
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(printRegisterDiffs), IARG_THREAD_ID,
                       (KnobUseIargConstContext) ? IARG_CONST_CONTEXT : IARG_CONTEXT, IARG_END);
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(printInstruction), IARG_THREAD_ID, IARG_ADDRINT, formatInstruction(ins),
                       IARG_END);
    }
    ...
    if (memops && doTranslate(ins))
    {
        ...
        for (int i = 0; i < memops; i++)
        {
            ...
            if (rewriteOp[i])
            {
                ...
                INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(ProcessAddress), IARG_UINT32, i, IARG_MEMORYOP_EA, i,
                               IARG_INST_PTR, IARG_RETURN_REGS, REG_INST_G0 + i, IARG_END);
            }
        }

        if (INS_IsValidForIpointTakenBranch(ins))
        { // Check that IPOINT_TAKEN_BRANCH instrumentation is called.
            INS_InsertCall(ins, IPOINT_TAKEN_BRANCH, MAKE_AFUNPTR(BranchTaken), IARG_INST_PTR, IARG_END);
        }
    }
}
...
{
    out << "Target exited with code : " << code << std::endl << std::endl;
    out << "Dynamically rewrote " << rewritten << " memory operands" << std::endl;
    out << "Branch taken " << afterBranch << std::endl;

    out << "Statically rewritten opcodes" << std::endl;
