...
INT32 Usage()
{
    std::cerr << "This tool checks GS/FS segment base register values are correct after they changed by the application.\n";
...
VOID handle_segment_register(UINT32 seg_reg, CONTEXT const* context, ADDRINT seg_base, std::string* disassm)
{
    ...
    if (seg_base != seg_base_from_context)
    {
        ...
        std::cerr << "[TOOL] Error: different segment base address values:" << std::endl;
        std::cerr << "[TOOL] Error: base address from IARG_CONST_CONTEXT = 0x" << std::hex << seg_base << std::endl;
        std::cerr << "[TOOL] Error: base address from IARG_REG_VALUE     = 0x" << std::hex << seg_base_from_context << std::endl;
        ASSERTX(FALSE);
    }
