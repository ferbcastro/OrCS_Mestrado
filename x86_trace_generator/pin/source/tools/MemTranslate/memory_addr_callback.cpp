...
VOID PIN_FAST_ANALYSIS_CALL readXmmMemoryFunc(ADDRINT memea_callback, UINT32 bytes, std::string* dis, ADDRINT ip)
{
    ...
    {
        OutFile << "XMM bytes error found: " << lastBytes << " and not 16 for " << *dis << std::endl;
        errors++;
    }
}

VOID PIN_FAST_ANALYSIS_CALL verifyPrefetchFunc(ADDRINT memea_callback, UINT32 bytes, std::string* dis, ADDRINT ip)
{
    // case of multithreading - we care just about the main thread
    if (PIN_ThreadId() != myThread) return;
    ...
    if (!lastIsPrefetch)
    {
        OutFile << "Prefetch flag not set: " << *dis << std::endl;
        errors++;
    }
}

VOID PIN_FAST_ANALYSIS_CALL verifyRmwFunc(ADDRINT memea_callback, UINT32 bytes, std::string* dis, ADDRINT ip)
{
    // case of multithreading - we care just about the main thread
    if (PIN_ThreadId() != myThread) return;
    ...
    if (!lastIsRmw)
    {
        OutFile << "RMW flag not set: " << *dis << std::endl;
        errors++;
    }
}

VOID PIN_FAST_ANALYSIS_CALL verifyAtomicFunc(ADDRINT memea_callback, UINT32 bytes, std::string* dis, ADDRINT ip)
{
    // case of multithreading - we care just about the main thread
    if (PIN_ThreadId() != myThread) return;
    ...
    if (!lastIsAtomic)
    {
        OutFile << "Atomic flag not set: " << *dis << std::endl;
        errors++;
    }
}

VOID PIN_FAST_ANALYSIS_CALL readMemoryFunc(ADDRINT memea_orig, ADDRINT memea_callback, THREADID threadIndex, std::string* dis,
                                           ADDRINT ip)
{
    // case of multithreading - we care just about the main thread
    if (PIN_ThreadId() != myThread) return;
    ...
    if (ip != lastIp)
    {
        OutFile << "read analysis missing: " << std::hex << ip << " " << *dis << std::endl;
        errors++;
    }

    if (memea_orig != lastReadAddr)
    {
        OutFile << "read orig address incorrect: " << std::hex << memea_orig << " " << *dis << std::endl;
        errors++;
    }

    if ((memea_callback & 0xf) != 0)
    {
        OutFile << "read address not 16 aligned: " << std::hex << memea_callback << " " << *dis << std::endl;
        errors++;
    }

    if ((memea_orig & mask) != memea_callback)
    {
        OutFile << "read addresses incorrect: " << std::hex << memea_orig << " " << memea_callback << " " << *dis << std::endl;
        errors++;
    }
...
VOID PIN_FAST_ANALYSIS_CALL read2MemoryFunc(ADDRINT memea_orig, ADDRINT memea_callback, ADDRINT memea2_orig,
                                            ADDRINT memea2_callback, THREADID threadIndex, std::string* dis, CONTEXT* ctxt,
                                            ADDRINT ip)
{
    // case of multithreading - we care just about the main thread
    if (PIN_ThreadId() != myThread) return;
    ...
    if ((memea_callback & 0xf) != 0)
    {
        OutFile << "read2 first address not 16 aligned: " << std::hex << memea_callback << " " << *dis << std::endl;
        errors++;
    }

    if ((memea_orig & mask) != memea_callback)
    {
        OutFile << "read2 first addresses incorrect: " << std::hex << memea_orig << " " << memea_callback << " " << *dis << std::endl;
        errors++;
    }

    if ((memea2_callback & 0xf) != 0)
    {
        OutFile << "read2 second address not 16 aligned: " << std::hex << memea2_callback << " " << *dis << std::endl;
        errors++;
    }

    if ((memea_orig & mask) != memea_callback)
    {
        OutFile << "read2 second addresses incorrect: " << std::hex << memea2_orig << " " << memea2_callback << " " << *dis
                << std::endl;
        errors++;
    }
                                            ...
                                            std::string* dis)
{
    ...
    {
        OutFile << "write orig address incorrect: " << std::hex << memea_orig << " " << ip << " " << *dis << std::endl;
        errors++;
    }

    if ((memea_callback & 0xf) != 0)
    {
        OutFile << "write address not 16 aligned: " << std::hex << memea_callback << " " << *dis << std::endl;
        errors++;
    }

    if ((memea_orig & mask) != memea_callback)
    {
        OutFile << "write addresses incorrect: " << std::hex << memea_orig << " " << memea_callback << " " << *dis << std::endl;
        errors++;
    }
...
VOID PIN_FAST_ANALYSIS_CALL opMemoryFunc(ADDRINT memea_orig, ADDRINT memea_callback, UINT32 bytes, ADDRINT ip, std::string* dis)
{
    // case of multithreading - we care just about the main thread
    if (PIN_ThreadId() != myThread) return;
    ...
    if (bytes != lastBytes)
    {
        OutFile << "op bytes error found: " << bytes << " and not " << lastBytes << " for " << *dis << std::endl;
        errors++;
    }

    if ((memea_callback & 0xf) != 0)
    {
        OutFile << "op address not 16 aligned: " << std::hex << memea_callback << " " << *dis << std::endl;
        errors++;
    }

    if ((memea_orig & mask) != memea_callback)
    {
        OutFile << "op addresses incorrect: " << std::hex << memea_orig << " " << memea_callback << " " << ip << " " << *dis
                << std::endl;
        errors++;
    }
...
ADDRINT PIN_FAST_ANALYSIS_CALL memoryCallback(PIN_MEM_TRANS_INFO* memTransInfo, VOID* v)
{
    ...
    ASSERT(memTransInfo->threadIndex == PIN_ThreadId(), "Thread id passed by memory callback is incorrect");

    if (memTransInfo->flags.bits.isFromPin || PIN_ThreadId() != myThread)
    {
        // PIN Internal memory dereference or irrelevant thread
    ...
    char buffer[4096];
    size_t sizein = memTransInfo->bytes;
    if (memTransInfo->bytes > sizeof(buffer)) sizein = sizeof(buffer);

    // Verify that we can call PIN API functions inside PIN tool
    size_t size = PIN_SafeCopy((void*)buffer, (void*)memTransInfo->addr, sizein);
    ...
    else
    {
        if (size != sizein)
        {
            OutFile << "SafeCopy read operation size missmatch, expected: " << sizein << " , read: " << size << std::endl;
            errors++;
        }
    ...
    ASSERT((ADDRINT)v == 0xa5a5a5a5, "Unexpected memory callback parameter");
...
{
    std::string* disptr = new std::string(INS_Disassemble(ins));
...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "memaddrcall.out", "specify output file name");
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    OutFile << "Count Mem Reads " << icountMemRead << std::endl;
    OutFile << "Count Mem Read2s " << icountMemRead2 << std::endl;
    OutFile << "Count Mem Writes " << icountMemWrite << std::endl;
    OutFile << "Count Mem Ops " << icountMemOp << std::endl;
    OutFile << "Count Mem callbacks " << icountMemCall << std::endl;
    OutFile << "Errors " << errors << std::endl;
    OutFile.close();

    // If we have errors then terminate abnormally
    if (errors)
    {
        std::cout << "Test memory_addr_callback is terminated cause found " << errors << " errors " << std::endl;
...
INT32 Usage()
{
    std::cerr << "This tool tests memory address translation callback" << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    {
        std::cout << "Test memory_addr_callback found PIN API callback not null before registration " << std::endl;
        PIN_ExitProcess(-1);
    }
    ...
    if (!PIN_GetMemoryAddressTransFunction())
    {
        std::cout << "Test memory_addr_callback found PIN API callback null after registration " << std::endl;
        PIN_ExitProcess(-1);
    }
