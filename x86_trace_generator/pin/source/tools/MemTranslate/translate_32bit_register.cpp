...
static int* replaceMemoryReadFunc(int* readAddr, void* pc)
{
    if (KnobVerbose)
    {
        std::cerr << pc << ") replaceMemoryReadFunc called with " << (void*)readAddr << std::endl;
    }
    if (*readAddr != 0xbadc0de) return readAddr;
    if (KnobVerbose)
    {
        std::cerr << pc << ") replacing " << (void*)readAddr << " to " << (void*)replacement_ptr << std::endl;
    }
...
VOID Instruction(INS ins, VOID* v)
{
        ...
        INS_MemoryDisplacement(ins) == 0)
    {
        if (KnobVerbose)
        {
            std::cerr << "found " << INS_Disassemble(ins) << std::endl;
        }
...
INT32 Usage()
{
    std::cerr << "This tool tests rewrite memory operand prefixed with segment register" << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    {
        std::cerr << "Failed to allocate memory above the 32 bit address space" << std::endl;
        exit(2);
    }
    if (KnobVerbose)
    {
        std::cerr << "replacement_ptr = " << (void*)replacement_ptr << std::endl;
    }
