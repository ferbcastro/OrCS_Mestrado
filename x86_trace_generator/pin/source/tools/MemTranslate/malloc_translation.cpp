...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "malloc_translation.out", "specify output file name");
...
VOID SysBefore(ADDRINT num, ADDRINT* arg0, ADDRINT* arg1, ADDRINT* arg2, ADDRINT* arg3, ADDRINT* arg4, ADDRINT* arg5)
{
    ...
    {
        case SYS_open:
            // The filename std::string needs to be translated
...
VOID Instruction(INS ins, VOID* v)
{
    ...
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(SysBefore), IARG_SYSCALL_NUMBER, IARG_SYSARG_REFERENCE, 0,
    ...
    else
    {
        ...
        {
            OutFile << "Instrumenting at " << std::hex << INS_Address(ins) << " " << INS_Disassemble(ins).c_str() << " operand #" << i
                    << std::endl;
...
INT32 Usage()
{
    std::cerr << "This tool tests memory address translation with malloc replacement" << std::endl;
