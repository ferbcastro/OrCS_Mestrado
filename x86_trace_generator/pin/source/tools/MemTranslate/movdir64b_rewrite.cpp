...
void CheckAppBuffersAfterMovdir64b(char* srcPtr, char* dstPtr, int iteration)
{
    if (iteration < 3)
    {
        ...
        ASSERTX(strcmp(OriginalSrcBuff[iteration], OriginalSrcAddr[iteration]) ==
                0); // verify that the original source std::string didn't change
    ...
    switch (iteration)
    {
        case 0:
            ...
            ASSERTX(strcmp(OriginalDstBuff[0], OriginalDstAddr[0]) ==
                    0); // verify that the original destination std::string of the first movdir64b didn't change
            break;
        case 1:
    ...
    if (INS_IsMovdir64b(ins))
    {
        ...
        switch (CountInst)
        {
            case 0: // Rewrite both source and destination addresses
                ...
                INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(SetupBeforeMovdirInstrumentation), IARG_CALL_ORDER,
                               CALL_ORDER_FIRST, IARG_MEMORYOP_EA, 0, IARG_MEMORYOP_EA, 1, IARG_UINT32, CountInst, IARG_END);
                INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(CalcRewrittenAddressSrc), IARG_UINT32, CountInst,
                               IARG_RETURN_REGS, scratchRegSrc, IARG_END);
                INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(CalcRewrittenAddressDst), IARG_UINT32, CountInst,
                               IARG_RETURN_REGS, scratchRegDst, IARG_END);
                INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(CheckAppBuffersAfterMovdir64b), IARG_MEMORYOP_PTR, 0,
                               IARG_MEMORYOP_PTR, 1, IARG_UINT32, CountInst, IARG_END);
                break;
