...
ADDRINT PIN_FAST_ANALYSIS_CALL memoryCallback(PIN_MEM_TRANS_INFO* memTransInfo, VOID* v)
{
    ...
    if (!ifTrue)
    {
        std::cout << "Callback shouldn't be excuted because ifRoutine returned false." << std::endl;
        PIN_ExitProcess(1);
    }
...
VOID Fini(INT32 code, VOID* v)
{
    if (!foundTwoMemOpIns)
    {
        std::cout << "Didn't found two memory operands instruction." << std::endl;
        PIN_ExitProcess(1);
    }

    if (callbackCounter != expectedCallbackCounter)
    {
        std::cout << "Then-analysis routine executed different number of times than memory callback." << std::endl;
        std::cout << "callbackCounter: " << callbackCounter << ", "
             << "expectedCallbackCounter: " << expectedCallbackCounter << std::endl;
        PIN_ExitProcess(1);
    }
}
