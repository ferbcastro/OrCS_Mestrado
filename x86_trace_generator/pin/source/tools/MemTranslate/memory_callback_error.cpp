...
ADDRINT PIN_FAST_ANALYSIS_CALL returnMemoryFunc(ADDRINT memea_orig, ADDRINT memea_callback, ADDRINT ip, std::string* dis)
{
    OutFile << "returnMemoryFunc: " << std::hex << ip << " " << *dis << std::endl;
...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "memcallerr.out", "specify output file name");
...
INT32 Usage()
{
    std::cerr << "This tool tests memory address translation callback assert" << std::endl;
