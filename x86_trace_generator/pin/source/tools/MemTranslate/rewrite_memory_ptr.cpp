...
static VOID CheckMemPtr(ADDRINT ea, ADDRINT ptr)
{
    if (ptr != NegValue(ea))
    {
        std::cerr << "Wrong IARG_MEMORYOP_PTR value: " << std::hex << ptr << " != " << NegValue(ea) << std::endl;
        PIN_ExitProcess(1);
    }
}
...
INT32 Usage()
{
    std::cerr << "This tool tests IARG_MEMORY*_PTR arguments for rewritten memory operands" << std::endl;
    return -1;
}
