...
std::atomic< BOOL > AllExit {FALSE};
std::atomic< int > ActiveReaders {0};
std::atomic< BOOL > IsActiveWriter {FALSE};
...
std::set< THREADID > appThreads;

static VOID AppThreadStart(THREADID tid, PINTOOL_REGISTER* regval)
{
...
static void InstrumentRtn(RTN rtn, VOID*)
{
    ...
    {
        RTN_Open(rtn);
        RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(AppThreadStart), IARG_THREAD_ID, IARG_REG_REFERENCE, RegThreadInfo,
                       IARG_END);
        RTN_Close(rtn);
    }
    ...
    if (RTN_Name(rtn) == "TellPinThreadCount" || RTN_Name(rtn) == "_TellPinThreadCount")
    {
        ...
        RTN_Open(rtn);
        RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(GetThreadCount), IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
        RTN_Close(rtn);
    }
    ...
    if (RTN_Name(rtn) == "InstrumentedWithPin" || RTN_Name(rtn) == "_InstrumentedWithPin")
    {
        ...
        {
            case TEST_LOCK_INTEGRITY:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestLockIntegrity), IARG_THREAD_ID, IARG_REG_VALUE,
                               RegThreadInfo, IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_LOCK_STRESS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestLockStress), IARG_THREAD_ID, IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_MUTEX_INTEGRITY:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestMutexIntegrity), IARG_THREAD_ID, IARG_REG_VALUE,
                               RegThreadInfo, IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_MUTEX_STRESS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestMutexStress), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_MUTEX_TRYSTRESS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestMutexTryStress), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_WRITER_INTEGRITY:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestWriterIntegrity), IARG_THREAD_ID, IARG_REG_VALUE,
                               RegThreadInfo, IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_WRITER_STRESS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestWriterStress), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_WRITER_TRYSTRESS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestWriterTryStress), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_READER_STRESS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestReaderStress), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_READER_TRYSTRESS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestReaderTryStress), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_RW_INTEGRITY:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestReaderWriterIntegrity), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_RW_STRESS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestReaderWriterStress), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_RW_TRYSTRESS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestReaderWriterTryStress), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_SEMAPHORE:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestSemaphore), IARG_REG_VALUE, RegThreadInfo,
                               IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            case TEST_TRYLOCKS:
                RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(DoTestTryLocks), IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
                break;
            default:
                ASSERTX(0);
        }
...
static bool CheckIfDone(THREAD_INFO* info, UINT32* done)
{
    ...
    {
        if (RunningWorkers-- <= 1) AllExit.store(TRUE);
    }
    if (AllExit.load())
    {
        *done = 1;
