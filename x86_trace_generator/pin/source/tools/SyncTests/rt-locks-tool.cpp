...
static void InstrumentIns(INS ins, VOID*)
{
    ...
    {
        FoundDoWork = true;
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(UseLocksWorker), IARG_END);
    }
}
...
static void InstrumentRtn(RTN rtn, VOID*)
{
    if (RTN_Name(rtn) == "DoGetLockWithPin")
    {
        FoundDoGetLock = true;
        RTN_Open(rtn);
        RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(UseLocksScheduler), IARG_END);
        RTN_Close(rtn);
    }
    ...
    {
        RTN_Open(rtn);
        RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(CannotRunTest), IARG_END);
        RTN_Close(rtn);
    }
}
