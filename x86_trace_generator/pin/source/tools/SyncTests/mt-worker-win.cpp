...
extern "C" __declspec(dllexport) void InstrumentedWithPin(volatile uint32_t*);
extern "C" __declspec(dllexport) void TellPinThreadCount(uintptr_t);
extern "C" __declspec(dllexport) void TellPinThreadStart();

typedef void (*FUNPTR1)(volatile uint32_t*);
typedef void (*FUNPTR2)(uintptr_t);
...
extern "C" void InstrumentedWithPin(volatile uint32_t* done)
{
    // Pin tool places instrumentation here.
    // It stores non-zero to 'done' when the thread should exit.
}

extern "C" void TellPinThreadCount(uintptr_t threadCount)
{
