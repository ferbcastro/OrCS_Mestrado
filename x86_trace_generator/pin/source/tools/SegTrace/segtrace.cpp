...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "segtrace.out", "trace file");
...
static VOID Instruction(INS ins, VOID* v)
{
    ...
    if (INS_SegmentPrefix(ins))
    {
        ...
        if ((seg == REG_SEG_GS) || (seg == REG_SEG_FS))
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(OnSegGsOrFsReference), IARG_UINT32, seg, IARG_REG_VALUE, seg,
        ...
        else
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(OnSegReference), IARG_UINT32, seg, IARG_REG_VALUE, seg,
    ...
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(OnSegWriteBefore), IARG_UINT32, seg, IARG_REG_VALUE, seg,
                       IARG_REG_CONST_REFERENCE, seg, IARG_INST_PTR, IARG_THREAD_ID, IARG_END);

        INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(OnSegWriteAfterContext), IARG_CONST_CONTEXT, IARG_END);

        // This is only possible in Linux 32 bits so using IARG_REG_REFERENCE for segment is allowed (although not changing value)
        INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(OnSegWriteAfter), IARG_UINT32, seg, IARG_REG_VALUE, seg,
                       IARG_REG_REFERENCE, seg, IARG_REG_CONST_REFERENCE, REG_SEG_GS_BASE, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_END);
    ...
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(OnSyscallBefore), IARG_SYSCALL_NUMBER, IARG_SYSARG_VALUE, 0,
                       IARG_SYSARG_VALUE, 1, IARG_SYSARG_VALUE, 2, IARG_SYSARG_VALUE, 3, IARG_SYSARG_VALUE, 4, IARG_REG_VALUE,
                       REG_SEG_FS, IARG_REG_VALUE, REG_SEG_GS, IARG_INST_PTR, IARG_THREAD_ID, IARG_END);

        INS_InsertCall(ins, IPOINT_AFTER, MAKE_AFUNPTR(OnSyscallAfter), IARG_SYSRET_VALUE, IARG_REG_VALUE, REG_SEG_FS,
