...
{
    std::cout << "GlobalHandler: Caught unexpected exception. " << PIN_ExceptionToString(pExceptInfo) << std::endl << std::flush;
...
EXCEPT_HANDLING_RESULT DivideHandler(THREADID tid, EXCEPTION_INFO* pExceptInfo, PHYSICAL_CONTEXT* pPhysCtxt, VOID* appContextArg)
{
    ...
    {
        std::cout << "GlobalHandler: Caught divide by zero exception. " << PIN_ExceptionToString(pExceptInfo) << std::endl << std::flush;
...
VOID InstrumentDivide(INS ins, VOID* v)
{
    ...
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(EmulateIntDivide), IARG_REG_REFERENCE, REG_GDX, IARG_REG_REFERENCE,
                       REG_GAX, IARG_REG_VALUE, REG(INS_OperandReg(ins, 0)), IARG_CONTEXT, IARG_THREAD_ID, IARG_END);
        INS_Delete(ins);
    }
    if ((INS_Mnemonic(ins) == "DIV") && (!INS_OperandIsReg(ins, 0)))
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(EmulateMemDivide), IARG_REG_REFERENCE, REG_GDX, IARG_REG_REFERENCE,
                       REG_GAX, IARG_MEMORYREAD_EA, IARG_MEMORYREAD_SIZE, IARG_CONTEXT, IARG_THREAD_ID, IARG_END);
        INS_Delete(ins);
    }
}
...
INT32 Usage()
{
    std::cerr << "This tool emulates divide" << std::endl;
    std::cerr << KNOB_BASE::StringKnobSummary() << std::endl << std::flush;
    return -1;
}
