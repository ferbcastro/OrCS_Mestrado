...
static BOOL DebugInterpreter(THREADID, CONTEXT*, const std::string&, std::string*, VOID*);
...
INT32 Usage()
{
    std::cerr << "This tool demonstrates the use of extended debugger commands" << std::endl;
 ...
 */
static BOOL DebugInterpreter(THREADID tid, CONTEXT* ctxt, const std::string& cmd, std::string* result, VOID*)
{
...
static VOID DoBreakpoint(const CONTEXT* ctxt, THREADID tid)
{
    ...
    // Construct a std::string that the debugger will print when it stops.  If a debugger is
 ...
 *
 * Returns: A std::string with the whitespace trimmed.
 */
