...
KNOB< std::string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "call_function_natively.out", "specify file name");
...
static INT32 Usage()
{
    std::cerr << "This pin tool domonstrate the difference between PIN_CallApplicationFunction() and "
            "PIN_CallApplicationFunctionNatively."
         << std::endl;
...
{
    TraceFile << depthString() << "Calling world from helloX (instrumented " << instType << ")" << std::endl;
}
...
static void callApplicationFunction(THREADID tid, CONTEXT* ctxt, BOOL natively)
{
    ...
    TraceFile << depthString() << "Calling helloX from tool " << (natively ? "natively" : "regularly") << std::endl;
    callingDepth++;
    ...
    callingDepth--;
    TraceFile << depthString() << "Returned from helloX in tool with '" << ret << "'" << std::endl;
}
...
static void Instruction(INS ins, VOID* v)
{
    ...
    {
        INS_InsertCall(ins, IPOINT_BEFORE, MAKE_AFUNPTR(HandleCallInHello), IARG_ADDRINT, v, IARG_END);
    }
}

// Utility function to get the address of an RTN using its name
static ADDRINT FindAddressOfRtn(IMG img, const std::string& rtnName)
{
...
static void Image(IMG img, VOID* v)
{
    ...
    RTN_Open(mainRtn);
    RTN_InsertCall(mainRtn, IPOINT_AFTER, MAKE_AFUNPTR(AfterMain), IARG_THREAD_ID, IARG_CONTEXT, IARG_END);
    RTN_Close(mainRtn);
}
