...
KNOB< std::string > KnobFilesPrefix(KNOB_MODE_WRITEONCE, "pintool", "p", "", "prefix to add to all generated filenames");
 ...
 * replay time.
 */
static std::string LogImageNameToReplayImageName(const std::string& logImage) { return logImage + FAKE_IMAGE_SUFFIX; }
 ...
 * replay time.
 */
static std::string ReplayImageNameToLogImageName(const std::string& replayImage)
{
...
static void ParseImageLoadLine(std::string& imageName, ADDRINT* startAddr, USIZE* size, ADDRINT* offset, BOOL* mainExe,
                               std::vector< RTN_INFO >* rtns)
{
...
static void ParseImageUnloadLine(std::string& imageName)
{
    char imgNameBuffer[MAX_FILENAME_LENGTH];
...
static IMG FindNamedImg(const std::string& imgName)
{
...
static void ReplayImageEntry()
{
    ...
    switch (tag)
    {
        case 'L':
        {
            ...
            for (std::vector< RTN_INFO >::iterator it = rtns.begin(); it < rtns.end(); it++)
            {
...
int main(int argc, char* argv[])
{
    ...
    std::string logFile = KnobFilesPrefix.Value() + ".record";

    if (logging)
    {
        std::string traceFile = KnobFilesPrefix.Value() + "_rec.out";
    ...
    { // Replaying
        std::string traceFile = KnobFilesPrefix.Value() + "_play.out";
        trace            = fopen(traceFile.c_str(), "w");
        ASSERTX(NULL != trace);
