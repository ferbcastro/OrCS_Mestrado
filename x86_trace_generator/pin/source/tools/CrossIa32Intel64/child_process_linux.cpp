...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        if (status != 0)
            std::cout << "Child report: Grand child process failed with status " << WEXITSTATUS(status) << std::endl;
        else
            std::cout << "Child report: Grand child process exited successfully" << std::endl;
    }
    return 0;
}
